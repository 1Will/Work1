 ...
 */
public class ListSpareOutBillDetailAction extends ValueListAction {
	...
	private String allSheBeiNo;//设备编号
	private String allUseTypes;//备件用途
	...
	public ListSpareOutBillDetailAction(
	...
	public void prepare() {
		...
		if (!StringUtils.isEmpty(request.getParameter("allBanZu"))) {
		 ...
		 //设备
		if (!StringUtils.isEmpty(request.getParameter("allSheBeiNo"))) {
			this.allSheBeiNo = request.getParameter("allSheBeiNo");
		}
        //班组
		if (!StringUtils.isEmpty(request.getParameter("allUseTypes"))) {
			this.allUseTypes = request.getParameter("allUseTypes");
		}
	...
	private String saveDetails() {
		spareOutBillDetailManager.saveSpareOutBillDetail(allDetailIds,allNumber,allborrowerPeople,
				allNewOrOld,allOutDate,allComment,allOutPeopleIds,allSheBei,allBanZu,allSheBeiNo,allUseTypes);
		
		String oldInBillid  = request.getParameter("oldInBillid");
		if(oldInBillid != null && !oldInBillid.equals("")) {
			SpareInBill s = new SpareInBill();
			s.setId(Long.parseLong(oldInBillid));
			spareOutBill.setOldForNew(s);
