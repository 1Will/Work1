 ...
 * @version Id:
 */
public class ListOldSpareLocationToFrameAction extends ValueListAction{
	...
	public ListOldSpareLocationToFrameAction(SpareInventoryDetailManager spareInventoryDetailManager,
			SpareInventoryManager spareInventoryManager,
			UserManager userManager, com.yongjun.tdms.service.asset.spare.warehouseInfo.warehouse.WarehouseManager
		    newwarehouseManager){
	...
	protected Map getRequestParameterMap() {
       ...
       // map.put("spareInventoryBill.id",this.getId("spareInventoryBill.id"));
		...
		for(Warehouse  ws :user.getWarehouses()){
			Warehouse  jjws = newwarehouseManager.loadJJWareHouse("JJ-"+ws.getCode());
			if(jjws!=null){
				wareList.add(jjws);
			}
		}
		if(wareList.size() > 0) {
			map.put("warehouse.id", wareList.get(0).getId());
		}
		return map;
	}
