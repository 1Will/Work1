 ...
 */
public class EditOldSpareMaintainBillAction extends PrepareAction{
	...
	public EditOldSpareMaintainBillAction(SpareOutBillManager spareOutBillManager,
...
//		this.getLogger().logStore(spareOutBill,logContent);
		...
		if (isNew) {
			this.addActionMessage(this.getText("维修单" + spareOutBill.getCode() + "保存成功",
					Arrays.asList(new Object[] { spareOutBill.getCode() })));
			return NEW;
		} else {
			this.addActionMessage(this.getText("维修单" + spareOutBill.getCode() + "修改成功",
					Arrays.asList(new Object[] { spareOutBill.getCode() })));
			return SUCCESS;
		}
