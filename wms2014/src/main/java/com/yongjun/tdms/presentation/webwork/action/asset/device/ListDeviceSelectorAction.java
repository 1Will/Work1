...
/**
 * <p>Title: ListDeviceSelectorAction
 * <p>Description: 设备选择页面访问控制类</p>
 * <p>Copyright: Copyright (c) 2007 yj-technology</p>
 * <p>Company: www.yj-technology.com</p>
 * @author zbzhang@yj-technology.com
 * @version $Id:$
 */
public class ListDeviceSelectorAction extends ValueListAction {
	private static final long serialVersionUID = 7888301441332837172L;
...
private final Log log = LogFactory.getLog(getClass());
	...
	public ListDeviceSelectorAction(DepartmentManager departmentManager, 
			...
			if (this.hasId("oldDeviceIds")) {
				if (!this.isFirst() || this.isSearch()) {
					...
					for (int i = 0; i < oldDeviceId.length; i++) {
						if(!"".equals(oldDeviceId[0].replace("[","").replace("]", "").trim())){
							//将需要过滤得设备ID放入List集合中
							oldDeviceIds.add(Long.valueOf(oldDeviceId[i].replace("[","").replace("]", "").trim()));
						}
					}
				}
...
//	}
	...
	protected Map getRequestParameterMap() {
		...
		if (!this.hasId("deivceStatus.code")) {
			if (getDeviceStateId().size() != 0) {
				map.put("filterDeviceStateIds",getDeviceStateId());
			}
		}
		return map;
	}
	
	public List<Long> getDeviceStateId() {
		//获取设备状态的所有Id值
		List<Long> allDeviceStateIds = this.codeValueManager.LoadAllIdsByCode(CodeConstants.DEVICE_STATUS);
		//获取已经过滤得设备状态集合
		List<CodeValue> deviceStates = getDeviceStatus();
		//过滤"所有"值
		deviceStates.remove(0);
		//过滤现有还存在的设备状态ID
		for (CodeValue deviceState : deviceStates) {
			allDeviceStateIds.remove(deviceState.getId());
		}
		return allDeviceStateIds;
	}
...
//	}
	...
	public List<CodeValue> getDeviceStatus() {
