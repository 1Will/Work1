...
public class EditAcceptTechAlterDetailAction extends PrepareAction{
    ...
    public EditAcceptTechAlterDetailAction(AcceptBillManager acceptBillManager,AcceptBillDetailManager acceptBillDetailManager,
	 ...
	 */
	 public String save() {
	    	...
	    	//设置明细类别
	    	this.acceptBillDetail.setDetailType(YearPlanDetailCategory.TECH_ALTER);
	    	
	    	this.acceptBillDetailManager.storeToolingAcceptBillToolingMakeDetail(acceptBillDetail);
			if (isNew) {
				this.addActionMessage(this.getText("techAlterDtl.add.success", Arrays
						.asList(new Object[] { acceptBillDetail.getTooling().getName() })));

				return NEW;
			} else {
				this.addActionMessage(this.getText("techAlterDtl.edit.success", Arrays
						.asList(new Object[] {acceptBillDetail.getTooling().getName() })));
				return SUCCESS;
			}
	    }
