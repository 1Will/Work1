 ...
 * @version Id:
 */
public class EditOldSpareInBillForOldToNewAction extends PrepareAction {
	...
	public EditOldSpareInBillForOldToNewAction(SpareInBillManager spareInBillManager,
			SupplierManager supplierManager, UserManager userManager,
			EventNewManager eventNewManager, WarehouseManager warehouseManager,
			CodeValueManager codeValueManager,SpareOutBillManager spareOutBillManager) {
	...
	public void prepare() throws Exception {
		if (null == spareInBill) {
			...
			if (this.hasId("spareInBill.id")) {
				...
				String outDetailids =spareInBill.getOutDetailids() ;
				...
				if(outId!=null&&!outId.equals("")){
					...
					List<SpareOutBillDetail> details = this.spareOutBillManager.loadAllSpareOutBill(Long.parseLong(outId),spareInBill.getOutDetailids());
					List<SpareInBillDetail> inDetails  = new ArrayList<SpareInBillDetail>();
					for(SpareOutBillDetail detail:details){
						if(outDetailids.equals("")){
							outDetailids =detail.getId()+"";
						}else{
							outDetailids +=","+detail.getId();
						...
						inDetail.setDepartment(detail.getDepartment());
						inDetail.setCreator(detail.getCreator());
						inDetail.setCreatedTime(new Date());
						inDetail.setDeptName(detail.getDeptName());
						inDetail.setInstatus(SpareInBillDetailStatus.NEW);
						inDetail.setNumber(detail.getNumber());
						inDetail.setSpare(detail.getSpareLocation().getSpare());
						inDetail.setCode("JJ-"+detail.getSpareLocation().getSpare().getSpareNo());
						inDetail.setName(detail.getSpareLocation().getSpare().getName());
						inDetail.setTaxPrice(detail.getSpareLocation().getSpare().getUnitPrice());
						inDetail.setUnitPrice(detail.getSpareLocation().getSpare().getUnitPrice());
						inDetail.setLastModifiedTime(new Date());
						inDetail.setLastOperator(detail.getCreator());
						inDetail.setTotalPrice(inDetail.getNumber() *inDetail.getTaxPrice());
						inDetail.setSpare(detail.getSpareLocation().getSpare());
//						Location l = new Location();
//						l.setId(51577l);
//						inDetail.setLocation(l);
						inDetails.add(inDetail);
					}
					spareInBillManager.storeAllSpareInBillDetail(inDetails);
					
					
				}
				
				
				spareInBill.setOutDetailids(outDetailids);
	...
	public String save() {
		...
		if(outId!=null&&!outId.equals("")){
			SpareOutBill outBill = this.spareOutBillManager.loadSpareOutBill(Long.parseLong(outId));
			outBill.setOldForNew(spareInBill);
			this.spareOutBillManager.storeSpareOutBill(outBill);
			List<SpareOutBillDetail> details = this.spareOutBillManager.loadAllSpareOutBill(outBill.getId());
			List<SpareInBillDetail> inDetails  = new ArrayList<SpareInBillDetail>();
			for(SpareOutBillDetail detail:details){
				if(outDetailids.equals("")){
					outDetailids =detail.getId()+"";
...
//				inDetail.setLocation(l);
		...
		if (isNew) {
			this.addActionMessage(this.getText("入库单" + spareInBill.getCode() + "保存成功",
			...
			this.addActionMessage(this.getText("入库单" + spareInBill.getCode() + "保存失败",
					Arrays.asList(new Object[] { spareInBill.getCode() })));
			return SUCCESS;
		}
	}
	 ...
	 * @throws DaoException 
	 */
	public List<Warehouse> getAllValidWarehouse() throws DaoException {
		List<Warehouse> wareList = new ArrayList<Warehouse>();
//		wareList = this.newwarehouseManager.loadAllWarehouse();
		String[] keys={"storageGrade","disabled","oldWarehouse"};
		String[] values={"209","0","Y"};
...
//			wareList=this.newwarehouseManager.loadAllWarehouse();
	...
	public String getInBillName(){
		String  inBillName="";
		String outId = request.getParameter("outId");
		if(outId!=null&&!outId.equals("")){
		SpareOutBill outBill = this.spareOutBillManager.loadSpareOutBill(Long.parseLong(outId));
		inBillName ="旧件"+outBill.getName();
		}
		return inBillName;
	...
	public String getInBillId(){
		...
		if(outId!=null&&!outId.equals("")){
		SpareOutBill outBill = this.spareOutBillManager.loadSpareOutBill(Long.parseLong(outId));
		Warehouse ws = newwarehouseManager.loadJJWareHouse("JJ-"+outBill.getWarehouse().getCode());
		inBillId =ws.getId()+"";
		}
		return inBillId;
