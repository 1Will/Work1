 ...
 * @version $
 */
public class EditPurchaseTeachAlterDetailAction extends PrepareAction {
	...
	public EditPurchaseTeachAlterDetailAction(
    ...
    public String save() {
    	...
    	//设置明细类别
    	this.subscribeDtl.setDetailType(YearPlanDetailCategory.TECH_ALTER);
    	
    	this.subscribeManager.storeSubscribeDetail(subscribeDtl);
    	
		if (isNew) {
			this.addActionMessage(this.getText("tachAlterDetail.add.success", Arrays
					.asList(new Object[] { subscribeDtl.getTooling().getName() })));

			return NEW;
		} else {
			this.addActionMessage(this.getText("tachAlterDetail.edit.success", Arrays
					.asList(new Object[] {subscribeDtl.getTooling().getName() })));
			return SUCCESS;
		}
    }
