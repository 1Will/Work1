...
<script language="JavaScript" type="text/JavaScript"> 
	window.onload = function () {
		//		document.forms["area"].elements["ext"].click();
 		document.all.frame.src='${req.contextPath}/preview/check/listCheckPlanImplsItems.html';
	...
	function checkPointPlan_openDialog() {
		popupModalDialog('${req.contextPath}/popup/checkPlanSelector.html',600,400);
	}
    ...
    <@ww.form namespace="'/base/area'" name="'area'" action="'saveArea'" method="'post'" validate="true">
        ...
        <@buttonBar>
    		<@vbutton class="button" value="选择检查计划" onclick="checkPointPlan_openDialog();"/>
    	</@buttonBar>
        <@inputTable>
        	<tr>
        		<@ww.textfield label="'检查实施编号'" name="'area.code'" value="" cssClass="'underline'" readonly="true"/>
        		<@ww.textfield label="'检查实施名称'" name="'area.code'" value="" cssClass="'underline'" readonly="true" required="true"/>
        	</tr>
        	 <tr>
            ...
            <tr>
             ...
             <tr>
            	<@pp.datePicker label="'计划执行时间'" name="'repair.estimate.time1'"
			     			value="" cssClass="'underline'" size="15"/>
            	<@ww.textfield label="'检查计划内容'" name="'area.code'" value="" cssClass="'underline'" readonly="true"/>
            </tr>    
            <tr>
                <@ww.textfield label="'计划实施要求'" name="'area.code'" value="" size="50" cssClass="'underline'" readonly="true"/>
                <@ww.textfield label="'备注'" name="'area.code'" value="" size="50" cssClass="'underline'" readonly="true"/>
            </tr>
            <tr>            
            	<@pp.datePicker label="'实际实施时间'" name="'repair.estimate.time0'"
			     			value="" cssClass="'underline'" size="15" required="true"/>
            	<@ww.textfield label="'检查摘要'" name="'area.code'" size="50" value="" cssClass="'underline'" required="true"/>
            </tr>
             <@audit2 />
        </@inputTable>
       	<@buttonBar>
       		<@redirectButton value="${action.getText('save')}" url="#"/>	
        	<@redirectButton value="${action.getText('back')}" url="${req.contextPath}/preview/check/listCheckPlanImpls.html"/>	
        </@buttonBar>
    </@ww.form>
	...
	<ul id="beautytab">
		<li><a id="fees" href="${req.contextPath}/preview/check/listCheckPlanImplsItems.html" target="frame" class="selectedtab">设备定期检查实施明细</a></li>
	</ul>
