    ...
    <@ww.form namespace="'/base/area'" name="'area'" action="'saveArea'" method="'post'" validate="true">
        ...
        <@inputTable>
             <tr>
                ...
                <@ww.textfield label="'润滑标准名称'" name="'area.code'" value="" cssClass="'underline'"  required="true"/>
            </tr>
            <@deviceSelector/>
            <tr>
            ...
            <tr>
            	<@ww.select label="'实施部门'"
	                    	...
	                    	required="true"
	              />
            	<@ww.textfield label="'备注'" name="'area.code'" value="" cssClass="'underline'"  size="70"/>
            </tr>
        </@inputTable>
       	<@buttonBar>
       		<@redirectButton value="${action.getText('save')}" url="${req.contextPath}/preview/device/smoothInfo/listSmoothRules.html"/>	
        	<@redirectButton value="${action.getText('back')}" url="${req.contextPath}/preview/device/smoothInfo/listSmoothRules.html"/>	
        </@buttonBar>
    </@ww.form>
