...
<script language="JavaScript" type="text/JavaScript"> 
	window.onload = function () {
		//		document.forms["area"].elements["ext"].click();
 		document.all.frame.src='listRunRecordInfoHistories.html';
	...
	function device_OpenDialog() {
	 ...
	 <@ww.form name="'listForm'" action="'searchProducts'" method="'post'" validate="true">
	 	  ...
	 	  <@inputTable>
	 	  	  <tr>
	 	  	  	<@ww.textfield label="'停机记录编号'" name="'area.code'" value="'0001'" cssClass="'underline'" disabled="true" readonly="true" />
	 	  	  	<@ww.textfield label="'停机记录名称'" name="'area.code'" value="" cssClass="'underline'" required="true" />
	 	  	  </tr>
	 	  	  <@deviceSelector/>		 	  	 	 
		 	  <tr>
		 	  	<@pp.datePicker label="'停机开始时间'" name="'repair.estimate.time0'"
	     				value="" cssClass="'underline'" size="15" required="true"/>
	     		<@pp.datePicker label="'停机结束时间'" name="'repair.estimate.time0'"
	     				value="" cssClass="'underline'" size="15" required="true"/>
		 	  </tr>
		 	  <tr>
		 	  	<@ww.textfield label="'停机原因'" size="50" name="'area.code'" value="" cssClass="'underline'" required="true" />
	 	  ...
	 	  <@buttonBar>
	        	...
	        	<@redirectButton  value="返回" url="listRunRecordInfo.html"/>
	     </@buttonBar>
	</@ww.form>
	
	<ul id="beautytab">
		<li><a id="planItems" onclick="activeTab(this);" href="listRunRecordInfoHistories.html" target="frame" class="selectedtab">停机历史记录</a></li>	
	</ul>
