...
<tr class="input">
    ...
    <script type="text/javascript">
	  			...
	  			<#if user.filiale?exists>  
		      		$('filiale.id').value = ${user.filiale.id?if_exists};
	  			<#else>
        ...
        function validate() {
          ...
          if ('' == $('user.name').value) {
            ...
            return false;
          	...
          	if (!isValidLengthValue($('user.name').value,0,20)) {
          ...
          <#if user.new>
          	//当用户类型是系统用户
			if ('SYSTEM_USER' == $('user.type').value) {
		          //验证密码
		          if ('' == $('newPassword').value) {
		            ...
		            return false;
		          	...
		          	if (!isValidLengthValue($('newPassword').value,0,30)) {
