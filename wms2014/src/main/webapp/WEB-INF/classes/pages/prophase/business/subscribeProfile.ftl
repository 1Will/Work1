...
<script language="javascript">	
	  window.onload = function()  { 
         ...
         //单据类型
		<#if subscribe.detailKind?exists>
         document.getElementById("detailKind.id").value = #{subscribe.detailKind.id};
         <#else>
          document.getElementById("detailKind.id").value='204';
       </#if>
	   }
	    <#--
	    /*
	  ...
	  function  saveValidate(){
       ...
       //验证费用来源
	     ...
	     /**验证日期 */
		  ...
		  if(document.getElementById("subscribe.comment").value!=''){
