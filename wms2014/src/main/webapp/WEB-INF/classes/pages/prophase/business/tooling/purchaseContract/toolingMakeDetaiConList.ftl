...
<@framePage title="${action.getText('ToolingMakeDetailPurchaseConcat')}">	
  <style type="text/css">
   ...
   <script language="javascript" src="${req.contextPath}/javascripts/calendar-setup.js"></script>  
	<@ww.form name="'listForm'" namespace="'/toooling/purchaseContract'"  action="'searchToolingMakeDetailPurchaseConcat'" method="'post'">
		<@ww.token name="searchToolingMakeDetailPurchaseConcatToken"/>
       ...
       <@buttonBar>
			...
			<#assign confirmMessage = "${action.getText('confirm.delete')}${action.getText('ToolingMakeDetailPurchaseConcat')}?" />
...
<script language="javascript">
	window.onload=function(){
        <#if req.getParameter('errorFlag')?has_content>
   	       alert("${action.getText('delete.ToolingMakeDtl.failure')}");
        </#if>
        <#if purchaseBill.id?exists> 
		...
		<#if valueAry?exists>
	       ...
	       if(valueAry!=""){
	          ...
	          for(var i=0;i<ary.length;i=i+3){
	              setPurchaseConcatValue(ary[i],ary[i+1],ary[i+2]);
	          }
	       }
	    </#if>
        }
     function setPurchaseConcatValue(subscribeDtlId,subscribeDtlAmountValue,subscribeDtlUntiPriceValue){
	 ...
	 function open_toolingMakeDtlDialog(purchaseBillId,toolingMakeDtlId){
		 var url = '${req.contextPath}/toooling/purchaseContract/editToolingMakeDetailPurchaseConcat.html?readOnly=${req.getParameter('readOnly')?if_exists}&purchaseBill.id='+purchaseBillId;	
		 if(toolingMakeDtlId != null){
		 	url = url + "&toolingMakeDetail.id=" + toolingMakeDtlId
		 }
		 popupModalDialog(url,800,600);
		 var reloadUrl = "${req.contextPath}/toooling/purchaseContract/listToolingMakeDetailPurchaseConcat.html?readOnly=${req.getParameter('readOnly')?if_exists}&purchaseBill.id=#{purchaseBill.id}";
	 ...
	 //弹出采购单明细查询页面
        function open_PurchaseBillDialog(){
        		var url = '${req.contextPath}/toooling/purchaseContract/listPurchaseBillchooses.html?detailType=TOOLING_MAKE&purchaseBill.id='+#{purchaseBill.id};
	    ...
	    */ 
	    ...
	    function changeTotalePrice(){
	        ...
	        if(!checkInputUnitPrice()){
	          ...
	          if (selector.length) {
				for (var i = 0; i < selector.length; i++) {
			         number=parseFloat(formatDigital(valueselector[i].value));
