...
<base target="_self">
	...
	<script language="javascript">
		window.onload=function(){
	    	 ...
	    	 document.getElementById("category.id").value = #{purchaseBillDetail.toolingCategory.id};
	   		</#if>
	 	...
	 	function validateToolingMake(){
			if ('' != document.forms[0].elements["purchaseBillDetail.graphNo"].value) {
			 ...
			 if(document.getElementById("purchaseBillDetail.name").value==''){
		    ...
		    if ('' != document.forms[0].elements["purchaseBillDetail.name"].value) {
			 ...
			 //验证数量是否为空,以及格式
       		...
       		if(document.getElementById("purchaseBillDetail.amount").value==''){
		 		alert('${action.getText('input.number')}');
		 		return false;
				}
	 		if(document.getElementById("purchaseBillDetail.unitPrice").value==''){
        	...
        	if ('' == document.forms["listForm"].elements["calUnit.id"].value) {
	 		...
	 		if(document.getElementById("purchaseBillDetail.reqDeliveryDate").value==''){ //验证日期
	    		alert('${action.getText('purchaseBillDtl.reqDeliveryDate.not.null')}');
	    		return false;
	   		 }
	   		 
	   		 var date=document.getElementById("purchaseBillDetail.reqDeliveryDate").value;
				if(!isDate("purchaseBillDetail.reqDeliveryDate")){
		 		 alert("${action.getText('select.right.purchaseBillDtl.reqDeliveryDate')}");
		 		return false;
		 		}
	       	if(document.getElementById("purchaseBillDetail.comment").value!=''){//验证字符长度
	 ...
	 /* 判断数量格式 */
	  function checkInputNumber(){
	  	var number = document.getElementById("purchaseBillDetail.amount").value;
	     ...
	     /* 判断单价格式 */
	  	function checkInputUnitPrice(){
	      	var unitPrice = document.getElementById("purchaseBillDetail.unitPrice").value;
