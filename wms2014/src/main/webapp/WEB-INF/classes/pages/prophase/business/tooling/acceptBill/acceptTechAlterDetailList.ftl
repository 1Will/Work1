...
<@framePage title="${action.getText('repairPurchaseMaintenanceDetail.title')}">
	<@ww.form namespace="'/tooling/acceptBillSelector'" name="'techAlter'" action="'searchAcceptTechAlterDetails'" method="'post'">
		...
		<@vlh.checkbox property="id" name="techAlterIds">
	 ...
	 <#--<#assign establish = ''/>
      ...
      <#--  <@vcolumn title="${action.getText('comment')}">
       ...
       <@buttonBar>
			...
			<#assign confirmMessage = "${action.getText('confirm.delete')}${action.getText('deteletechAlterDetail')}?" />
            <@vsubmit name="'delete'" value="'${action.getText('delete')}'">
                <@ww.param name="'onclick'" value="'return confirmDeletes(\"techAlterIds\", \"${confirmMessage}\")'"/>
                <@ww.param name="'disabled'" value="${valueListNoRecords?string}"/>
            </@vsubmit>
           <@vbutton name="'new'"  class="button" value="${action.getText('fromPurchaseContnactSelect')}" onclick="open_purchaseContractDtlDialog()"/>
      <#-- <@vbutton name="'establish'"  class="button" value="${action.getText('创建台帐')}" onclick="open_establishAccountDialog(techAlterIds)"/>-->
       </@buttonBar>
        </#if>
        </#if>
	</@ww.form>
...
<script language="javascript">
	...
	//新建、更新
	function open_toolingMakeDtlDialog(subscribeId,techAlterDtlId){
		 var url = '${req.contextPath}/tooling/acceptBillSelector/editAcceptTechAlterDetail.html?readOnly=${req.getParameter('readOnly')?if_exists}&acceptBill.id='+subscribeId;	
		 if(techAlterDtlId != null){
		 	url = url + "&acceptBillDetail.id=" + techAlterDtlId
		 }
		 popupModalDialog(url,800,600);
		 var reloadUrl = "${req.contextPath}/tooling/acceptBillSelector/listAcceptTechAlterDetail.html?readOnly=${req.getParameter('readOnly')?if_exists}&acceptBill.id=#{acceptBill.id}";
		//如果是点击项目号进入维护页面，保存当前页，已使在维护页面点击关闭，会回到当前页
        if (null != techAlterDtlId) {
        	reloadUrl = reloadUrl + '&pagingPage=${pagingPage?if_exists}'
        }
		self.location.href = reloadUrl;
	}
	function open_purchaseContractDtlDialog(){
		var url = '${req.contextPath}/tooling/acceptBillSelector/listPurchaseContractDtlOfTeachAlterSelector.html?detailType=TECH_ALTER&subscribeId='+#{acceptBill.id};
...
//创建台帐
       function  open_establishAccountDialog(boxName){
        ...
        var checkBoxIds = document.getElementsByName("techAlterIds");
	...
	function customize_validate() {
	     if (0 != document.getElementsByName("techAlterIds").length) {    //判断明细列表是否有记录
	 ...
	 function retrieveAcceptRepairMaintennaceDetailIdText(){
	    var allrepairMaintennanceDtlId = document.getElementsByName("techAlterIds");
        var ary = new Array();
	 ...
	 function informationValidate(){
	  //验证备注长度
       ...
       for(var i=0;i<allrepairMaintennanceDtlId.length;i++) {
          	if(''!= allComment[i].value){
		  		var comment = allComment[i].value;
