...
<@htmlPage title="${action.getText('acceptList.title')}">
	<@ww.form namespace="'/tooling/acceptBillSelector'" action="'searchAcceptBills'" method="'post'">
		...
		<@buttonBar>
			...
			<@redirectButton value="${action.getText('new')}" url="${req.contextPath}/tooling/acceptBillSelector/editAcceptBill.html?toolingDevFlag=${toolingDevFlag?if_exists}"/>
		   </#if>
		</@buttonBar>
		<@list title="${action.getText('acceptBill')}" 
		includeParameters="acceptBillNo|readOnly|acceptBilName|department.id|acceptDate_start|acceptDate_end|supplier.name|acceptPeople.name|merchandiseName|specification|model|toolingDevFlag|onlyValid|onlyInvalid"
