...
<@framePage title="${action.getText('payDetails.title')}">	
	...
	<@ww.form name="'listForm'" action="'searchPayDetails'" method="'post'">
		...
		<@ww.hidden name="'purchaseBill.id'" value="#{purchaseBill.id}"/>
		 ...
		 <#assign itemNo=1/>
		<@list title="" excel=false setupTable=false  includeParameters="purchaseBill.id|readOnly" fieldMap="like:purchaseBill.id">
        ...
        window.onload=function(){
         <#if purchaseBill.id?exists>
                parent.document.getElementById("alreadyPayOut").value = '${purchaseBill.alreadyPayOut?if_exists}';
           </#if>
        }
        
            	function open_payDtlDialog(purchaseBillId,payDtlId) {
            		var url = '${req.contextPath}/prophase/editPayDetail.html?readOnly=${req.getParameter('readOnly')?if_exists}&purchaseBill.id=' + purchaseBillId;
            		if (null != payDtlId) {
