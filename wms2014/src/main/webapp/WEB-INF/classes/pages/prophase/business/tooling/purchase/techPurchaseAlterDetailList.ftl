...
<@framePage title="${action.getText('teachAlterDetail.title')}">
	...
	<@ww.form namespace="'/tooling/prophase/business'" name="'listForm'" action="'searchPurchaseTechAlterDetails'" method="'post'">
		<@ww.token name="searchTeachAlterDetailToken"/>
...
<script language="javascript">
	window.onload = function(){
		<#if req.getParameter('errorFlag')?has_content>
   	       alert("${action.getText('delete.TeachAlter.failure')}");
        </#if>
   		//更新当前采购单状态
	...
	function open_toolingMakeDtlDialog(subscribeId,toolingMakeDtlId){
		 var url = '${req.contextPath}/tooling/prophase/business/editPurchaseTeachAlterDetail.html?subscribe.id='+subscribeId+'&readOnly=${req.getParameter('readOnly')?if_exists}';	
		 if(toolingMakeDtlId != null){
		 	url = url + "&subscribeDtl.id=" + toolingMakeDtlId
		 }
		 popupModalDialog(url,800,600);
		 var reloadUrl = "${req.contextPath}/tooling/prophase/business/listPurchaseTechAlterDetails.html?subscribe.id=#{subscribe.id}&readOnly=${req.getParameter('readOnly')?if_exists}";
	...
	function open_quarterPlanToolingMakeDtlDialog(){
		var url = '${req.contextPath}/tooling/prophase/business/listPurchaseTeachAlterDetailFromQuarterPlanDetail.html?detailType=TECH_ALTER&subscribeId='+#{subscribe.id};
