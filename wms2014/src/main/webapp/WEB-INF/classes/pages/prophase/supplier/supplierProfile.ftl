...
<@htmlPage title="${supplierEditTitle}">
    ...
    <script language="JavaScript" type="text/JavaScript"> 
     ...
     window.onload = function() {
       ...
       <#if supplier.companyType?exists>
         document.getElementById("companyType.id").value = #{supplier.companyType.id};
       </#if>
       <#if supplier.supplierType?exists>
         document.getElementById("supplierType.id").value = #{supplier.supplierType.id};
       </#if>
        <#if supplier.country?exists>
         document.getElementById("country.id").value = #{supplier.country.id};
       </#if>
        <#if supplier.tradeType?exists>
         document.getElementById("tradeType.id").value = #{supplier.tradeType.id};
       </#if>
        <#if supplier.level?exists>
         document.getElementById("level.id").value = #{supplier.level.id};
       </#if>
       ...
       function levelChange_OpenDialog(supplierLevelHistoryId) {
   	   ...
   	   function validateLevel(){
		   ...
		   return false;
   	     ...
   	     if(document.getElementById("supplier.enName").value!='')	{
	     ...
	     -->
	     if(document.getElementById("supplier.legalPerson").value!='')	{
	     ...
	     if (!isValidLength(document.forms[0],"supplier.registeredFunds",0,50)) {
		   ...
		   return false;
   	     ...
   	     if(document.getElementById("level.id").value ==''){//供应商等级不能为空值
   	       alert('${action.getText('supplierLevel.not.null')}');
   	       return false;
   	       }
   	      if(document.getElementById("supplier.comment").value!='')	{
