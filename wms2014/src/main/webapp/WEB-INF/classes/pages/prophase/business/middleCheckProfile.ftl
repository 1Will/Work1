...
<script language="javascript">	
	...
	if(document.getElementById("middleCheck.content").value==''){
	...
	if(document.getElementById("middleCheck.resultCheck").value!=''){
		if(!isValidLength(document.forms[0], "middleCheck.resultCheck", null, 250)){
			alert("${action.getText('middleCheck.resultCheck.length')}");
			return  false;
			   }
			}
		 if(document.getElementById("middleCheck.checkDate").value==''){ //验证日期
	    alert('${action.getText('middleCheck.checkDate.not.null')}');
	    return false;
	    }
	 var date=document.getElementById("middleCheck.checkDate").value;
