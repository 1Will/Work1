...
<@htmlPage title="${action.getText('pDetail.title')}">
<base target="_self">
	
	<@ww.form name="'purchaseBillDtl'" action="'savePurchaseBillDetail'" method="'post'" validate="true">
		...
		<@inputTable>
			...
			<tr>
				<@ww.textfield label="'${action.getText('unitPrice')}'" name="'purchaseBillDtl.unitPrice'" value="'${purchaseBillDtl.unitPrice?if_exists}'" cssClass="'underline'" disabled="false" required="true" onchange="'calAllPrice();'"/>
			...
			<tr>
			 <@ww.select label="'${action.getText('tooling.calUnit')}'" required="false" name="'calUnit.id'" 
				...
				<@ww.textfield label="'${action.getText('total.Price')}'" name="'totalPrice'" value="'${purchaseBillDtl.totalPrice?if_exists}'"  cssClass="'underline'"   readonly="true"/>
			...
			<@pp.datePicker label="'${action.getText('reqDeliveryDate')}'" name="'purchaseBillDtl.reqDeliveryDate'"
					value="'${(purchaseBillDtl.reqDeliveryDate?string('yyyy-MM-dd'))?if_exists}'" cssClass="'underline'" size="10" required="true"/>
 ...
 <script language="javascript">
 	window.onload=function(){
	   	<#if purchaseBillDtl.calUnit?exists>
         		document.forms[0].elements["calUnit.id"].value = #{purchaseBillDtl.calUnit.id?if_exists};
       </#if>
	}
	/*
	 * 根据单价和数量,计算总价
	*/
	 function calAllPrice() {  
	   if ('' != document.forms[0].elements["purchaseBillDtl.unitPrice"].value && isDoubleNumber("purchaseBillDtl.unitPrice")) {
	     var unitPrice = parseFloat(formatDigital(document.forms[0].elements["purchaseBillDtl.unitPrice"].value));
	     if ('' != document.forms[0].elements["purchaseBillDtl.amount"].value && isNumber("purchaseBillDtl.amount")) {
	       var number = parseFloat(formatDigital(document.forms[0].elements["purchaseBillDtl.amount"].value));
	       document.forms[0].elements["totalPrice"].value = unitPrice*number;
	       document.forms[0].elements["purchaseBillDtl.totalPrice"].value =unitPrice*number;
	     } else {
	...
	function validatePurchase(){
			if ('' != document.forms[0].elements["purchaseBillDtl.graphNo"].value) {
			 ...
			 if(document.getElementById("purchaseBillDtl.name").value==''){
				 alert('${action.getText('EnculodeName.not.null')}');
				 return false;
				}
		    if ('' != document.forms[0].elements["purchaseBillDtl.name"].value) {
	 ...
	 if(document.getElementById("purchaseBillDtl.amount").value==''){
		 alert('${action.getText('input.number')}');
		 return false;
		}
	 if(document.getElementById("purchaseBillDtl.unitPrice").value==''){
    ...
    if ('' == document.forms[0].elements["calUnit.id"].value) {
	 ...
	 if(document.getElementById("purchaseBillDtl.reqDeliveryDate").value==''){ //验证日期
	    alert('${action.getText('purchaseBillDtl.reqDeliveryDate.not.null')}');
	    return false;
	    }
	 var date=document.getElementById("purchaseBillDtl.reqDeliveryDate").value;
		if(!isDate("purchaseBillDtl.reqDeliveryDate")){
		 ...
		 return false;
	       ...
	       if(document.getElementById("purchaseBillDtl.comment").value!=''){//验证字符长度
