...
<base target="_self">
	...
	function  saveVlidate(){
	  if(document.getElementById("payDetail.payMoney").value==''){ //验证付款金额
	    alert('${action.getText('payDetail.payMoney.not.null')}');
	    return false;
	    }
	  if(isNotEmpty(document.forms[0],"payDetail.payMoney")) {
	     if (!isDoubleNumber("payDetail.payMoney")){
		    alert("${action.getText('payDetail.payMoney.maxLength')}");
		       return false;
      ...
      if(document.getElementById("payDetail.payDate").value==''){ //验证日期
	    alert('${action.getText('middleCheck.payDetail.payDate')}');
	    return false;
	    }
	 var date=document.getElementById("payDetail.payDate").value;
	...
	if(document.getElementById("payDetail.comment").value!=''){
