...
<base target="_self">
	...
	<@ww.form name="'sparePart'" action="'saveSparePart'" method="'post'" validate="true">
	...
	if(document.getElementById("sparePart.spareName").value==''){
   ...
   if(document.getElementById("sparePart.spareModel").value!='')	{
	if(!isValidLength(document.forms[0], "sparePart.spareModel", null, 50)){
			alert("${action.getText('sparePart.spareModel.length')}");
			return  false;
			   }
	}	
	if(document.getElementById("sparePart.spareSpecification").value!='')	{	   
     ...
     if(document.getElementById("sparePart.installPosition").value!='')	{	  	
