      ...
      //按月份供应商pdf
       function open_subscribeSupplierPdf(){
	      // var purIds = getCheckId();
	       ...
	       var url="${req.contextPath}/reports/subscribe/subscribeDtlMonthSupplierView.pdf?supplier="+$("supplier.name").value+"&month="+$("month").value;  
           url = encodeURI(url);
	   ...
	   function open_subscribeSupplierXls(){
	      ...
	      if(checkInvalidParms()==false){
	       ...
	       var url="${req.contextPath}/reports/subscribe/subscribeDtlMonthSupplierView.xls?supplier="+$("supplier.name").value+"&month="+$("month").value;  
           url = encodeURI(url);
           window.open(url,"_new","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=screen.width,height=screen.height,left=0,top=0"); 
	   } 
