...
<@htmlPage title="${action.getText('toolingBorrowEdit.title')}">
    <@ww.form namespace="'/runmaintenance/tooling/record'" name="'toolingBorrowBill'" action="'saveToolingBorrowBill'" method="'post'" validate="true">
        ...
        <@inputTable>
        	...
        	<#if toolingBorrowBill.returnFlag == false>
        	  <@eam2008_ToolingSelector flag="Borrow"/>
        	<#else>
        	  <@eam2008_ToolingSelector toolingStatus="true" flag="Borrow"/>
        	</#if>
            ...
            <tr>
	        	<#assign usedDeviceName = ''/>
				<#if toolingBorrowBill.device?exists>
				 <#assign usedDeviceName = "${toolingBorrowBill.device.name}" />
				</#if>
            	<td align="right" valign="top"><label class="label">${action.getText('usedDevice')}:</label></td>
            	<td>
	            	<input type="text" name="device.name" class="underline"  value="${usedDeviceName}"  maxlength="150" disabled="true"/>
			 		<#if toolingBorrowBill.returnFlag == false>
			 		<a onClick='device_OpenDialog("${req.contextPath}/popup/deviceSelector.html")'>
		 					<img src="${req.contextPath}/images/icon/files.gif" align=absMiddle border=0/>
		 			</a>
		 			</#if>
		 		</td>
		        <#assign usedDeviceId = ''/>
				<#if toolingBorrowBill.device?exists>
				 <#assign usedDeviceId = "#{toolingBorrowBill.device.id}" />
				</#if>
				<input type="hidden" name="device.id" value="${usedDeviceId}" />
            ...
            <#if toolingBorrowBill.id?exists>
             <tr><td colspan="4"><HR align="middle" size="1" width="95%" color="#999999" noshade></td></tr>
		 	  <tr>
                ...
                <td id="requestState" style="display:none" align="right" valign="top"><span id="produceNumSapn" class="required" style="display:none">*</span><label class="label">${action.getText('producrNumber')}:</label></td>
                <td id="requestState0" style="display:none">
                  <input type="text" name="toolingBorrowBill.produceNum" class="underline"  value="${toolingBorrowBill.produceNum?if_exists}"  maxlength="150" disabled="true"/>
                </td>
                <#--
            	<@ww.textfield label="'${action.getText('producrNumber')}'"  name="'toolingBorrowBill.produceNum'" value="'${toolingBorrowBill.produceNum?if_exists}'" cssClass="'underline'" required="false"/>
                -->
                <input type="hidden" name="produceNum" value="" />
		 	  </tr>
            <tr id="requestState1" style="display:none">
	        	<@ww.textarea  label="'${action.getText('productTailState')}'" 
	        	...
	        	<@ww.textarea  label="'${action.getText('tooling.repairMaintenance')}'" 
        ...
        <@buttonBar>
         <#if !(action.isReadOnly())>
	      ...
	      <@redirectButton name="back" value="${action.getText('back')}" url="${req.contextPath}/runmaintenance/tooling/record/listToolingBorrowBills.html?toolingDev_Flag=${req.getParameter('toolingDev_Flag')?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}"/>
	      <#if toolingBorrowBill.id?exists>
		...
		<script language="JavaScript" type="text/JavaScript">
	      ...
	      window.onload = function () {
	        ...
	        */
	        <#if toolingBorrowBill.id?exists>
	         document.getElementById("toolingBorrowBill.produceNum").value=formatDigital(
		     document.getElementById("toolingBorrowBill.produceNum").value);
		    </#if>
	        <#if (toolingBorrowBill.produceNum)?exists>
	        var produceNum = document.forms["toolingBorrowBill"].elements["toolingBorrowBill.produceNum"].value;
	        document.forms["toolingBorrowBill"].elements["produceNum"].value = produceNum; 
	        </#if>       
            ...
            if (returnFlag) {                                         //如果归还，置returnStateFlag标签为选中状态
              ...
              document.getElementById("requestState").style.display='inline';
		      document.getElementById("requestState0").style.display='inline';
		      document.getElementById("requestState1").style.display='inline';
		      document.getElementById("requestState2").style.display='inline';
		      document.getElementById("requestState3").style.display='inline';
            }
            /*
	    ...
	    <script language="JavaScript" type="text/JavaScript">
	      ...
	      */
	      function input_isDisabled(msg) {
	        document.forms["toolingBorrowBill"].elements["toolingBorrowBill.produceNum"].disabled=msg;
	      ...
	      */
	      function img_isDisplay(msg) {
            document.getElementById("checkerImg").style.display=msg;
	        document.getElementById("storemanImg").style.display=msg;
	        document.getElementById("toolingBorrowBill_toolingBorrowBill.inDateTm_trigger").style.display=msg;
	        document.getElementById("toolingBorrowBill_toolingBorrowBill.checkDateTm_trigger").style.display=msg;	        
	      }
	      /*
	      ...
	      */
	      function span_isDisplay(msg) {
	        document.getElementById("checkerSpan").style.display=msg;
	        document.getElementById("storemanSpan").style.display=msg;
	        document.getElementById("produceNumSapn").style.display=msg;
	      }
		  ...
		  function changeReturnState() {
	        ...
	        <#else>
		      ...
		      document.getElementById("requestState0").style.display='none';
		      document.getElementById("requestState1").style.display='none';
		      document.getElementById("requestState2").style.display='none';
		      document.getElementById("requestState3").style.display='none';
		      document.forms["toolingBorrowBill"].elements["toolingState"].value = 'TOOLING_BORROW';
		      span_isDisplay("none");
	          input_isDisabled(true);
	          img_isDisplay("none");	      
		  ...
		  function customize_validate() {
		   if(document.getElementById("toolingBorrowBill.billName").value==''){
		  ...
		  function borrowReturnDateTmValidate() {
		  	var borrowReturnDateTm=document.getElementById("toolingBorrowBill.borrowReturnDateTm").value;
		    ...
		    //验证数量是否为空,以及格式
	  if (document.getElementById("produceNumSapn").style.display == 'inline') {	    
          var number = document.forms[0].elements["toolingBorrowBill.produceNum"].value;
          if ('' == number) {            //验证是否为空
           alert("${action.getText('input.toolingBorrowBill.produceNum')}");
           return false;
          } else if (!isNumberBetweenBoolen(number, 10000000001, 0)){ //验证范围
            alert("${action.getText('toolingBorrowBill.produceNum.maxLength')}");
            return false;
          }
		   ...
		   if (document.getElementById("checkerSpan").style.display == 'inline') {
		      if (document.forms["toolingBorrowBill"].elements["checker.name"].value == '') {
		        alert("${action.getText('select.checker.name')}");
		        return false;
		      }
		    }
		    if (document.getElementById("storemanSpan").style.display == 'inline') {
		      if (document.forms["toolingBorrowBill"].elements["storeman.name"].value == '') {
		        alert("${action.getText('select.store.name')}");
		        return false;
		      }
		    }
         if ('' != document.getElementById("toolingBorrowBill.checkDateTm").value) {
              if (!checkDateTm()) {
                return false;
              }
            }
            if ('' != document.getElementById("toolingBorrowBill.inDateTm").value) {
              if (!inDateTmValidate()) {
                return false;
              }
            }
            ...
            if(document.getElementById("toolingBorrowBill.repairMaintenance").value!=''){
		  ...
		  function checkDateTm() {
		    var borrowReturnDateTm=document.getElementById("toolingBorrowBill.borrowReturnDateTm").value;
		    var checkDateTm=document.getElementById("toolingBorrowBill.checkDateTm").value;
