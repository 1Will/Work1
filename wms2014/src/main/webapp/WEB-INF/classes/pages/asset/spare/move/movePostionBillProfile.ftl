...
<script language="javascript">
		<#if movePostionBill.storageGrade?exists>
	    	$('storageGrade.id').value = #{movePostionBill.storageGrade.id?if_exists};
	    </#if>
	...
	window.onload=function()
	{
		...
		<#if movePostionBill.warehouse?exists>
			$('warehouse.id').value=#{movePostionBill.warehouse.id?if_exists};
	    <#else>
...
<#--
    ...
    function checkInput(){
      ...
      //验证日期
	  if(document.getElementById("movePostionBill.moveTime").value==''){
        alert('${action.getText('movePostionBill.moveTime.null')}');
        return false;
	  }
	  var date=document.getElementById("movePostionBill.moveTime").value;
