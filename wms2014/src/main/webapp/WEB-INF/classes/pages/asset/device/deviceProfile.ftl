  ...
  <script language="javascript">
	...
	function initNav(num) {
		for (var i = 1; i <= noCount[num].length; i ++) {
			...
			document.getElementById(i).innerHTML = navItem[temp];
		}
		c=num;
	}
	...
	function scrollNav(pos,pageCount) {
		...
		for (var i = 0,j = 1; i < noCount[x].length; i ++,j ++) {
			if(j<=4){
				var t = noCount[x][i];
				document.getElementById(j).innerHTML = navItem[t];
			} 
		}
		 <#if device.id?exists>
    ...
    window.onload = function () {
	  ...
	  <#if device.id?exists>
		...
		if (getCookie("activeFrameId") != null) {
	      document.getElementById(getCookie("activeFrameId")).className = "selectedtab";
		} else {
