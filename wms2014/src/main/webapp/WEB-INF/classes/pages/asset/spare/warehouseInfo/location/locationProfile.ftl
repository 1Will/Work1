...
<script language="javascript">
	<#if location.storageGrade?exists>
          $('storageGrade').value ='#{location.storageGrade.id?if_exists}';         
	...
	window.onload= function(){
  	...
  	<#if location.warehouse?exists>
			$('warehouse').value='${location.warehouse.id?if_exists}';
	    	//设置同步
	    	DWREngine.setAsync(false); 
	    	//获得仓库的值后触发DWR 级联库区 
    ...
    //库区
	    <#if location.regional?exists>
		    $('regional').value='${location.regional.id}';
		    <#elseif req.getParameter('regional')?exists>
		    $('regional').value='${req.getParameter('regional')}';
	    </#if>
	//库位类型
	    <#if location.locationType?exists>
		    $('locationType').value='${location.locationType.id}';
		    <#elseif req.getParameter('locationType')?exists>
		    $('locationType').value='${req.getParameter('locationType')}';
	    </#if>
	//承载类型 
	    <#if location.bearload?exists>
		    $('bearload').value='${location.bearload.id}';
		    <#elseif req.getParameter('bearload')?exists>
		    $('bearload').value='${req.getParameter('bearload')}'; 
	    </#if>
	//混放模式 
	    <#if location.mixmode?exists>
		    $('mixmode').value='${location.mixmode.id}';
		    <#elseif req.getParameter('mixmode')?exists>
		    $('mixmode').value='${req.getParameter('mixmode')}';  
	    </#if>
	//相同备件存放 
	    <#if location.samestore?exists>
		    $('samestore').value='${location.samestore.id}';
		    <#elseif req.getParameter('samestore')?exists>
		    $('samestore').value='${req.getParameter('samestore')}'; 
	    </#if>
	//状态��
