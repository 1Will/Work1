<@inputTable>
	...
	<script language="JavaScript" type="text/JavaScript"> 
	  ...
	  function checkInvalidParms() {
			...
			if (document.getElementById("toolingTypeDetail.id").value == -1) {
		    ...
		    if (document.getElementById("toolingStatus.code").value == -1) {
				document.getElementById("toolingStatus.code").value = '';
			}
			//<#if req.getParameter('hiddenCheckBox')?has_content>
			//<#else>
	...
	function toolTypeValueChange() {      //处理部门选择框的事件
	  ...
	  if (selectedToolTypeId != -1) {
		  for (var i = 0; i < detailTypeForSortToolType.length; i++) {
	       ...
	       document.getElementById("toolingTypeDetail.id").value = -1;
