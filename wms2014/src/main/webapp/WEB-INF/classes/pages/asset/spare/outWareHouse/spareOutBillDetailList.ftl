	            ...
	            <input type="text" name="banzu" class="underline"  value="${object.banzu?if_exists}" onchange="" maxlength="20" size="7" style="text-align:right"/>
	             ...
	             <@vcolumn title="${action.getText('以旧换新')}" >
	                ...
	                <script>
	                    ...
	                    strOld = strOld+item;
	                    strNew = strNew+item;
	                   // alert($("oldT"+item).innerHTML)
		               <#if object.newOrOld>
		                 $(strOld).checked="checked";
		                 //$("oldT"+item).style="color:red;";
		                  
		               <#else> 
		                 $(strNew).checked="checked";
		               </#if>
	               </script>
...
<script language="JavaScript" type="text/JavaScript">
    ...
    <#--    
		...
		if(208==parent.document.getElementById("storageGrade.id").value){
		...
		<#if !valueListNoRecords>
			...
			var tag1 = parent.document.getElementById("storageGrade.id");
			
			var psave = parent.document.getElementById("save");
			psave.disabled="true";
		...
		//改变父页面的总金额
	 	<#if spareOutBill.id?exists>
            parent.document.getElementById("spareOutBill.totalPrice").value = '${(spareOutBill.totalPrice?string('#,###,##0.00'))?if_exists}';
      	</#if>
    ...
    function getAllOutDateText(){
 	...
 	function getOutPeopleText(){
 	   ...
 	   if(length){
 	      for(var i =0;i<length;i++){
 	        if(""!= $(outPeopleIdTag+(i+1)).value){
 	            ary.push(detailIdSelector[i].value);
	            ary.push($(outPeopleIdTag+(i+1)).value);
 	        }else{
  		...
  		if(0==countNEW){
  		   ...
  		   return false;
  		    ...
  		    <#if spareOutBill.warehouse?exists && spareOutBill.warehouse.storageGrade.id==208>   
		    	...
		    	document.getElementById("saveAllDetail").value=allNumbers;-->
	  	    </#if>
	  	    return true;
  		}
