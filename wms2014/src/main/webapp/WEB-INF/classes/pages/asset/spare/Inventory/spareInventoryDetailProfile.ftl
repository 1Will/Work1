...
<base target="_self">
 ...
 <script language="javascript">
        ...
        window.onload=function(){
	       ...
	       if(""!= name){
	          $("spareInventoryDetail.name").value=name;
	       }
       }
      
	    <#--仓库级别级联仓库-->
	   <#if spareInventoryDetail.storageGrade?exists>
          $('storageGrade.id').value = #{spareInventoryDetail.storageGrade.id?if_exists};
		...
		<#if spareInventoryDetail.warehouse?exists>
			$('warehouse.id').value='#{spareInventoryDetail.warehouse.id?if_exists}';
	    <#else>
	  ...
	  <#if spareInventoryDetail.regional?exists>
		    $('regional.id').value='#{spareInventoryDetail.regional.id}';
	  ...
	  <#if spareInventoryDetail.location?exists>
		    $('location.id').value='#{spareInventoryDetail.location.id}';
	  <#elseif req.getParameter('location.id')?exists>
		    $('location.id').value='#{req.getParameter('location.id')?if_exists}';
	  </#if>
	  
         <#--种类 级联明细种类-->
         <#--
	   <#if spareInventoryDetail.category?exists>
     		$("category.id").value = #{spareInventoryDetail.category.id?if_exists};
     		DWREngine.setAsync(false); 
    		//回调种类的值后触发DWR 级联明细种类  
       ...
       <#if spareInventoryDetail.spareDetailType?exists>
         		$("detailCategory.id").value = #{spareInventoryDetail.spareDetailType.id?if_exists};
       </#if>
       -->
       //单位
	   	<#if spareInventoryDetail.unit?exists>
         	$("calUnit.id").value = #{spareInventoryDetail.unit.id?if_exists};
       </#if>
	  ...
	  function changeTotalPrice(){
	      var price = $("spareInventoryDetail.unitPrice").value; //单价
	      var beforeNum = $("spareInventoryDetail.actualNumber").value;//盘点前数量
	      var num = $("spareInventoryDetail.inventoryNum").value;//盘点数量
	      if("" != beforeNum){
	      ...
	      if(isDoubleNumber("spareInventoryDetail.unitPrice")){
	          $("spareInventoryDetail.inventoryTotalPrice").value=toMoney((price*num),2);//盘点金额
	          $("spareInventoryDetail.actualTotalPrice").value=toMoney((parseFloat(price)*parseInt(beforeNum)),2);//盘点前金额
	          //差额
		     $("spareInventoryDetail.marginPrice").value=toMoney(((price*num)-(parseFloat(price)*parseInt(beforeNum))),2);
	      }
	   ...
	   function changeActualTotalPrice(){
	       ...
	       var beforePrice = $("spareInventoryDetail.actualTotalPrice").value;//盘点前金额
	       var beforeNum = $("spareInventoryDetail.actualNumber").value;//盘点前数量
	       ...
	       if("" != num && isDoubleNumber("spareInventoryDetail.inventoryNum")){
			       ...
			       //差额
			       $("spareInventoryDetail.marginPrice").value=toMoney(((price*num)-beforePrice),2);
			        
			       //差异
			       $("spareInventoryDetail.different").value=(parseInt(num)-parseInt(replaceComma(beforeNum)));
		   }
	   }
  
	  //验证字段格式
	  function subscribeValidate(){
	  	  if(document.getElementById("spareInventoryDetail.code").value!=''){//验证图号
	        if(!isValidLength(document.forms[0],"spareInventoryDetail.code",null,50)){
	          ...
	          $("spareInventoryDetail.code").focus();
	          return false;
	         }
	      }
	      if(document.getElementById("spareInventoryDetail.name").value==''){//验证品名是否为空
              alert('${action.getText('input.proudctName')}');
              $("spareInventoryDetail.name").focus();
              return false;	       
	      }
	     if(document.getElementById("spareInventoryDetail.name").value!=''){//如果品名不为空,验证品名的输入长度
	     ...
	     if(document.getElementById("spareInventoryDetail.model").value==''){//验证规格是否为空
               alert('${action.getText('input.spareInventoryDetail.specification')}');
               $("spareInventoryDetail.model").focus();
               return false;
	     }
	     if(document.getElementById("spareInventoryDetail.model").value!=''){//验证规格的输入长度
	     ...
	     if(document.getElementById("detailCategory.id").value=='' || document.getElementById("detailCategory.id").value=='-1'){//验证明细种类是否为空
	      ...
	      <#--验证单价是否为空,以及格式 
         if((""==$("spareInventoryDetail.unitPrice").value)||(0 == $("spareInventoryDetail.unitPrice").value)) {
                alert("请填入单价");
                $("spareInventoryDetail.unitPrice").focus();
			    return false;
         ...
         <#--验证仓库级别-->
         if("" == $("storageGrade.id").value || -1==$("storageGrade.id").value){
            alert("请选择仓库级别");
            $("storageGrade.id").focus();
            return false;
         }
         <#--验证仓库-->
         if("" == $("warehouse.id").value || -1==$("warehouse.id").value){
            alert("请选择仓库");
            $("warehouse.id").focus();
            return false;
         }
         <#--验证库区-->
         if("" == $("regional.id").value || -1==$("regional.id").value){
            alert("请选择库区");
            $("regional.id").focus();
            return false;
         }
         <#--验证库位-->
         if("" == $("location.id").value || -1==$("location.id").value){
            alert("请选择库位 ");
            $("location.id").focus();
            return false;
         }
         <#--验证盘点数量-->
         if("" == $("spareInventoryDetail.inventoryNum").value){
             alert("请填写盘点数量");
             $("spareInventoryDetail.inventoryNum").focus();
             return false;
	     ...
	     if(document.getElementById("spareInventoryDetail.comment").value!=''){//如果备注的长度不为空  验证备注的长度
	         if(!isValidLength(document.forms[0],"spareInventoryDetail.comment",null,250)){
	          ...
	          $("spareInventoryDetail.comment").focus();
	          return false;
	         }
	     }
	    return true;
	  ...
	  function autoCompleteSpare(){
	      ...
	      //获取品名
	      var spareName = $('spareInventoryDetail.name').value;
	      //获取规格型号
	      var model = $('spareInventoryDetail.model').value;
	      var orderNo = $('spareInventoryDetail.orderNo').value;
	      //获取种类
	      var categoryId = $('category.id').value;
	      //获取明细种类
	      //var spareDetailTypeId= $('detailCategory.id').value;
	      //获取生产厂家
	      var factoryName = $('spareInventoryDetail.factoryName').value;
	      //工装或设备标识
	      var toolingDevFlag = $('toolingDevFlag').value;
	      var storageGrade = $('storageGrade.id').value;
	      var warehouse = $('warehouse.id').value;
