...
<@htmlPage title="${action.getText('spareOutBillSearcher')}">
	 <STYLE TYPE="text/css" >
	 .displayRed{
	   font-weight: bold;
	   color: #FFFFFF;
    ...
    .definedLength{
	 ...
	 <@ww.form name="'spareOutBills'" action="'searchSpareOutBills'" method="'post'">
         <@ww.token name="searchSpareOutBillsToken"/>
          <@ww.hidden name="'readOnly'" value="'${req.getParameter('readOnly')?if_exists}'"/>
	 	  <#include "./spareOutBillSearcher.ftl"/>
	       <#assign itemNo = 0/>
        <@buttonBar>
        	<@vsubmit value="'${action.getText('search')}'" onclick="'return checkInvalidParms();'"/>
        	 <#if !(action.isReadOnly())>
	        <@redirectButton value="${action.getText('new')}" url="${req.contextPath}/spare/editSpareOutBill.html?readOnly=${req.getParameter('readOnly')?if_exists}"/>
             </#if>
             <#--
        ...
        <@list title="${action.getText('list')}" 
          	...
          	<#if (object.disabled)>
          	<@vcolumn title="${action.getText('spareOutBill.code')}" property="code" sortable="desc">
            	<@alignLeft />
            </@vcolumn>
          	<#else>
             <@vcolumn title="${action.getText('spareOutBill.code')}" property="code" sortable="desc">
                <a href="editSpareOutBill.html?spareOutBill.id=#{object.id}&readOnly=${req.getParameter('readOnly')?if_exists}&pagingPage=${pagingPage?if_exists}">${object.code}</a>
            	<@alignLeft/>
             </@vcolumn>
            </#if>	
            <@vcolumn title="${action.getText('spareOutBill.name')}" property="name" sortable="desc">
            	<@alignLeft/>
            </@vcolumn>
            <@vcolumn title="${action.getText('spareOutBill.department')}" property="department.name" sortable="desc">
            	<@alignLeft/>
            </@vcolumn>
             <@vcolumn title="${action.getText('spareOutBill.borrower')}" property="borrower" sortable="desc">
            	<@alignLeft/>
            </@vcolumn>
            
            <@vcolumn title="${action.getText('spareOutBill.outPeople')}" property="outPeople.name" sortable="desc">
            	<@alignLeft/>
            </@vcolumn>
           
            <@vcolumn title="${action.getText('spareOutBill.outDate')}" property="outDate" format="yyyy-MM-dd" sortable="desc">
            	<@alignLeft/>
            </@vcolumn>
            ...
            <@vcolumn title="${action.getText('warehouse')}"  property="warehouse.name" sortable="desc">
               ...
               <@vcolumn title="${action.getText('inWarehouse')}"  property="inWarehouse.name" sortable="desc">
				<@alignLeft />
            </@vcolumn>                 
            ...
            <@vcolumn title="${action.getText('spareOutBill.comment')}" property="comment" >
            	<@alignLeft/>
            </@vcolumn>
          <#assign status=''/>
...
function open_SpareOutBillReportPdf(){
      ...
      var url='${req.contextPath}/reports/spare/listAllSpareOutBill.pdf?spareOutBillNo='+spareOutBillNo+
...
function open_SpareOutBillReportXls(){
      ...
      var warehouseId = document.forms[0].elements["warehouse.id"].value;
      var inWarehouseId = document.forms[0].elements["inWarehouse.id"].value;
      var url='${req.contextPath}/reports/spare/listAllSpareOutBill.xls?spareOutBillNo='+spareOutBillNo+
       '&spareOutBillName='+spareOutBillName+'&deptId='+deptId+'&borrower='+borrower+'&outPeople='+
       outPeople+'&outDate_start='+outDate_start+'&outDate_end='+outDate_end + '&warehouseId=' + warehouseId + '&inWarehouseId=' + inWarehouseId;
      url = encodeURI(url);
      window.open(url,"_new","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=screen.width,height=screen.height,left=0,top=0");
	   ...
	   function excelButtonManagers(type){
		 ...
		 if(op.style.display=="none"){
			 $("excelPrints").value="${action.getText('pdfReportPrints')}";
       ...
       function open_spareOutBillPdf(){
	       var outDate = $("outDate_start").value;
