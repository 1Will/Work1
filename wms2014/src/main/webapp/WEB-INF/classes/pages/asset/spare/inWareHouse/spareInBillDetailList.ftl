...
<@framePage title="${action.getText('')}">	
	<@ww.form namespace="'/spare'" name="'listForm'" action="'searchSpareInBillDetails'" method="'post'">
		...
		<#assign arrivalNumIdentityName = 'arrivalNum' + '${arrivalNo}'/>
		 ...
		 <@list title=""  includeParameters="spareInBill.id" fieldMap="" >
            ...
            <input type="hidden" name="spareNo" value="${object.code}"/>
            
            <@vcolumn title="${action.getText('sibd.code')}" property="code">
            	<@alignLeft />
            </@vcolumn>
            <@vcolumn title="${action.getText('sibd.name')}" property="name">
            	<@alignLeft />
            </@vcolumn>
           
            <@vcolumn title="${action.getText('sibd.model')}" property="spare.modelSpecs" attributes="width='55'">
            	<@alignLeft />
            </@vcolumn>
             <#--
             <@vcolumn title="${action.getText('sibd.specification')}" property="spare.specification">
            	<@alignLeft />
            </@vcolumn>
            -->
             <@vcolumn title="${action.getText('sibd.category')}" property="spare.category.name">
            	<@alignLeft />
            </@vcolumn>
            <#--
            ...
            -->
              <@vcolumn title="${action.getText('sibd.unit')}" property="spare.unit.value">
            	<@alignLeft />
             </@vcolumn>
			<#-- 部门 -->
             <@vcolumn title="${action.getText('申购部门')}" >
				      ...
				      <#else>
				     <select name="${departmentIdentityName}" disabled>
					    <@ww.iterator value="departments" id="department">
					      <option value="<@ww.property value="id"/>">
					      	<@ww.property value="name"/>
					      </option>
            ...
            <@vcolumn title="${action.getText('location')}">
				...
				<#if object.location?exists>
				    <#assign LocationCode = "${object.location.code?if_exists}" />
				    <#if object.location?exists>
				    	<#assign LocationId = "#{object.location.id}"/>
				    </#if>
				</#if>
				<#if '${object.instatus}' == 'NEW'>
			        <@eam2008_remoteGetLocation id="${LocationId}" code="${LocationCode?if_exists}" loopNum="${loopNum}" inputName="location.code" disabled="false"/>
                <#else>
                    <@eam2008_remoteGetLocation id="${LocationId}" code="${LocationCode?if_exists}" loopNum="${loopNum}" inputName="location.code" disabled="true"/>
                </#if>	     	 
	     	 </@vcolumn>
	     	 <#--
                <@vcolumn title="${action.getText('sibd.unit')}" property="poDetail.calUnit.value">
            	<@alignLeft />
               </@vcolumn>
             -->
           ...
           <#elseif object.spareOutBillDtl?exists><#--出库单-->
            ...
            <#if object.instatus?exists>
              <#if '${object.instatus}' == 'INWAREHOUSED'>
                  <@vcolumn title="${action.getText('spareInNumber')}">
           ...
           <#--单价 -->
           <#if notFather >
	            <@vcolumn title="${action.getText('taxPrice')}" format="${action.getText('currencyFormat')}">
	            	<input type="text" 
            ...
            <@vcolumn title="${action.getText('stocksBeforeInOrOut')}">
            ...
            <@vcolumn title="${action.getText('stocksAfterInOrOut')}" property="stocksAfterInOrOut">
            	<@alignRight />
            </@vcolumn>
        ...
        <@buttonBar>
        	...
        	<#if notFather ><#-- notFather = true 是二级库-->
        	   <#--
        	    <@vbutton name="'selectSpare'"  class="button" value="${action.getText('从备件库台账选择')}" onclick="open_selectSpareAccountDialog()"/>
               -->
                <@vbutton name="'selectPurBillDtl'"  class="button" value="${action.getText('从出库单明细选择')}" onclick="open_spareOutBillDialog()"/>
        	<#else>
        	    <@vbutton name="'selectSpare'"  class="button" value="${action.getText('spareAccountSelect')}" onclick="open_spareDialog()"/>
                <@vbutton name="'selectPurBillDtl'"  class="button" value="${action.getText('PurBillDtlSelect')}" onclick="open_purchaseBillDtlDialog()"/>
        	</#if>
...
window.onload=function(){
   <#--
	  ...
	  <#if req.getParameter('notFather')?has_content>
		  $('notFather').value=${req.getParameter('notFather')};
    </#if>
	...
	<#if spareInBill.status?exists>
		 ...
		 //改变父页面的总金额
		 <#if spareInBill.id?exists>
             parent.document.getElementById("spareInBill.totalPrice").value = '${(spareInBill.totalPrice?string('#,###,##0.00'))?if_exists}';
          </#if>
	...
	function chickArrailNum(){
	  ...
	  // var purAmount = $('purchaseNumber').value;
	  // alert(purAmount);
    ...
    function open_spareDialog(){
        var url = '${req.contextPath}/popup/spareSelector.html?inOutFlag=spareInBill&toolingDevFlag=TOOLINGDEVICE&spareBillId='+#{spareInBill.id};
    ...
    function open_spareOutBillDialog(){
         //进入仓库
        var outWarehouseId = parent.document.getElementById("warehouse.id").value;
        //来源仓库
        var warehouseId = parent.document.getElementById("outWarehouse.id").value;
	...
	function submitDetailIds(){
	  <#if spareInBill?exists>
	     <#if !spareInBill.inPeople?exists>
	         alert("请先保存入库人！");
	         var tag = parent.document.getElementById("saveSpareInBill_spareIn.inPeople");
	         tag.focus();
	         return false;
	    </#if>
	  </#if>
	  ...
	  if(0!=document.getElementsByName("spareInBillDtlIds").length){
	  	 ...
	  	 if(informationValidate()==true){				//保存时信息安全验证
		  	 ...
		  	 retrieveSpareInBillDetailDepartmentText();
	<#--	  	 retrieveSpareInBillDetailLocationText();-->
	...
	<#--function retrieveSpareInBillDetailLocationText(){
       ...
       for (var i=0; i<allSpareInBillDtlId.length; i++) {
           ...
           ary.push(document.getElementById("moveId.location.code"+i).value);
       }
      ...
      */
      function retrieveLocationCodeText() {
        ...
        for (var i=0; i<objDetailId.length; i++) {
          ...
          ary.push(document.getElementById("location.code"+i).value);
        }
        document.forms[0].elements["allLocationCodeValue"].value=ary;
      }
	//获得入库明细的所有入库数量
   ...
   function spareValidate(){
      ...
      if(length>1){
        ...
        for(var v=0;v<length;v++){
           ...
           if(null != $("location.code"+v)){
               locationCode = $("location.code"+v).value;
           }
           if(""!=spareNo && ""!=taxPrice && ""!=locationCode){
                 for(var i=v+1;i<length;i++){
		             if(spareNo == allSpareNoSelector[i].value){
		                  if(locationCode == $("location.code"+i).value){
    ...
    function informationValidate(){
	    ...
	    var allNumberValue = document.getElementsByName("spareInNumber");
	    var allCommentValue = document.getElementsByName("comment");
	    var allTaxprice = document.getElementsByName("spareIn.taxPrice");
	    var allstocksBeforeInOrOutValue = document.getElementsByName("stocksBeforeInOrOut");
	    var allPurchaseNumber = document.getElementsByName("purchaseNumber");
	 
	    
	    for(var i=0;i<allSpareInBillDtlId.length;i++){
		   
		    var inorout=allstocksBeforeInOrOutValue[i].value;
		   
		    //验证部门
		     var departmentTagName = 'department';
		     departmentTagName = departmentTagName + (i+1);
		     
		    if(''==document.forms["listForm"].elements[departmentTagName].value || '-1'==document.forms["listForm"].elements[departmentTagName].value){
	            alert("${action.getText('di')} "+(i+1)+","+" ${action.getText('row')}"+"请选择部门！");
	            document.forms["listForm"].elements[departmentTagName].focus();
	            return false;
            }
		     /* 验证库位号是否存在 */
		    if ("false" == document.getElementById("hidden.location.code"+i).value) {
		      var locationCode = document.getElementById("location.code"+i).value;
		      alert("${action.getText('di')} "+(i+1)+","+" ${action.getText('row')}"+ "," + locationCode + "${action.getText('location_notFound')}");
		      document.getElementById("location.code"+i).focus();
		      return false;
		    }
		    if ("" == document.getElementById("location.code"+i).value) {
   			  var locationCode = document.getElementById("location.code"+i).value;
		      alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ "," + locationCode + "${action.getText('please.input.location')}");
		      document.getElementById("location.code"+i).focus();
		      return false;
   			}
		    ...
		    if(''== number){								//验证入库数量是否为空
		    	alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"${action.getText('spareInBill.checkInputNumber')}");
   			...
   			var purchaseNum = allPurchaseNumber[i].value;
   			if(''!=number && ''!=purchaseNum){
   			   if(parseInt(formatDigital(number)) > parseInt(formatDigital(purchaseNum))){
   			     alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"入库数量应该小于等于采购数量！");
   				 return false;
   			   }
   			}
   			
   			var maxRuKu =allstocksBeforeInOrOutValue[i].value;
   			if(''!=number && ''!=maxRuKu){
   			   if((0-parseInt(formatDigital(number))) > parseInt(formatDigital(maxRuKu))){
   			     alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"请核对库存再做该操作！");
   				 return false;
   			   }
   			}
   			<#--
   			else if((parseInt(inorout)+parseInt(number))<0){
   			    alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"${action.getText('spareInBill.number.beforeLength')}");
   				return false;
   			}
   			else if(!isNumberBetweenBoolen(number, 1000000001, -1000000000)){		//验证输入的入库数量的范围
   				alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"${action.getText('spareInBill.number.maxLength')}");
   				return false;
   			}
   			var purchaseNum = allPurchaseNumber[i].value;
   			 
   			if(''!=number && ''!=purchaseNum){
   			   if(parseInt(number) > parseInt(purchaseNum)){
   			     alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"入库数量应该小于等于采购数量！");
   				 return false;
   				 }
   			}
		    ...
		    //验证入库单价
		    var taxprice=allTaxprice[i].value;
		  
		    if(''== taxprice){								//验证入库价是否为空
			    	alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+","+"${action.getText('spareInBill.taxprice.error')}");
			    	allTaxprice[i].focus();
			    	return false;
		    }else if (!isDoubleNumberCheck(taxprice)){   			//验证输入的入库单价的格式
	         		alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+","+"${action.getText('spareInBill.taxprice.error')}");
	         		allTaxprice[i].focus();
	         		return false;
   			}else if(0==parseFloat(taxprice)){
	   			  alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"单价请输入有效的数字！");
	   			  allTaxprice[i].focus();
	   			  return false;
   			}else if(!isNumberBetweenBoolen(taxprice), 1000000001, 0){		//验证输入的入库单价的范围
   				  alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+","+"${action.getText('spareInBill.taxprice.error')}");
   				  allTaxprice[i].focus();
   				  return false;
   			} 
		  ...
		  //	if(''!= allCommentValue[i].value){
		  	...
		  	//	if (!isValidLengthCheck(document.forms[0],comment,null,250)) {
	       		//	alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"${action.getText("spareIn.comment.maxLength")}");
		   			//return false;
	     		//}
		  	//}
		  	 // return true;-->
	  ...
	  if (selector.length) {
	     ...
	     for (var i = 1; i < selector.length+1; i++) {
	       if(valueselector[i-1].value!=""){
	         ...
	         parent.document.getElementById("spareInBill.totalPrice").value=totalPrice;
	    } 
	}
