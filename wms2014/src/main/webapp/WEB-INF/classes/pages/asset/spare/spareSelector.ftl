	 ...
	 <@ww.form name="'listForm'" action="'spareSelector'" method="'post'">
         ...
         <@buttonBar>        
          ...
          <@list title="${action.getText('spare.list')}" 
                 ...
                 includeParameters="id|inOutFlag|toolingDevFlag|spareNo|putPostion|spareName|oldSpareIds|repairSpare|categoryName|modelSpecs|spareDetailType.id|category.code|department.id|checkbox|inoutFlag|inOutFlag|onlyValid|onlyInvalid|spareBillId" 
	    ...
	    <script language="javascript">
	     //  alert('jjjjjjj'+ document.getElementById("toolingDevFlag").value);
            ...
            for ( var i = 0; i < spareIdSelector.length; i ++) {
	      	...
	      	function confirmSelects(boxname) {
	      		if (!hasChecked(boxname)) {
	      			...
	      			return false;
	      		}else if(document.getElementById("inOutFlag").value=='out'){
