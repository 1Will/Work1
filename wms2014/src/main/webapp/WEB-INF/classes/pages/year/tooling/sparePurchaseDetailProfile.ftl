...
<@htmlPage title="${action.getText('sparePurchaseDetailProfile.title')}">
 <base target="_self">
   <@ww.form namespace="'/popup'" name="'sparePurchaseDetail'" action="'saveSparePurchaseDetail'" method="'post'" validate="true">
     ...
     <@inputTable>
	   <tr>
		 <@ww.textfield label="'${action.getText('sparePurchaseDetail.name')}'" name="'sparePurchaseDetail.name'" value="'${sparePurchaseDetail.name?if_exists}'" cssClass="'underline'" disabled="false"  required="true" />
	 	 <@ww.select label="'${action.getText('sparePurchaseDetail.category')}'" required="false" name="'category.id'" 
	   ...
	   <tr>
	     <@ww.select label="'${action.getText('sparePurchaseDetail.detailCategory')}'" required="false" name="'detailCategory.id'" 
		     listKey="id" listValue="name" list="spareDetailType" 
		     emptyOption="true" disabled="false" required="false">
		 </@ww.select>
		 <@ww.textfield label="'${action.getText('sparePurchaseDetail.specification')}'" name="'sparePurchaseDetail.specification'" value="'${sparePurchaseDetail.specification?if_exists}'" cssClass="'underline'" disabled="false"/>
	   </tr>
       <tr>
    	 <@ww.textfield label="'${action.getText('sparePurchaseDetail.model')}'" name="'sparePurchaseDetail.model'" value="'${sparePurchaseDetail.model?if_exists}'" cssClass="'underline'" disabled="false"/>
	     <@ww.textfield label="'${action.getText('sparePurchaseDetail.unitPrice')}'" name="'sparePurchaseDetail.unitPrice'" value="'${sparePurchaseDetail.unitPrice?if_exists}'" cssClass="'underline'" disabled="false" required="true" onchange="'calAllPrice();'"/>
	   </tr>
	   <tr>
    	 <@ww.textfield label="'${action.getText('sparePurchaseDetail.number')}'" name="'sparePurchaseDetail.number'" value="'${sparePurchaseDetail.number?if_exists}'" cssClass="'underline'" disabled="false" required="true" onchange="'calAllPrice();'"/>
         <@ww.textfield label="'${action.getText('sparePurchaseDetail.allPrice')}'" name="'allPrice'" value="'${sparePurchaseDetail.allPrice?if_exists}'" cssClass="'underline'" disabled="true"/>
    	 <@ww.hidden name="'sparePurchaseDetail.allPrice'" value="#{sparePurchaseDetail.allPrice?if_exists}"/>
       </tr>
       <tr>
    	 <@pp.datePicker label="'${action.getText('sparePurchaseDetail.requestDate')}'" name="'sparePurchaseDetail.requestDate'"
 						 value="'${(sparePurchaseDetail.requestDate?string('yyyy-MM-dd'))?if_exists}'" cssClass="'underline'" size="15" 
 						 required="true" maxlength="10"/>
         <@ww.textfield label="'${action.getText('sparePurchaseDetail.usedTooling')}'" name="'sparePurchaseDetail.usedTooling'" value="'${sparePurchaseDetail.usedTooling?if_exists}'" cssClass="'underline'" disabled="false" onchange="'calAllPrice();'"/>
       </tr>
	   <tr>
	     <@ww.textarea label="'${action.getText('sparePurchaseDetail.requestReason')}'" 
					   name="'sparePurchaseDetail.requestReason'" 
					   value="'${sparePurchaseDetail.requestReason?if_exists}'" rows="'3'" cols="'50'"
					   disabled="false" />
 	  	 <@ww.textarea label="'${action.getText('sparePurchaseDetail.comment')}'" 
   ...
   <script language="JavaScript" type="text/JavaScript">
     ...
     */
     function validateReasonAndComment() {
       if ('' != document.forms["sparePurchaseDetail"].elements["sparePurchaseDetail.usedTooling"].value) {
         if (!isValidLength(document.forms["sparePurchaseDetail"], "sparePurchaseDetail.usedTooling",0,50)) {
           alert("${action.getText("sparePurchaseDetail.usedTooling.maxLength")}");
		   return false;
         }
       }
        //验证需求原因长度
	 ...
	 function calAllPrice() {   
	   if ('' != document.forms["sparePurchaseDetail"].elements["sparePurchaseDetail.unitPrice"].value && isDoubleNumber("sparePurchaseDetail.unitPrice")) {
	     ...
	     if ('' != document.forms["sparePurchaseDetail"].elements["sparePurchaseDetail.number"].value && isNumber("sparePurchaseDetail.number")) {
	       var number = parseInt(document.forms["sparePurchaseDetail"].elements["sparePurchaseDetail.number"].value);
