...
function refresh_workType_information(result) {
  	if (result) {
  		$('workType.code').value = result[1];
  		$('workType.name').value = result[2];
  		$('workType.unitPrice').value = result[3];
  		$('workType.id').value = result[0];
  	}
}
/*
...
function refresh_device_information (result) {
  if (null != result) {
    $('device.name').value = result[1]; 
    $('device.deviceNo').value = result[2]; 
    if (null != $('device.department.name')) {
    	$('device.department.name').value = result[3]; 
    }
    if ( null != document.getElementById("department.id")) {
      $('department.id').value = result[9];
    }
    $('device.id').value = result[0];
    if (null != $('device.assetNo')) {
      $('device.assetNo').value = result[4];
    } 
    if (null != $('device.model')) {
      $('device.model').value = result[5];
    }
    if (null != $('device.specification')) {
      $('device.specification').value = result[6];
    }  
    if (null != $('device.category')) {
      $('device.category').value = result[7];
    }
    if (null != $('deviceCategory')) {
      $('deviceCategory').value = result[7];
    }
    if (null != $('device.supplierName')) {
      $('device.supplierName').value = result[8];
    } 
  }
}
...
function refresh_tooling_information (result) {
  if (null != result) {
    $('tooling.name').value = result[1]; 
    $('tooling.deviceNo').value = result[2]; 
    $('tooling.graphNo').value = result[3]; 
    $('tooling.id').value = result[0]; 
    if ( null != $('tooling.department')) {
      $('tooling.department').value = result[4];
    }
   if ( null != $('department.id')) {
      $('department.id').value = result[11];
    }
    if (null != $('tooling.totalOutput')) {
      $('tooling.totalOutput').value = result[5];
    }
    if (null != $('tooling.usedQuota')) {
      $('tooling.usedQuota').value = result[6];
    }
    if (null != $('tooling.toolingType')) {
      $('tooling.toolingType').value = result[7];
    }
    if (null != $('tooling.usedDate')) {
      $('tooling.usedDate').value = result[8];
    }
    if (null != $('tooling.model')) {
      $('tooling.model').value = result[9];
    }
    if (null != $('tooling.specification')) {
      $('tooling.specification').value = result[10];
    }
  }
}
...
function refresh_lubricationOil_information(result) {
	if (null != result) {
    	$('lubricationOil.code').value = result[1]; 
    	$('lubricationOil.name').value = result[2]; 
    	$('lubricationOil.measureUnit').value = result[3]; 
    	$('lubricationOil.id').value= result[0];
	}
}
...
function refresh_supplier_information (result) {
   $('supplier.id').value = result[0]; 
   $('supplier.name').value = result[1]; 
   $('supplier.supplierNo').value = result[2];
   if(null!=$('supplier.supplierName')){
       $('supplier.supplierName').value = result[3];
   }
   if(null!=$('supplier.telphone')){
       $('supplier.telphone').value = result[4];
   }
}
...
function refresh_budgetDetail_information(result) {
  if (result) {
    $('budgetDetail.id').value = result[0];
    $('budgetDetail.budgetNo').value = result[1];
    $('budgetDetail.name').value = result[2];
  }
}
...
function filialeSelectDeptDWR(){
	var filialeId = $('filiale.id').value;
		//alert("filialeId" + filialeId);
	FilialeSelectDeptJs.filialeSelectDept(filialeId , callBack);
}
function callBack(data){
	if(null != data ){
	 	DWRUtil.removeAllOptions("department.id");
	 	//alert(data.length);
		for(var i =0 ;i<data.length;i++){
			if(i == 0){
				$("department.id").options.add(new Option(all,0));
				document.all("department.id").options[i].value = -1;
			}else{
				$("department.id").options.add(new Option(data[i][1],data[i][0]));
				//alert( " " + i + "ge " + data[0][0]);
			}
		}	  
	}
...
function callBackEdit(data){
	if(null != data ){
		...
		for(var i =0 ;i<data.length;i++){
			if(i == 0){
				$("department.id").options.add(new Option("",0));
				document.all("department.id").options[i].value = -1;
			}else{
				$("department.id").options.add(new Option(data[i][1],data[i][0]));
			}
...
function WareCascadeRegionalDWR(warehouseId,regionalId,msg,flag){
	...
	message = msg;
	var whId = $(warehouse_Id).value;
	if(whId != -1 && whId != ""){
		WareCascadeRegionalJs.loadRegionalByWareId(whId,flag,WRCallBack);
	}else{
		DWRUtil.removeAllOptions(regional_Id);
		$(regional_Id).options.add(new Option(message,-1));
		$(regional_Id).options[0].value = -1;
	}
}
function WRCallBack(data){
	if(null!= data ){
	  //alert(data);
 		//DWRUtil.removeAllOptions(regional_Id);
		//$(regional_Id).options.add(new Option(message,0));
		...
		for(var i=0;i<data.length;i++){
		  var bean = data[i];
	 	  if(i == 0){
	 	    DWRUtil.removeAllOptions(regional_Id);
			$(regional_Id).options.add(new Option(message,-1));
			$(regional_Id).options.add(new Option(bean.name,bean.id));
		  }else{
			$(regional_Id).options.add(new Option(bean.name,bean.id));
		 }
...
function WareCascadeLocationDWR(regionalId,locationId,msg){
	...
	message = msg;
	var regId = $(regional_Id).value;
	if(regId != -1 && regId != ""){
		findLocationByRegionalJs.findLocationByRegional(regId,LocationCallBack);
	}else{
		DWRUtil.removeAllOptions(regional_Id);
		$(location_Id).options.add(new Option(message,0));
		$(location_Id).options[0].value = -1;
	}
}
function LocationCallBack(data){
	if(null != data){
		if(data.length > 0){
			DWRUtil.removeAllOptions(location_Id);
			for(var i =0 ;i<data.length;i++){
				var bean = data[i];
				if(i == 0){
				    $(location_Id).options.add(new Option(message,""));
					$(location_Id).options.add(new Option(bean[1],bean[0]));
					$(location_Id).options.add(new Option(bean.code,bean.id));
...
function spareTypeCascadeDWR(spareTypeId,spareDetailType,msg){
	spare_Type_Id = $(spareTypeId).value;
	spare_Detail_Type = spareDetailType;
	messgae=msg;
	if(spare_Type_Id == -1 || spare_Type_Id == ""){
		DWRUtil.removeAllOptions(spare_Detail_Type);
		$(spare_Detail_Type).options.add(new Option(messgae,""));
	}else{
	...
	if(null != data){
		if(data.length > 0){
			...
			for(var i =0 ;i<data.length;i++){
				if(i == 0){
					$(spare_Detail_Type).options.add(new Option(messgae,""));
					$(spare_Detail_Type).options.add(new Option(data[i][1],data[i][0]));
				}else{
					$(spare_Detail_Type).options.add(new Option(data[i][1],data[i][0]));
				}
			}
...
function wareHouseCascadeDWR(storageGradeId,wareHouseId,userId,msg,isSearch){
	storage_grade_Id = $(storageGradeId).value;
	wareHouse_Id = wareHouseId;
	message=msg;
	if(storage_grade_Id == -1 || storage_grade_Id == ""){
		DWRUtil.removeAllOptions(wareHouse_Id);
		if(isSearch=='true') {
			$(wareHouse_Id).options.add(new Option(message,"-1"));
		} else {
			$(wareHouse_Id).options.add(new Option(message,""));
		}
	}else{
		if (isSearch=='true'){
	...
	if(null != data){
		if(data.length > 0){
			...
			for(var i =0 ;i<data.length;i++){
				...
				if(i == 0){
					$(wareHouse_Id).options.add(new Option(message,''));
					$(wareHouse_Id).options.add(new Option(bean.name,bean.id));
				}else{
					$(wareHouse_Id).options.add(new Option(bean.name,bean.id));
				}
			}
