...
* s   :有没有滚动条    
*/
function openWin(url,n,w,h,s){
	sb = s == "1" ? "1" : "0";
	l = (screen.width - w)/2;
	t = (screen.height - h)/2;
	sFeatures = "left="+ l +",top="+ t +",height="+ h +",width="+ w
			+ ",center=1,scrollbars=" + sb + ",status=0,directories=0,channelmode=0";
	openwin = window.open(url , n , sFeatures );
	if (!openwin.opener)
		openwin.opener = self;
	openwin.focus();
	return openwin;
...
function returnDialogNoClose(result, reload)
{
    if (isIE()) {
        window.returnValue = result;
    } else {
...
function isDate_Hour_Min(eId) {
	...
	if (!isNumber_String(date.substr(11,2)) || date.substr(11,2)>=12 || date.substr(11,2)<0){
		return false;
	}
...
//通过name标签取得控件 替换document.getElementById
function getObjByNameRe(name){
	 var ret = ""; 
 ...
 * monthValue: 月份数
 */
function isMonth(monthValue) {   
	var dates = monthValue.split('-');
	if(dates.length !=2){
		return false;
	} else{
...
function isYearMonth(ym){
    ...
    /* 
* 函数名: isDateLessThenOtherDate
* 作  用: 比较前者日期是否小于后者日期
* 参  数: mindateVar -------- 前者日期格式
*      : maxdateVar -------- 后者日期格式
* 返回值: true 是 false 否
*/
function isDateLessThenOtherDate(mindateVar,maxdateVar){
