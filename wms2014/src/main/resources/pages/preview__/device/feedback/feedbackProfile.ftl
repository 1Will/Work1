...
<script language="JavaScript" type="text/JavaScript"> 
	window.onload = function () {
		//		document.forms["area"].elements["ext"].click();
 		document.all.frame.src='listFeedbackHistories.html';
	...
	function device_OpenDialog() {
	 ...
	 <@ww.form name="'listForm'" action="'searchProducts'" method="'post'" validate="true">
	 	  ...
	 	  <@inputTable>
	 	  	   <tr>
	 	  	  	<@ww.textfield label="'反馈记录编号'" name="'area.code'" value="'0001'" cssClass="'underline'" disabled="true" readonly="true" />
	 	  	  	<@ww.textfield label="'反馈记录名称'" name="'area.code'" value="" cssClass="'underline'" required="true" />
	 	  	  </tr>
	 	  	   <@deviceSelector/>			 	  
		      <tr>
		 	  	<@pp.datePicker label="'反馈时间'" name="'repair.estimate.time0'"
	     				value="" cssClass="'underline'" size="15" required="true"/>	    
	     		<@ww.textfield label="'反馈人'"  name="'area.code'" value="" cssClass="'underline'" required="true" /> 		
	     	  </tr>
		 	  <tr>
		 	  	<@feedback1/>
		 	  	<@ww.textfield label="'反馈内容'"  size="50"  name="'area.code'" value="" cssClass="'underline'" required="true" />
		 	  </tr>		
		 	  <@audit2/> 	 
        	<tr>
	        	<td align="right" valign="top"><label class="label">维修人:</label></td>
	        	<td>
	        	...
	        	<td align="right" valign="top"><label class="label">相关人:</label></td>
	        	<td>
	        		<input type="text" name="shipmentPlan.vehicle.code" 
	        			class="underline"  value="qs,sa,admin"  maxlength="150" size="50"/>
	        		<input type="hidden" name="shipmentPlan.vehicle.id" value="c" />
	 	  ...
	 	  <@buttonBar>
	        	...
	        	<@redirectButton  value="返回" url="listFeedbacks.html"/>
	     </@buttonBar>
	</@ww.form>
	
	<ul id="beautytab">
		<li><a id="planItems" onclick="activeTab(this);" href="listFeedbackHistories.html" target="frame" class="selectedtab">反馈历史记录</a></li>	
	</ul>
