...
<script language="JavaScript" type="text/JavaScript"> 
	...
	function checkPointRule_openDialog() {
		popupModalDialog('${req.contextPath}/popup/smoothRuleSelector.html',600,400);
	}
    ...
    <@ww.form namespace="'/base/area'" name="'area'" action="'saveArea'" method="'post'" validate="true">
        ...
        <@inputTable>
        	<tr>
             ...
             <@deviceSelector/>  
             <tr>
            	<@pp.datePicker label="'计划预计执行时间'" name="'repair.estimate.time0'"
	     			value="" cssClass="'underline'" size="15" required="true" />
            </tr>     
            <tr>
                <@ww.textfield label="'计划实施要求'" name="'area.code'" value="" size="70" cssClass="'underline'" />
                <@ww.textfield label="'备注'" name="'area.code'" value="" size="70" cssClass="'underline'" />
            </tr> 
        </@inputTable>
       	<@buttonBar>
       		<@redirectButton value="${action.getText('save')}" url="${req.contextPath}/preview/device/smoothInfo/listSmoothPlans.html"/>	
       		<@redirectButton value="${action.getText('back')}" url="${req.contextPath}/preview/device/smoothInfo/listSmoothPlans.html"/>	
        </@buttonBar>
    </@ww.form>
	<ul id="beautytab">
