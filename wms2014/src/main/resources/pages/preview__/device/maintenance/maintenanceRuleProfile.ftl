...
<script language="JavaScript" type="text/JavaScript"> 
	window.onload = function () {
		//		document.forms["area"].elements["ext"].click();
 		document.all.frame.src='../../device/maintenance/listMaintenanceRuleItems.html';
    ...
    <@ww.form namespace="'/base/area'" name="'area'" action="'saveArea'" method="'post'" validate="true">
        ...
        <tr>
        	<@ww.textfield label="'备注'" name="'area.code'" required="true" value="" size="50" cssClass="'underline'"/>
        </tr>
        </@inputTable>
       	<@buttonBar>
       		<@vbutton value="${action.getText('save')}" class="button" onclick="javascript:window.close();"/>	
       		<@redirectButton value="返回" url="${req.contextPath}/preview/device/maintenance/listMaintenanceRules.html"/>
        </@buttonBar>
    </@ww.form>
	...
	<ul id="beautytab">  
	    	<li><a id="ruleItems" href="../../device/maintenance/listMaintenanceRuleItems.html" target="frame" class="selectedtab">保养标准明细</a></li>
	</ul>
