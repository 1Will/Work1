...
<@htmlPage title="${action.getText('toolingMakeDetailSelector.title')}">
<base target="_self">
  <@ww.form name="'listQuarterPlan'" action="'searchPurchaseToolingMakeDetailSelector'" method="'post'">
    <@ww.token name="toolingMakeDetailSelectorToken"/> 
    ...
    <#assign itemNo = 1/>
 	<@list title="${action.getText('toolingMakeDetail.list')}" 
  ...
  function checkInvalidParms() {
	...
	if (getObjByNameRe("qarterType").value == -1) {
  		getObjByNameRe("qarterType").value = '';
	}
	if(document.forms[0].elements["quarter.year"].value!=""){
          if(!isYear(document.forms[0].elements["quarter.year"].value)){
               alert("${action.getText('quarter.year.start_EndTimeMistake')}");
               return false;
          }
    }
	if(document.forms[0].elements["planFinishedDate_start"].value!=""){
          if(!validateTime(document.forms[0].elements["planFinishedDate_start"].value)){
