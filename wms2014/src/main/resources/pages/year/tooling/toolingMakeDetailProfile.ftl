...
<@htmlPage title="${action.getText('toolingMakeDetailProfile.title')}">
 <base target="_self">
   <@ww.form namespace="'/popup'" name="'toolingMakeDetail'" action="'saveToolingMakeDetail'" method="'post'" validate="true">
     ...
     <@inputTable>
       ...
       <tr>               
         <@ww.textfield label="'${action.getText('allPrice')}'" name="'allPrice'" value="'${(toolingMakeDetail.allPrice?string('#,###,##0.##'))?if_exists}'" cssClass="'underline'" disabled="true"/>
     ...
     */
     function validateUnitPriceAndNumber() {
       ...
       if ('' == number) {            //验证是否为空
         ...
         return false;
       } else if (!isDoubleNumber("toolingMakeDetail.number")){   //验证格式
