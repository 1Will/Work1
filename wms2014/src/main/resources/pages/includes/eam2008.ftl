 ...
 <script language="javascript">
   ...
   */
   function changeBudgeNo() {
     ...
     if(getObjByName('sourceType').value=='OUT_BUDGET'){
 	   getObjByName('budgetNoTitle').style.display='none';
 	   getObjByName('budgetNoContent').style.display='none';
 	   //document.forms[0].elements["budgetDetail.budgetNo"].value='';
 	   getObjByName('budgetDetail.id').value='';
     }
     return true;
  }
 </script>
...
<#macro eam2008_remoteGetLocation id="" code=""  loopNum="" inputName=""  disabled="false" >
	...
	<script language="javascript">
	  ...
	  function getLocationCode(loopNum) {
	    LOOP_NUM = loopNum;
	    if(getObjByNameRe("${inputName}" + loopNum).value!=""){
	      try{
	        remoteGetLocationCode(getObjByNameRe("${inputName}" + loopNum).value);
	      }catch(x){}
	    }
	  }
	  ...
	  function locationCode_SetPickedResult(msg){
        ...
        if (msg.length == 0) {
                var value = getObjByNameRe("${inputName}" + LOOP_NUM).value;
                getObjByNameRe("hidden." + "${inputName}" + LOOP_NUM).value = "false";
                alert(value + "${action.getText('location_notFound')}");
            } else {
                getObjByNameRe("hidden." + "${inputName}" + LOOP_NUM).value = "true";
      ...
      function locationSelectorHandler(result) {
        ...
        if (null != result) {
          var objLocationCode = getObjByNameRe("${inputName}" + LOOP_NUM);
		  <#--var objWarehouseName = getObjByNameRe("warehouse"+ LOOP_NUM);-->
		  <#--var objRegionalName = getObjByNameRe("regional"+ LOOP_NUM);-->
		  
          objLocationCode.value = result[1];
          <#--zzb getObjByNameRe("afterMoveWarehouse"+ LOOP_NUM).value=result[3];-->
          <#--objWarehouseName.value=result[2];-->
          <#-- zzb getObjByNameRe("afterMoveRegional"+ LOOP_NUM).value=result[5];-->
          
          <#--objRegionalName.value=result[4];-->
		  getObjByNameRe("hidden." + "${inputName}" + LOOP_NUM).value = "true"; 
          getObjByNameRe("moveId." + "${inputName}" + LOOP_NUM).value = result[0];
        }
      }
	</script> 
</#macro>
...
<#macro factorySelector required="true" validateJs="null">
  ...
  <script language="javascript">
	...
	function showFactory(){
		var left = 0;					//设置弹出层的位置
		var top = 0;
		var e = getObjByName('factory.name');
		...
		top += 20;
		getObjByName('show').style.left=left+"px";
		getObjByName('show').style.top=top+"px";
		
		var obj = getObjByName('factory.name').value;
		if(obj != "" && obj != null){
	...
	function checkFactory(){
		var factoryName = getObjByName('factory.name').value;
	...
	function callbackCheck(data){
		if(null == data){
			if(getObjByName('factory.supplierNo').value != "" || getObjByName('factory.name').value != "" && getObjByName('show').style.display == "none"){
				clean();
			}
		}
	}
	...
	function callback(data){
				if(data.length > 0){
					...
					for(var i = 0;i<data.length;i++){
						...
						tag.onclick=function(){
									getObjByName('factory.name').value = this.value;
									getObjByName('show').style.display = "none";
					...
					getObjByName('show').style.display = "none";
					clean();
					getObjByName('factory.name').focus();
				}	
		...
		function autoCopyValue(flag){
			getObjByName('factory.id').value=resultData[flag]["id"];
			getObjByName('factory.name').value=resultData[flag]["name"];
			getObjByName('factory.supplierNo').value=resultData[flag]["supplierNo"];
			getObjByName('factory.name').focus();	
		}
		/**
		 * 将控件值清空
		 */
		 function clean(){
		 	getObjByName('factory.id').value="";
			getObjByName('factory.name').value="";
			getObjByName('factory.supplierNo').value="";
		 }
		 /**
	  	 * 页面点击事件源
	  	 */
		...
		function myMethod(){
			...
			if(event.keyCode==38){
				...
				if(document.selection.createRange().text != ""){
					getObjByName('factory.name').value = document.selection.createRange().text;
				}
				getObjByName('show').style.display = "none";
			}else if(event.keyCode==8 || event.keyCode==46 && getObjByName('show').style.display == "none"){
				showFactory();
			}
		}
...
<#macro eam2008_getBuyer id="" code=""  name="" loopNum="" inputName="" inputId="" inputN="" disabled="false" >
	...
	<script language="javascript">
	  ...
	  function getBuyerCode(loopNum) {	  
	    ...
	    if(getObjByNameRe("${inputName}" + loopNum).value!=""){
	      try{	      
	        remoteGetBuyerCode(getObjByNameRe("${inputName}" + loopNum).value);	       
	      }catch(x){}
	    }
	  }
	  ...
	  function getByName(name,loopNum){
	  	...
	  	LOOP_NUM = loopNum;
	  	if(name != ""){
	  		if(name != getObjByNameRe("NAME"+LOOP_NUM).value){
	  			getUserByNameJs.getUserByLoginName(name,getByNameCallBack)
	  		}
	  	}else{
	  		getObjByNameRe("${inputName}"+ LOOP_NUM).value='';
            getObjByNameRe("hidden." + "${inputName}"+ LOOP_NUM).value = "false";
            getObjByNameRe("${inputN}"+ LOOP_NUM).innerHTML=''; 
	  	}
	  ...
	  function getByNameCallBack(data){
	  	if(null != data){
	  		getObjByNameRe("${inputId}"+ LOOP_NUM).value=data.id;          
          	getObjByNameRe("${inputName}"+ LOOP_NUM).value=data.loginName;
          	getObjByNameRe("${inputN}"+ LOOP_NUM).innerHTML=data.name; 
	  	}else{
	  		var value = getObjByNameRe("${inputName}"+ LOOP_NUM).value;
	  		if(getObjByNameRe("${inputN}"+ LOOP_NUM).innerHTML==""){
	  			getObjByNameRe("${inputName}"+ LOOP_NUM).value='';
            	getObjByNameRe("hidden." + "${inputName}"+ LOOP_NUM).value = "false";
            	getObjByNameRe("${inputN}"+ LOOP_NUM).innerHTML=''; 
	  		}else{
	  			getObjByNameRe("${inputName}"+ LOOP_NUM).value=getObjByNameRe("NAME"+LOOP_NUM).value;
	  		}
      ...
      function buyerSelectorHandler(result) {
        if (null != result) {
          ...
          getObjByNameRe("${inputId}" + LOOP_NUM).value=result[0];          
          getObjByNameRe("${inputN}" + LOOP_NUM).innerHTML=result[1];  
          //getObjByNameRe("afterMoveWarehouse"+ LOOP_NUM).value=result[3];
          //getObjByNameRe("afterMoveRegional"+ LOOP_NUM).value=result[5];
          getObjByNameRe("hidden." + "${inputName}" + LOOP_NUM).value = "true";
        }
      }
	</script> 
