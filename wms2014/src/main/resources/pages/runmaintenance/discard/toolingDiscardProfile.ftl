...
<@htmlPage title="${action.getText('toolingDiscardEdit.title')}">
	       ...
	       <#if loginUser.department?exists>
	        getObjByNameRe("department.id").value = #{loginUser.department.id};
       		...
       		if(discardFlag){
	       		...
	       		getObjByNameRe("descardDatetime").style.display='inline';	
	        }
		    ...
		    if (document.forms["discard"].elements["discardAgree"].checked == true) {
		      ...
		      <#else>
		      	document.forms["discard"].elements["deviceState"].value = 'DEVICE_DISCARD';
			    	...
			    	getObjByNameRe("descardDatetime").style.display='none';
	...
	function validate(){
		   if(getObjByNameRe("discard.name").value=='' ){
           ...
           return false;
			...
			if(getObjByNameRe("discard.status").value!=''){
	...
	function discard_value(){
		 if(getObjByNameRe("discard.value").value!=''){
	    ...
	    return true;
		  ...
		  function discard_techDeparOpinion(){
			...
			function validateTime_applyDatetime(){
				...
				var date=getObjByNameRe("discard.applyDatetime").value;
			...
			function validateTime_descardDatetime(){
				...
				var date=getObjByNameRe("discard.descardDatetime").value;
				var dateApply=getObjByNameRe("discard.applyDatetime").value; 
