...
<@htmlPage title="${lubricationTitle}">
  ...
  <@ww.form name="'lubricationPlanDetail'" action="'searchLubricationDetailsSearch'" method="'post'">
    <@ww.token name="saveWashDetailsToken"/>
    <#include "lubricationDetailSearch.ftl" />
    ...
    <@list title="" excel=false setupTable=false
           includeParameters="lubrication.id|planProcFlag|readOnly|deviceNo|deviceName|position|planExePeople|planExectPeople|estimateExecDate_start|estimateExecDate_end|actualExePeople|actualExecDate_start|actualExecDate_end" 
           fieldMap="like:deviceNo|deviceName|position|planExePeople|planExectPeople|actualExePeople|,date:estimateExecDate_start|estimateExecDate_end|actualExecDate_start|actualExecDate_end" >
      ...
      <@vcolumn title="${action.getText('device.no')}" property="device.deviceNo" sortable="asc">
	    <@alignLeft/>
      </@vcolumn>
      ...
      <#if planProcFlag?exists>
        <#if planProcFlag == 'PLAN'>
          <@vcolumn title="${action.getText('lubricationDetail.lubricationOilQty')}">
	        <input type="text" name="lubricationOilQty" 
	    	       class="underline"  value="${object.planLubricatingOilQty?if_exists}"  size="15"/>
	    ...
	    <#else>
          ...
          <@vcolumn title="${action.getText('lubricationDetail.actualLubricationOilQty')}">
	        <input type="text" name="lubricationOilQty" 
	    	       class="underline"  value="${object.actualLubricatingOilQty?if_exists}"  maxlength="10" size="15" style="text-align:right"/>
	        <@vlh.attribute name="style" value="VERTICAL-ALIGN:middle"/>
	      </@vcolumn>
	    </#if>
	  </#if>
    ...
    function validate() {
      if (0 != document.getElementsByName("detailIds").length) { 
        if(validateInformation()==true){
	        ...
	        <#if planProcFlag?exists>
			<#if (planProcFlag=='PLAN')>
	        retrievePlanExePeopleText();                              //获取润滑计划明细计划执行人或实际执行人所有非空值
	        </#if>
	        </#if>
	        retrieveExectPeopleText();
	       // retrieveLubricationOilQty();   
