    ...
    <@ww.form  name="'detail'" action="'saveCheckPointProcDetail'" method="'post'" validate="true">  
	 ...
	 <@ww.hidden name="'detail.id'" value="'${req.getParameter('detail.id')?if_exists}'"/>
		...
		window.onload = function () {
	     for(var i=0;i<2;i++)
	     {
	         if(getObjByNameRe("checkType").options[i].value==getObjByNameRe("checkType").value)
	         {
	         	document.forms[0].elements["detail.checkResult"].value=getObjByNameRe("checkType").options[i].value;
	            getObjByNameRe("checkType").options[i].selected="true";
	            if (getObjByNameRe("checkType").options[i].value == 'NORMAL')
	            {
	               getObjByNameRe("descript").innerText='${action.getText('procDetail.comment')}:';        
	            }
	            else{
	              getObjByNameRe("descript").innerText='${action.getText('procDetail.exceptionComment')}:';              
	            }
	         }
	     }
	 	}
		...
		function showExceptionBox() {
			var i = getObjByNameRe("checkType").value;
			/*if (2 == i ) {*/
			if ('EXCEPTION' == i) {
