...
<@ww.form namespace="'/runmaintenance/spareBorrow'"  name="'spareBorrow'" action="'saveSpareBorrow'" method="'post'">
 ...
 <script language="javascript">	
    ...
    *   验证领用单编号
    */
   <#-- if(getObjByNameRe("spareBorrow.billNo").value==''){
	 ...
	 * 验证领用单名称
	 */    
	 if(getObjByNameRe("spareBorrow.name").value==''){
	  ...
	  */
	  ...
	  /**
	    验证领用日期
	  */  
	  if(getObjByNameRe("spareBorrow.borrowDate").value==''){
	        alert('${action.getText('spareBorrow.borrowDate.not.null')}');
	        return false;
	      }
	    var date=getObjByNameRe("spareBorrow.borrowDate").value;
		if(!isDate("spareBorrow.borrowDate")){
		  ...
		  * 验证领用人
		  */
	   if(getObjByNameRe("spareBorrow1.borrowUser.id").value==''){
	    ...
	    *验证用途说明
	    */    
	  if(getObjByNameRe("spareBorrow.description").value!=''){
