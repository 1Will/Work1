...
<@htmlPage title="${unusedBillEditTitle}">
  ...
  <script language="JavaScript" type="text/JavaScript">
   ...
   function unused_Vlidate(){
	    if(getObjByNameRe("unused.name").value==''){
	...
	<#if toolingDevFlag?exists>
	     ...
	     <#else>
	       if(getObjByNameRe("tooling.deviceNo").value==''){
	        alert('${action.getText('asset.tooling.not.null')}');
	        return false;
	     }
	    </#if>
   </#if>
	  if(getObjByNameRe("unused.unUseDate").value==''){       //验证闲置日期及格式
	        alert('${action.getText('unused.unUseDate_not_null')}');
	        return false;
	      }
	    var date=getObjByNameRe("unused.unUseDate").value;
	...
	*/   
	<#if unused.id?exists>
	  if(getObjByNameRe("onlyDisapprove").checked==true){ 
	        if(getObjByNameRe("unUsedApr.name").value==''){
	           alert('${action.getText('unUsedApr.name_not_null')}');
	           return false;
	       }
	     }
	  if(getObjByNameRe("unused.unUsedAprDate").value==''){
	        alert('${action.getText('unused.unUsedAprDate_not_null')}');
	        return false;
	      }
	    var date=getObjByNameRe("unused.unUsedAprDate").value;
	  ...
	  if(getObjByNameRe("unused.devise").value!=''){
	...
	<#if unused.id?exists &&unused.status=="NORMAL">
		 if(getObjByNameRe("onlyisUsed").checked==true){
		  if(getObjByNameRe("usedApr.name").value==''){
	        alert('${action.getText('usedApr.name_not_null')}');
	        return false;
	      }
	     }
	     if(getObjByNameRe("unused.usedAprDate").value==''){
		 ...
		 var date=getObjByNameRe("unused.usedAprDate").value;
		 if(!isDateLessEqualThenCurrent(date)){
			    alert("${action.getText('afresh.unused.usedAprDate')}");
				return false;
		   }
		 if(getObjByNameRe("unused.usedreason").value!=''){
