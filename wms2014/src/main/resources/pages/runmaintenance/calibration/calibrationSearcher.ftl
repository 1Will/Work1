	...
	<script language="javascript">
	  ...
	  function checkInvalidParms() {
	  	if (getObjByNameRe("department.id").value == -1) {
			...
			<#--
		var scheduleDate_start = getObjByNameRe("scheduleDate_start").value;
	    var scheduleDate_end = getObjByNameRe("scheduleDate_end").value;
		...
		-->
		var makeDate_start = getObjByNameRe("makeDate_start").value;
	    var makeDate_end = getObjByNameRe("makeDate_end").value;
	    if (makeDate_start!='') {
		...
		<#if planProcFlag?exists>
			 <#if (planProcFlag=='PROC')>
				var reportDate_start = getObjByNameRe("reportDate_start").value;
			    var reportDate_end = getObjByNameRe("reportDate_end").value;
			    if (reportDate_start!='') {
