    ...
    <@ww.form namespace="'/prophase/supplier'" name="'evaluate'" action="'saveSupplierEvaluate'" method="'post'">
     ...
     window.onload = function() {
	  ...
	  <#if loginUser.department?exists>
	   ...
	   <#if supplierEvaluate.department?exists>
	     getObjByNameRe("department.id").value = #{supplierEvaluate.department.id};
	   </#if>
	 }
     ...
     function validateDate(){
        if(getObjByNameRe("supplier.supplierNo").value==''){
	      ...
	      //验证日期
	    var date=getObjByNameRe("supplierEvaluate.evaluatedate").value;
		if(getObjByNameRe("supplierEvaluate.evaluatedate").value!=''&&!isDate("supplierEvaluate.evaluatedate")){
		    alert("${action.getText('select.right.supplierEvaluate.evaluatedate')}");
			return false;
		  }
		if(getObjByNameRe("supplierEvaluate.evaluatedate").value!=''&&!isDateLessEqualThenCurrent(date)){
			alert("${action.getText('afresh.evaluatedate')}");
		    return false;
		  }
		 //验证年度
		 if(getObjByNameRe("supplierEvaluateYear").value==''){
	      alert("${action.getText('supplierEvaluateyear.not.null')}");
	       return false;
	      }  
		 var year =getObjByNameRe("supplierEvaluateYear").value;
