 ...
 <script language="javascript">
              ...
              for(var i=0;i<selector.length;i++){
                  ...
                  if('NEW'==statu){
                     num++;
                      ...
                      // getObjByName(id).parentNode.parentNode.disabled="true";
       ...
       -->
    	...
    	//如果没有盘点明细，“发送提醒”按钮状态改为不可用
          ...
          <#if purchaseBill.id?exists>
             parent.getObjByNameRe("totalPrice").value = '${purchaseBill.totalPrice?if_exists}';
         </#if>
          ...
          function checkValue(id, statu){
           ...
           if(selector.length){
              ...
              if(selector[i].checked==true){
                  var id = selector[i].value;
                  var status = getObjByName(id).value;
                   if('NEW'==status){
       ...
       function  changPrice(pId,num){
          ...
          if(null != getObjByName('taxPrice'+length)){
            taxPrice = getObjByName('taxPrice'+length).value;   //税前单价
            taxPrice = taxPrice.Trim();
            taxPrice = replaceComma(taxPrice); 
          }
          
          if(null != getObjByName('unitPrice'+length)){
             price = getObjByName('unitPrice'+length).value; //税后单价
             price = price.Trim();
             price = replaceComma(price);
          }
          
          if(null != getObjByName('amount'+length)){
             amount = getObjByName('amount'+length).value;
             if(''==amount || 0==amount){
	            alert("第 "+length+" 行采购数量应大于0！");
	            getObjByName('amount'+length).focus();
	            return false;
             }
             if (!isDoubleNumberCheck(amount)){
	             ...
	             if (isDoubleNumberCheck(taxPrice) ){
	                 ...
	                 if(null != getObjByName('unitPrice'+length)){
	                        getObjByName('unitPrice'+length).value=toMoney(price,2);
	                 }
	                 if(null !=  getObjByName('totalPrice'+length)){
	                     getObjByName('totalPrice'+length).innerHTML=toMoney(parseFloat(price*amount).toFixed(2),2); 
	                 }
	                 
	                 if(null != getObjByName('taxTotalPrice'+length)){
	                        getObjByName('taxTotalPrice'+length).innerHTML=toMoney(parseFloat(taxPrice*amount).toFixed(2),2);
	                 }
	               
	             
	             }else{
	                 if(null != getObjByName('taxTotalPrice'+length)){
	                        getObjByName('taxTotalPrice'+length).innerHTML="";
	                 }
	             ...
	             if(isDoubleNumberCheck(price)){
	                   ...
	                   if(null != getObjByName('taxPrice'+length)){
	                        getObjByName('taxPrice'+length).value=toMoney(taxPrice,2);
	                   }
	                   if(null != getObjByName('taxTotalPrice'+length)){
	                      getObjByName('taxTotalPrice'+length).innerHTML=toMoney(parseFloat(taxPrice*amount).toFixed(2),2);
	                   }
	                   ...
	                   if(null !=  getObjByName('totalPrice'+length)){
	                     getObjByName('totalPrice'+length).innerHTML=""; 
	                   }
	             }
	         }
       ...
       function toMoney(s, n){   
        ...
        function  changeTaxPrice(pId,num){
            var length = (parseInt(num)-1);
            var price = getObjByName('unitPrice'+length).value; //税后
            price = replaceComma(price);
            var amount ="";
            if(null != getObjByName('amount'+length)){
            ...
            if(''==price || price<=0){
                ...
                getObjByName('unitPrice'+length).focus();
                return false;
           }
           if (!isDoubleNumberCheck(price)){
        ...
        function channgeUnitPrice(pId,num){
             var length = (parseInt(num)-1);
             var taxPrice = getObjByName('taxPrice'+length).value;//税前
             var amount = "";
             if(null != getObjByName('amount'+length)){
                amount = getObjByName('amount'+length).value;
             }
             ...
             if(''==taxPrice || taxPrice<=0){
                   ...
                   getObjByName('taxPrice'+length).focus();
                   return false;
         ...
         function changeRegDate(pId,num){
	    	 ...
	    	 var date = getObjByName(regDateTagName).value;
	    	  <#--保存到数据库-->
	    	  if(''!=date){
	    	      var ary = new Array();
		          ary.push("D");
        ...
        function savePurchaseDtl(){
	     ...
	     function retrievePurchaseDetailAmountText(){
           ...
           for (var i=0; i<allPurchaseBillDtlId.length; i++) {
              var amount= getObjByName(TagName+(i+1)).value;
              if ('' != amount) {
	     ...
	     function retrievePurchaseDetailTaxPriceText(){
           ...
           for (var i=0; i<allPurchaseBillDtlId.length; i++) {
              var taxPrice= getObjByName(TagName+(i+1)).value;
              if ('' != amount) {
	    ...
	    function  retrievePurchaseDetailUnitPriceText(){
	         ...
	         var ary = new Array();
           for (var i=0; i<allPurchaseBillDtlId.length; i++) {
               var unitPrice=  getObjByName(TagName+(i+1)).value;
               if ('' !=unitPrice) {
	    ...
	    */
	    function informationValidate(){
		  ...
		  for(var i=0;i<allPurchaseBillDtlId.length;i++){
		    var amount = getObjByName(TagAmount+(i+1)).value;//数量
		    amount = amount.Trim();
		   	//验证数量
		  	if(''!= amount){
		  	 ...
		  	 //验证税后单价
		  	var unitPrice = getObjByName(TagPrice+(i+1)).value;//单价
		    ...
		    //验证税前单价
            var taxPrice = getObjByName(TagTaxPrice+(i+1)).value;//单价
            taxPrice = taxPrice.Trim();
		  	if(''!= taxPrice){
		  	...
		  	var reqDate = getObjByName(TagReqDate+(i+1)).value;
