...
<@inputTable>	
   ...
   <tr>
   	...
   	<@ww.textfield label="'${action.getText('型号')}'" name="'model'" value="'${req.getParameter('model')?if_exists}'" cssClass="'underline'" />
...
<script language="javascript" type="text/JavaScript">
    ...
    <#--
     <#if req.getParameter('spareType.id')?exists>
        getObjByName('spareType.id').value = "${req.getParameter('spareType.id')?if_exists}";
...
function checkInvalidParms(){
	  ...
	  <#--
	  if (getObjByName('collDepartment.id').value == -1) {
	     getObjByName('collDepartment.id').value = '';
	  }-->
	  if (getObjByName('spareType.id').value == -1) {
	     getObjByName('spareType.id').value = '';
	  }   
	 // if (getObjByName('spareDetailType.id').value == -1) {
	    // getObjByName('spareDetailType.id').value = '';
	 // }
