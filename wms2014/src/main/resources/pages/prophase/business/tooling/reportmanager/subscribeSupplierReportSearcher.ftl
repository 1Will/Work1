...
<script language="javascript" type="text/JavaScript">
  ...
  function checkInvalidParms(){
      ...
      var date_start = getObjByName("subscribeDate_start").value;
      var date_end = getObjByName("subscribeDate_end").value;
	  if(date_start=="" || !validateTime(date_start)){
              alert("请输入正确的时间格式，正确格式2011-01-01");
              getObjByName("subscribeDate_start").focus();
              return false;
       }
     
	  if(date_end=="" || !validateTime(date_end)){
              alert("请输入正确的时间格式，正确格式2011-01-01");
              getObjByName("subscribeDate_end").focus();
              return false;
       }
