...
<@htmlPage title="${action.getText('repairMaintenanceDetailProfile.title')}">
 <base target="_self">
   <@ww.form namespace="'/tooling/acceptBillSelector'" name="'repairMaintenanceDetail'" action="'saveAcceptRepairMaintenanceDetail'" method="'post'" validate="true">
   ...
   <script language="JavaScript" type="text/JavaScript">
     ...
     */
     function validateComment(){
        if ('' != document.forms["repairMaintenanceDetail"].elements["acceptBillDetail.memo"].value) {
	     if (!isValidLength(document.forms["repairMaintenanceDetail"],"acceptBillDetail.memo",0,250)) {
	       alert("${action.getText("comment.maxLength")}");
		   return false;
	     }
       }
       return true;
