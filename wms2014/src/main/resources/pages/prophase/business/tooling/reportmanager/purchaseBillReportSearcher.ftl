...
<#--
  ...
  for (i=0; i<groups; i++) {
   ...
   function checkInvalidParms(){
	   var date_start = getObjByName("purchaseDate_start").value;
	   var date_end= getObjByName("purchaseDate_end").value;
	   date_start = date_start.Trim();
	   date_end = date_end.Trim();
	   
	   if(date_start=="" || !validateTime(date_start)){
              alert("请输入正确的时间格式，正确格式2011-01-01");
              getObjByName("purchaseDate_start").focus();
              return false;
       }
     
	  if(date_end=="" || !validateTime(date_end)){
              alert("请输入正确的时间格式，正确格式2011-01-01");
              getObjByName("purchaseDate_end").focus();
              return false;
       }
