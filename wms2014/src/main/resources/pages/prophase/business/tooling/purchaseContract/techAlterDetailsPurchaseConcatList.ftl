...
<@framePage title="${action.getText('ToolingMakeDetailPurchaseConcat')}">	
  <style type="text/css">
   ...
   <script language="javascript" src="${req.contextPath}/javascripts/calendar-setup.js"></script>  
	<@ww.form name="'listForm'" namespace="'/toooling/purchaseContract'"  action="'searchTechAlterDetailsPurchaseConcat'" method="'post'">
		<@ww.token name="searchTechAlterDetailsPurchaseConcatToken"/>
		...
		<#if purchaseBill.id?exists>
		 ...
		 <@list title="" excel=false setupTable=false includeParameters="purchaseBill.id" fieldMap="">
            ...
            <@vcolumn title="${action.getText('itemNo')}">
                <a href="#" onclick="return open_techAlterDtlDialog(#{purchaseBill.id}, #{object.id})">${itemNo}</a>
                <@alignCenter />
           </@vcolumn>
       ...
       <@buttonBar>
    	 	<input type="button" class="button" name="new" value="${action.getText('new')}" onclick="open_techAlterDtlDialog(#{purchaseBill.id}, null)"/>
			<#assign confirmMessage = "${action.getText('confirm.delete')}${action.getText('techAlterDetailsPurchaseConcat')}?" />
...
<script language="javascript">
	window.onload=function(){
        <#if req.getParameter('errorFlag')?has_content>
   	       alert("${action.getText('delete.techAtlerDtl.failure')}");
        </#if>
          <#if purchaseBill.id?exists> 
	 ...
	 function open_techAlterDtlDialog(purchaseBillId,toolingMakeDtlId){
		 var url = '${req.contextPath}/toooling/purchaseContract/editTechAlterDetailsPurchaseConcat.html?readOnly=${req.getParameter('readOnly')?if_exists}&purchaseBill.id='+purchaseBillId;	
		 if(toolingMakeDtlId != null){
		 	url = url + "&toolingMakeDetail.id=" + toolingMakeDtlId
		 }
		 popupModalDialog(url,800,600);
		 var reloadUrl = "${req.contextPath}/toooling/purchaseContract/listTechAlterDetailsPurchaseConcat.html?readOnly=${req.getParameter('readOnly')?if_exists}&purchaseBill.id=#{purchaseBill.id}";
	 ...
	 //弹出采购单明细查询页面
        function open_PurchaseBillDialog(){
        		var url = '${req.contextPath}/toooling/purchaseContract/listPurchaseBillchooses.html?detailType=TECH_ALTER&purchaseBill.id='+#{purchaseBill.id};
