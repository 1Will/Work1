...
<@htmlPage title="${action.getText('subscribeList.title')}">
	<@ww.form namespace="'/tooling/report'" name="'searchSubscribeDetailMonthReport'" action="'searchSubscribeDetailMonthReport'" method="'post'">
		...
		<#include "subscribeReportSearcher.ftl"/>
		...
		<@list title="${action.getText('subscribeList')}" 
            ...
            <@vcolumn title="${action.getText('subscribe.billNo')}" property="billNo" sortable="desc">
            	<a href="${req.contextPath}/prophase/business/editSubscribe.html?toolingDevFlag=${toolingDevFlag?if_exists}&subscribe.id=#{object.id}&readOnly=true&report=report">${object.billNo}</a>
            	<@alignLeft />
            </@vcolumn>
            </#if>
...
<script language="JavaScript" type="text/JavaScript">
	   ...
	   function excelButtonManager(type){
		  ...
		  if(op.style.display=="none"){
			 getObjByName("excelPrint").value="${action.getText('pdfDeptReportPrints')}";
       ...
       function open_subscribePdf(){
	       ...
	       var dept = getObjByName("department.id").value;
	       ...
	       var url='${req.contextPath}/reports/subscribe/subscribeDetailMonthView.pdf?date_start='+getObjByName("subscribeDate_start").value+"&date_end="+getObjByName("subscribeDate_end").value+"&department.id="+dept+"&subIds="+subIds;  
           url = encodeURI(url);
	   ...
	   function open_subscribeXls(){
	       ...
	       var url='${req.contextPath}/reports/subscribe/subscribeDetailMonthView.xls?date_start='+getObjByName("subscribeDate_start").value+"&date_end="+getObjByName("subscribeDate_end").value+"&department.id="+dept+"&subIds="+subIds; 
           url = encodeURI(url);
           window.open(url,"_new","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=screen.width,height=screen.height,left=0,top=0"); 
	   }   
       ...
       function open_subscribeSupplierPdf(){
	       var subscribeDate = getObjByName("subscribeDate").value;
	       if(""==subscribeDate ||!isYearMonth(subscribeDate)){
	          alert("请输入正确的时间格式，正确格式2011-01");
	          getObjByName("subscribeDate").focus();
	          return false;
	       } 
