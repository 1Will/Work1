...
<@framePage title="${action.getText('purchaseBillDetails.title')}">	
	<@ww.form name="'listForm'" action="'sercherSparePart'" method="'post'">
		...
		<@ww.hidden name="'acceptBill.id'" value="'#{acceptBill.id}'"/>
		 ...
		 <#assign itemNo=1/>
		<@list title="" excel=false setupTable=false includeParameters="acceptBill.id|readOnly" fieldMap="like:">
       ...
       <#if !(action.isReadOnly())>
         ...
         <script language="javascript">
	      function open_sparePartDialog(acceptId,sparePartId) {
        		var url = '${req.contextPath}/accept/editSparePart.html?readOnly=${req.getParameter('readOnly')?if_exists}&acceptBill.id=' + acceptId;
        		if (null != sparePartId) {
