...
<base target="_self">
    ...
    <script language="JavaScript" type="text/JavaScript"> 
     ...
     window.onload = function() {
       ...
       <#if factory.companyType?exists>
         getObjByNameRe("companyType.id").value = #{factory.companyType.id};
       </#if>
       <#if factory.supplierType?exists>
         getObjByNameRe("supplierType.id").value = #{factory.supplierType.id};
       </#if>
        <#if factory.country?exists>
         getObjByNameRe("country.id").value = #{factory.country.id};
       </#if>
        <#if factory.tradeType?exists>
         getObjByNameRe("tradeType.id").value = #{factory.tradeType.id};
       </#if>
       ...
       function levelChange_OpenDialog(supplierLevelHistoryId) {
	     ...
	     if(getObjByNameRe("factory.legalPerson").value!='')	{
	     ...
	     if (!isValidLength(document.forms[0],"factory.registeredFunds",0,50)) {
		   ...
		   return false;
   	      ...
   	      if(getObjByNameRe("factory.comment").value!='')	{
