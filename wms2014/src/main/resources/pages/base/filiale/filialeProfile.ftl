...
//     alert(document.forms[ "filiale" ]["filiale.code.code"].value);
       ...
       function storeValidation(){
        if(getObjByName('filiale.code').value==''){
		...
		if(getObjByName('filiale.name').value==''){
		...
		if(getObjByName("filiale.leader").value==''){
         ...
         var pattern =/^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/;
             if(getObjByName('filiale.tel').value !="")
             {
                 if(!pattern.exec(getObjByName('filiale.tel').value))
                 {
		...
		//验证邮编
		if(getObjByName('filiale.postCode').value != ''){
		...
		//验证税号
		if(getObjByName('filiale.taxId').value != ''){
		...
		//验证开户行
		if(getObjByName('filiale.bank').value != ''){
		...
		//验证银行帐号
		if(getObjByName('filiale.bankAccount').value != ''){
