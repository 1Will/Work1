...
<@htmlPage title="${action.getText('productEdit.title')}">
	...
	　<#if product.disabled?exists>
	 ...
	 function validate() {
		  ...
		  */
		  if ('' == getObjByName('product.productNo').value) {       
		    alert("${action.getText('product.productNo.requiredstring')}");
		    return false;
		  } else {
		    if (!isValidLengthValue(getObjByName('product.productNo').value,0,30)) {
		  ...
		  */
		  if ('' == getObjByName('product.name').value) {        
		    alert("${action.getText('product.name.requiredstring')}");
		    return false;
		  } else {
		    if (!isValidLengthValue(getObjByName('product.name').value,0,30)) {
