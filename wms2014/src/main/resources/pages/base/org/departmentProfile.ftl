...
<@htmlPage title="${action.getText('department.title1')}">
  ...
  window.onload=function(){
       ...
       <#if department.filiale?exists>
            getObjByName('filiale.id').value=#{department.filiale.id};              
      </#if>
      </#if>
  }
  ...
  for (i=0; i<groups; i++) {
	...
	function checkInvalidParms() {
		if (getObjByNameRe("filiale.id").value == -1) {
     ...
     <script>
       function storeValidation(){
        if(getObjByNameRe("department.code").value==''){
		...
		if(getObjByNameRe("department.name").value==''){
		 ...
		 if(getObjByNameRe("filiale.id").value==''){
		...
		if(getObjByNameRe("department.tel").value!=''){
