...
<script language="javascript">
	...
	function occursHistorySubmit(op,cp,wp,up,fp){
		if(getObjByName(op).alt == "${action.getText('open')}"){
			getObjByName(op).alt="${action.getText('close')}";
			getObjByName(op).src="${req.contextPath}/images/icon/minimize.gif";
			cp.href=requestUrls[up];
			getObjByName(wp).style.display="block";
		}else if(getObjByName(op).alt == "${action.getText('close')}"){
			getObjByName(op).alt="${action.getText('open')}";
			getObjByName(op).src="${req.contextPath}/images/icon/maximize.gif";
			getObjByName(wp).style.display="none";
		}
	}
