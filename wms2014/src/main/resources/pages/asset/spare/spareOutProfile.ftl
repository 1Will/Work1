...
<@htmlPage title="${action.getText('spareOutBill.title')}">
  <style type="text/css">
	   ...
	   var billInfo=decodeURI("${billInfo?if_exists}");
	     if(billInfo!=""){
	        var ary=billInfo.split(",");
	         getObjByNameRe("spareOutBill.billNo").value=ary[0];
	         getObjByNameRe("spareOutBill.name").value=ary[1];
              getObjByNameRe("spareOutBill.inoutDateTm").value=ary[2];
	           getObjByNameRe("spareOutBill.inoutPeople").value=ary[3];
	            getObjByNameRe("spareOutBill.comment").value=ary[4];
	        }
	}
	...
	function transformInStockValue(){
	  var ary=new Array();
	   ary.push(getObjByNameRe("spareOutBill.billNo").value);
	   ary.push(getObjByNameRe("spareOutBill.name").value);
       	ary.push(getObjByNameRe("spareOutBill.inoutDateTm").value);
	         ary.push(getObjByNameRe("spareOutBill.inoutPeople").value);
	            ary.push(getObjByNameRe("spareOutBill.comment").value);
	              return ary;
	}
