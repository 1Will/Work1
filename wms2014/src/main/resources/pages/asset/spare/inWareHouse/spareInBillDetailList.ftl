...
window.onload=function(){
   ...
   function spareValidate(){
    ...
    function informationValidate(){
	    ...
	    var allmaxNumber = document.getElementsByName("maxNumber");
	    ...
	    for(var i=0;i<allSpareInBillDtlId.length;i++){
		    ...
		    if ("false" == getObjByNameRe("hidden.location.code"+i).value) {
		      ...
		      getObjByNameRe("location.code"+i).focus();
		      return false;
		    }
		    if ("" == getObjByNameRe("location.code"+i).value) {
   			  var locationCode = getObjByNameRe("location.code"+i).value;
		    ...
		    if(''== number){								//验证入库数量是否为空
   			...
   			var maxN = allmaxNumber[i].value;
   			if(''!=number && ''!=maxN){
   			   if(parseInt(formatDigital(number)) > parseInt(formatDigital(maxN))){
   			     alert("${action.getText('di')} "+(i+1)+" ${action.getText('row')}"+ ","+"入库数量应该小于等于采购数量！");
   				 return false;
   			   }
   			}
