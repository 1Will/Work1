...
<script language="javascript">
            ...
            getObjByName('storageGrade.id').value= #{spareInBill.storageGrade.id};
             var url = '';
              <#if spareInBill.storageGrade.id==208?if_exists>
                   getObjByName('supper').parentNode.parentNode.firstChild.style.display="inline";
	...
	-->
	 ...
	 var storageGradeId = getObjByName('storageGrade.id').value;
      ...
      if(storageGradeId ==209 && getObjByName('outWarehouse.id').value==""){
         alert("请选择来源仓库！");
         getObjByName('outWarehouse.id').focus();
         return false;
      } 
   ...
   //缁х画鏂板缓
    ...
    <ul id="beautytab">
			<li>
				<a id="details" onclick="activeTab(this);" href="${req.contextPath}/spare/listSpareInBillDetails.html?spareInBill.id=#{spareInBill.id}&notFather=true&readOnly=${req.getParameter('readOnly')?if_exists}&warehouse=${spareInBill.warehouse.id?if_exists}" target="frame" >${action.getText('spareInBillDetail')}</a>
			</li>
	</ul>
	<iframe name="frame" frameborder="0.5" src='${req.contextPath}/spare/listSpareInBillDetails.html?spareInBill.id=#{spareInBill.id}&notFather=true&readOnly=${req.getParameter('readOnly')?if_exists}&warehouse=${spareInBill.warehouse.id?if_exists}' marginHeight="0" marginWidth="0" scrolling="auto"  vspace=0 hspace=0 width="100%" height="100%"/>
	</#if>
</@htmlPage>