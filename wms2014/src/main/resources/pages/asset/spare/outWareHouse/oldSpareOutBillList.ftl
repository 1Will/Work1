...
<@htmlPage title="出库单查询">
    ...
    .definedLength{
	 ...
	 <@ww.form name="'spareOutBills'" action="'searchOldSpareOutBills'" method="'post'">
         <@ww.token name="searchSpareOutBillsToken"/>
          <@ww.hidden name="'readOnly'" value="'${req.getParameter('readOnly')?if_exists}'"/>
	 	  <#include "./oldSpareOutBillSearcher.ftl"/>
	       <#assign itemNo = 0/>
        <@buttonBar>
        	<@vsubmit value="'${action.getText('search')}'" onclick="'return checkInvalidParms();'"/>
        	 <#if !(action.isReadOnly())>
	        <@redirectButton value="${action.getText('new')}" url="${req.contextPath}/spare/editOldSpareOutBill.html?readOnly=${req.getParameter('readOnly')?if_exists}"/>
             </#if>
        </@buttonBar>  
        ...
        <@list title="出库单列表" 
          	...
          	<#if (object.disabled)>
          	<@vcolumn title="出库单编码" property="code" sortable="desc">
            	<@alignLeft />
            </@vcolumn>
          	<#else>
             <@vcolumn title="出库单编码" property="code" sortable="desc">
                <a href="editOldSpareOutBill.html?spareOutBill.id=#{object.id}&readOnly=${req.getParameter('readOnly')?if_exists}&pagingPage=${pagingPage?if_exists}">${object.code}</a>
            	<@alignLeft/>
             </@vcolumn>
            </#if>	
            <@vcolumn title="出库单名称" property="name" sortable="desc">
            	<@alignLeft/>
            </@vcolumn>
...
function open_SpareOutBillReportPdf(){
      ...
      var url='${req.contextPath}/reports/spare/listAllSpareOutBill.pdf?oldSpare=0&outType=0&spareOutBillNo='+spareOutBillNo+
...
function open_SpareOutBillReportXls(){
      ...
      var warehouseId = document.forms[0].elements["warehouse.id"].value;
      var inWarehouseId = document.forms[0].elements["inWarehouse.id"].value;
      var url='${req.contextPath}/reports/spare/listAllSpareOutBill.xls?oldSpare=0&outType=0&spareOutBillNo='+spareOutBillNo+
       '&spareOutBillName='+spareOutBillName+'&deptId='+deptId+'&borrower='+borrower+'&outPeople='+
       outPeople+'&outDate_start='+outDate_start+'&outDate_end='+outDate_end + '&warehouseId=' + warehouseId + '&inWarehouseId=' + inWarehouseId;
      url = encodeURI(url);
      window.open(url,"_new","toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=screen.width,height=screen.height,left=0,top=0");
