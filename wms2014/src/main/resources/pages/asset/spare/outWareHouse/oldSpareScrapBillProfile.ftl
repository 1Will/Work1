...
<@htmlPage title="报废单维护">
  <style type="text/css">
	...
	<base target="_self">
	<@ww.form name="'spareoutBill'" action="'saveOldSpareScrapBill'" method="'post'">
		<@ww.token name="saveSpareOutBillToken"/>	
		<@ww.hidden name="'spareOutBill.oldSpare'" value="'0'"/>
		<@ww.hidden name="'spareOutBill.outType'" value="'1'"/>
		...
		<@inputTable>
			...
			<tr>
				<@ww.textfield label="'报废单编码'" name="'spareOutBill.code'" value="'${spareOutBill.code?if_exists}'" cssClass="'underline'" readonly="false" required="true" />
	 	  		<@ww.textfield label="'报废单名称'" name="'spareOutBill.name'" value="'${spareOutBill.name?if_exists}'" cssClass="'underline'" required="true"/>
	 	    </tr>
		...
		<@buttonBar>
	 	    <#if !(action.isReadOnly())>	
	        	<@vsubmit name="'save'" value="'${action.getText('save')}'" onclick="'return checkInput()'"/>
	        	   <#if spareOutBill.id?exists>	 
	        	       <@redirectButton value="${action.getText('继续新建')}" url="${req.contextPath}/spare/editOldSpareScrapBill.html?readOnly=${req.getParameter('readOnly')?if_exists}&pagingPage=${pagingPage?if_exists}"/>  
	               </#if>
	        </#if>
	        	<@redirectButton  value="${action.getText('back')}" url="listOldSpareScrapBill.html?readOnly=${req.getParameter('readOnly')?if_exists}&pagingPage=${pagingPage?if_exists}"/>
	        	<#if spareOutBill.id?exists>
	...
	<script language="JavaScript" type="text/JavaScript">
      ...
      <#if spareOutBill.id?exists>   
      	<#if spareOutBill.inWarehouse?exists>
			 var url = 'listOldSpareScrapBillDetail.html?spareOutBill.id=#{spareOutBill.id}&readOnly=${req.getParameter('readOnly')?if_exists}&warehouse=${spareOutBill.warehouse.id}&inwarehouse=${spareOutBill.inWarehouse.id}';      
			 <#else>
			 var url = 'listOldSpareScrapBillDetail.html?spareOutBill.id=#{spareOutBill.id}&readOnly=${req.getParameter('readOnly')?if_exists}&warehouse=${spareOutBill.warehouse.id}';      
		 </#if>
    ...
    <#if spareOutBill.id?exists>
		<ul id="beautytab">
			<li><a id="details" onclick="activeTab(this);"  
				href="listOldSpareScrapBillDetail.html?spareOutBill.id=#{spareOutBill.id}&readOnly=${req.getParameter('readOnly')?if_exists}" target="frame" >报废单明细</a>
			</li>
		</ul>
