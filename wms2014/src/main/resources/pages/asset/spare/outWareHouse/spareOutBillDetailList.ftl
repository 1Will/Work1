...
<@framePage title="${action.getText('spareOutBillDetail.title')}">	
  <style type="text/css">
	...
	<@ww.form namespace="'/spare'" name="'listForm'" action="'seacherSpareOutBillDetail'" method="'post'">
		...
		<@ww.hidden name="'pagingPage'" value="'${pagingPage?if_exists}'"/>
		 ...
		 <@ww.hidden name="'allSheBeiNo'" value="" />
		  <@ww.hidden name="'allUseTypes'" value="" />
		...
		<#assign useTypes = 'useTypes' + '${itemNo}'/>
		...
		<@list title=""   includeParameters="spareOutBill.id" fieldMap="like:">
          ...
          <#--  <input type="hidden" name="spareLocationId" value="#{object.spareLocationId.id}"/>-->
            <@vcolumn title="${action.getText('spareOutBillDetail.itemNo')}">
             ...
             <@vcolumn title="${action.getText('spareOutBillDetail.modelSpecs')}" property="spare.modelSpecs" >
              	<@alignLeft />
             </@vcolumn>
            ...
            <#if spareOutBill.warehouse?exists && spareOutBill.warehouse.storageGrade.id==208?if_exists>
             ...
             <@vcolumn title="${action.getText('spareOutBillDetail.unit')}" property="spare.unit.value" >
              	<@alignLeft />
             </@vcolumn>
			...
			<#if object.status?exists>
				<#if '${object.status}' == 'OUTWAREHOUSEED'>
                	<@vcolumn title="${action.getText('spareOutBillDetail.number')}" format="${action.getText('currencyFormat')}">
         ...
         <#if spareOutBill.warehouse?exists && spareOutBill.warehouse.storageGrade.id==209?if_exists>
             <@vcolumn title="${action.getText('领料人')}" property="" >
	            ...
	            <@vcolumn title="设备编号" >
	           	 <input type="text" name="shebeiNo" class="underline"  value="${object.shebeiNo?if_exists}" onchange="" maxlength="20" size="7" style="text-align:right"/>
					<@alignLeft />
	            </@vcolumn>  
	            <@vcolumn title="备件用途" >
	             <#if '${object.status}' == 'NEW'>
	              <select name="${useTypes}">
	              		 <option value=""></option>
					      <option value="001">安全整改</option>
					      <option value="002">应急维修</option>
					      <option value="003">预防维护</option>
					      <option value="004">公用及其他</option>
				      </select>
				        <#else>
				          <select name="${useTypes}" disabled >
				          <option value=""></option>
					      <option value="001">安全整改</option>
					      <option value="002">应急维修</option>
					      <option value="003">预防维护</option>
				        ...
				        <script language="javascript">
				         <#if object.useTypes?exists>
			              document.forms[0].elements["${useTypes}"].value='${object.useTypes?if_exists}';
			              //document.forms[0].elements["${useTypes}"].value='003';
			              </#if>
			          </script>
			          <#assign useTypes = 'useTypes' + '${itemNo}'/> 
	            </@vcolumn>    
	            <#-- 所属班组 -->
	            <@vcolumn title="${action.getText('groups')}" >
	            ...
	            <#-- 
	             <@vcolumn title="${action.getText('以旧换新')}" >
	          
	                <input type="radio" name="${newOrOldIdentity}" id = "${oldIdentity}" value="true" ><font id = "${oldText}">是 </font></input>
	                <input type="radio" name="${newOrOldIdentity}" id ="${newIdentity}"  value="false"><fontid = "${newText}">否</font></input>
	                <#assign newIdentity = 'new'+'${itemNo}'>
		            <#assign oldIdentity = 'old'+'${itemNo}'>
		            <#assign newText = 'newT'+'${itemNo}'>
		            <#assign oldText = 'oldT'+'${itemNo}'>
	                <script>
	                    ...
	                    strOld = strOld+item;
	                    strNew = strNew+item;
	                   // alert(getObjByName("oldT"+item).innerHTML)
		               <#if object.newOrOld>
		                 getObjByName(strOld).checked="checked";
		                 //getObjByName("oldT"+item).style="color:red;";
		                  
		               <#else> 
		                 getObjByName(strNew).checked="checked";
		               </#if>
	               </script>
	            ...
	            -->
	            <input type="hidden" name="${newOrOldIdentity}" id ="${newIdentity}"  value="false" />
	        <@vcolumn title="${action.getText('备注')}" property="" >
                 <input type="text" name="comment" class="underline"  value="${object.comment?if_exists}"  maxlength="100"   />
             </@vcolumn>       
            <@vcolumn title="${action.getText('出库人')}" >
              	<@peopleSelectorInList/>
              	<#assign peopleIdentityName = 'outPeopleName'+'${itemNo}'/>
		        <#assign peopleIdentityId= 'outPeopleId'+'${itemNo}'/>
             </@vcolumn>   
             
	              <@vcolumn title="${action.getText('出库日期')}">
             ...
             <input type="hidden" name="stocksBeforeInOrOut" value="${object.stocksBeforeInOrOut}" />
        ...
        <@buttonBar>
        	...
        	<@vbutton name="'new'"  class="button" value="从备件库明细台帐选择" onclick="open_spareDtlDialog()"/>	       	
            <#--
            ...
            <#if spareOutBill.oldForNew?exists>
            <@vbutton name="'loolOld'"   class="button" value="查看旧件" onclick="lookOld('#{spareOutBill.oldForNew.id}','#{spareOutBill.id}','${itemNo}','${itemNewNo}')"/>
            <#else>	
            <@vbutton name="'oldToNew'" class="button" value="以旧换新" onclick="open_yjhxDialog('${spareOutBill.warehouse.code?if_exists}','#{spareOutBill.id}','${itemNo}','${itemNewNo}')"/>	 
            </#if> 
            </#if>
            </#if>
        </@buttonBar>
        <input type="hidden" name="oldInBillid" id="oldInBillid">
	</@ww.form>
<script language="JavaScript" type="text/JavaScript">
	...
	function open_yjhxDialog(oldWhouseCode,outId,itemNo,itemNewNo){
	if(itemNo ==1){
	alert("请填写入库单明细！");
	return;
	}
	...
	var co = 'JJ-'+oldWhouseCode;
	var url ='${req.contextPath}/spare/editSpareInBillForOldToNew.html?isOpen=true&outId='+outId; //'${req.contextPath}/spare/listOldSpareInBill.html?isOpen=true&oldWhouseCode='+co;
		window.open(url);
	}
	
	function lookOld(oldid,outId,itemNo,itemNewNo){
	if(itemNo ==1){
	alert("请填写入库单明细！");
	return;
	}
	...
	var url = '${req.contextPath}/spare/editSpareInBillForOldToNew.html?spareInBill.id='+oldid+'&isOpen=true&outId='+outId;
		window.open(url);
	}
    ...
    //取得所有设备的值
    function getSheBeiNoText(){
      var detailIdSelector = document.getElementsByName("spareOutBillDetailIds"); 
      var shebeiNoSelector =  document.getElementsByName("shebeiNo");
      var length = detailIdSelector.length;
 	  var ary = new Array();
 	  if(length){
 	     for(var i=0;i<length;i++){
 	        if("" !=shebeiNoSelector[i].value){
 	           ary.push(detailIdSelector[i].value);
 	           ary.push(shebeiNoSelector[i].value);
 	        }else{
 	            alert("第"+(i+1)+"行设备编号未填写！");
		  	    return false;
 	        }
 	     }
 	  }
      document.forms[0].elements["allSheBeiNo"].value=ary;
    }
    
    function getUseTypesText()
	{
	   var detailIdSelector = document.getElementsByName("spareOutBillDetailIds"); 
       var ary = new Array();
       for (var i=0; i<detailIdSelector.length; i++) {
         var useTypes = 'useTypes';
         useTypes = useTypes + (i+1); 
        
         if("" != document.forms["listForm"].elements[useTypes].value) {
           ary.push(detailIdSelector[i].value);
           ary.push(document.forms["listForm"].elements[useTypes].value);
         }else{
          alert("第"+(i+1)+"行备件用途未填写！");
		  	    return false;
         }
       }
        
       document.forms["listForm"].elements["allUseTypes"].value = ary;
	}
    ...
    var black="";
 	function submitDetailIds(){
	    ...
	    <#if spareOutBill.warehouse?exists && spareOutBill.warehouse.storageGrade.id==209>
		    ...
		    //验证使用设备
		   if(false == getSheBeiText()){
		        return false;
		    }
		    
		     //验证使用设备编号
		   if(false == getSheBeiNoText()){
		        return false;
		    }
		    
		     //验证设备用途
		   if(false == getUseTypesText()){
		        return false;
		    }
		    //验证出库人
