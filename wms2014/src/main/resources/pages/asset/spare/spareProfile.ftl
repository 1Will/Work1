...
<@htmlPage title="${action.getText('spare.title')}">
	 	  ...
	 	  <#if spare.id?exists>
	 	     <@ww.hidden name="'spare.id'" value="#{spare.id?if_exists}"/>
	 	  	...
	 	  	<td align="right" valign="top">
	       		...
	       		<label class="label">${action.getText('spare.spareNo')}:</label>
	     	</td>
            <td>
	             <input type="text" name="spare.spareNo" value="${spare.spareNo?if_exists}" class="underline" onblur="toEditSpare()" >
	             <input type="hidden" name="spareNoTmep" value="${spare.spareNo?if_exists}" >
        	 </td>
    ...
    */
    <#if spare.enabled?exists>
     ...
     window.onload = function () {
        <#if spare.id?exists>
			...
			getObjByNameRe("spareDetail").className = "selectedtab";
		</#if>
     	<#--<#if spare.id?exists>
     		var url = '${req.contextPath}/asset/spare/listSpareDocs.html?spare.id=#{spare.id}';
		    document.all.frame.src= url;
		    getObjByNameRe("spareDoc").className = "selectedtab";
     	</#if>-->
    ...
    -->
    	//if (!(''==document.forms[0].elements["spare.graphNo"].value)) {
    		//if(!isValidLength(document.forms[0], "spare.graphNo", null, 50)){
			  ...
			  //}
    	//}
    	var spareN =document.forms["spare"].elements["spare.spareNo"].value; 
    	var spareNo=spareN.replace(/(^\s*)|(\s*$)/g,"");
        if ('' == spareNo) {
          alert("${action.getText('spare.spareNo.requiredstring')}");
		  return  false;
        }
    	if (!('' == spareNo)) {
    		if(!isValidLength(document.forms[0], "spare.spareNo", null, 50)){
			    alert("${action.getText('spare.spareNo.maxLength')}");
				return  false;
			  }
			  dwr.engine.setAsync(false);
    	<#if spare.id?exists>
    	  var spareN=getObjByName('spare.spareNo').value;//用户输入的备件编码
    	 var spareNo = spareN.replace(/(^\s*)|(\s*$)/g,"");
    	  var spareNoTmep=getObjByName('spareNoTmep').value;
    	 if(spareNoTmep!=spareNo){
    	 loadSpareByPamrameterJs.loadBySpareNo(spareNo,callbackSpareNoValidate);
    		if(resultData != null && resultData != ""){
    		if(confirm("您所输入的备件编码已经存在，是否需要跳转到编辑页面?")){
							document.location='${req.contextPath}/asset/spare/editSpares.html?spare.id='+resultData["id"]+'&toolingDevFlag=${toolingDevFlag?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}';
							 return false;
							 }else{
							 return false;
							 }
    	...
    	<#else>
	    	 var spareNo=getObjByName('spare.spareNo').value;
	    	loadSpareByPamrameterJs.loadBySpareNo(spareNo,callbackSpareNoValidate);
    		if(resultData != null && resultData != ""){
    		if(confirm("您所输入的备件编码已经存在，是否需要跳转到编辑页面?")){
							document.location='${req.contextPath}/asset/spare/editSpares.html?spare.id='+resultData["id"]+'&toolingDevFlag=${toolingDevFlag?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}';
    	...
    	//验证备件型号数据
    	var modelSpecs=document.forms["spare"].elements["modelSpecs"].value;
        if ('' == modelSpecs) {
          alert("请输入型号!");
		  return  false;
        }
    	if (!('' == modelSpecs)) {
    		if(!isValidLength(document.forms[0], "modelSpecs", null, 75)){
			    alert("型号的长度最大为75字符");
				return  false;
			  }
    	}
    ...
    function toEditSpare(){
    var spareN=getObjByName('spare.spareNo').value;//用户输入的备件编码
     var spareNo = spareN.replace(/(^\s*)|(\s*$)/g,"");
    var spareNoTmep=getObjByName('spareNoTmep').value;//未修改前的备件编码
    if(spareNo==""){
     return false;
	 }
	 <#if spare.id?exists>  //判断如果是修改编辑情况下。 备件编码如果不变， 不需要触发检索
	 if(spareNoTmep==spareNo){
	 return false;
	 }
	 </#if>
	 loadSpareByPamrameterJs.loadBySpareNo(spareNo,callbackSpareNo);
    
    }
      function callbackSpareNo(data){
      if(data!=null&&data["id"]!=''){
       if(confirm("您所输入的备件编码已经存在，是否需要跳转到编辑页面?")){
		document.location='${req.contextPath}/asset/spare/editSpares.html?spare.id='+data["id"]+'&toolingDevFlag=${toolingDevFlag?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}';
		}
      }
    }
    </script>
