...
<script language="javascript">
	window.onload= function(){
		...
		<#if regional.storageGrade?exists>
          getObjByName('storageGrade.id').value = #{regional.storageGrade.id?if_exists};
	...
	function validate(){
		//验证库区编码
		if(getObjByName("regional.code").value==""){
			alert('${action.getText('regional.code.requiredstring')}');
			getObjByName("regional.code").focus();
			return false;
		}
		if(getObjByName("regional.code").value.length>50){
			alert('${action.getText('regional.code.stringlength')}');
			getObjByName("regional.code").focus();
			return false;
		}
		//验证库区名称
		if(getObjByName("regional.name").value==""){
			alert('${action.getText('regional.name.requiredstring')}');
			getObjByName("regional.name").focus();
			return false;
		}
		if(getObjByName("regional.name").value.length>50){
		...
		//验证备注
		if(getObjByName("regional.comment").value.length>500){
			alert('${action.getText('regional.comment.stringlength')}');
			getObjByName("regional.comment").focus();
			return false;
		}
