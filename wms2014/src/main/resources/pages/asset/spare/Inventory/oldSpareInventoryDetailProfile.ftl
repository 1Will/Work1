...
<base target="_self">
	<@ww.form name="'saveSpareInventoryDetail'" action="'saveOldInventoryBillDetail'" method="'post'" validate="true">
		...
		<@buttonBar>
	      ...
	      <#if spareInventoryDetail.id?exists>
			  <@hrefButton value="${action.getText('继续新建')}" url="${req.contextPath}/spare/editOldInventoryBillDetail.html?spareInventoryBill.id=#{spareInventoryBill.id}&toolingDevFlag=${toolingDevFlag?if_exists}"/>	
		  </#if>
 ...
 <script language="javascript">
	...
	getObjByName('regional.id').value='1687';
	getObjByName('location.id').value='51577';
	  ...
	  function autoCompleteSpare(){
	      ...
	      var url = "${req.contextPath}/spare/searchOldSpareLocationToFrames.html?name=" + spareName
