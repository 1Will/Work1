...
<@htmlPage title="${action.getText('spare.title')}">
	 <@ww.form name="'spare'" action="'saveOldSpares'" method="'post'" validate="true">
	 	<@ww.token name="saveSparesToken"/>
	 	<@ww.hidden name="'toolingDevFlag'" value="'${toolingDevFlag?if_exists}'"/>
		<@ww.hidden name="'spare.oldSpare'" value="'0'"/>
	 	 <@ww.hidden name="'readOnly'" value="'${req.getParameter('readOnly')?if_exists}'"/>
	 	  ...
	 	  <#if !(action.isReadOnly())>
			 ...
			 <#if toolingDevFlag=='TOOLING'>	        	
	        	<@redirectButton  name="back" value="${action.getText('back')}" url="listOldSpares.html?oldSpare=0&toolingDevFlag=TOOLING&readOnly=${req.getParameter('readOnly')?if_exists}"/>
	         <#elseif toolingDevFlag=='DEVICE'>
	         	<@redirectButton  name="back" value="${action.getText('back')}" url="listOldSpares.html?oldSpare=0&toolingDevFlag=DEVICE&readOnly=${req.getParameter('readOnly')?if_exists}"/>
	         <#else>
	            <@redirectButton  name="back" value="${action.getText('back')}" url="listOldSpares.html?oldSpare=0&toolingDevFlag=TOOLINGDEVICE&readOnly=${req.getParameter('readOnly')?if_exists}"/>
	         </#if>
	         <#else>
	            <@redirectButton  name="back" value="${action.getText('back')}" url="listOldSpares.html?oldSpare=0&toolingDevFlag=TOOLINGDEVICE&readOnly=${req.getParameter('readOnly')?if_exists}"/>
	     	</#if>
    ...
    */
    ...
    -->
    	...
    	<#if spare.id?exists>
    	<#else>
    		//alert("name:"+getObjByName('spare.name').value+"   modelSpecs:"+getObjByName('modelSpecs').value+"   orderNo:"+getObjByName('spare.orderNo').value+"   factoryStr:"+getObjByName('factory.name').value);
	    	
	    	 var ary =new Array();
	    	ary.push(getObjByName('spare.name').value);
	    	ary.push(getObjByName('modelSpecs').value);
	    	ary.push(getObjByName('spare.orderNo').value);
	    	ary.push(getObjByName('factory.id').value);
	    	ary.push(getObjByName('category.id').value);
	    	//ary.push(getObjByName('spareDetailType.id').value);
