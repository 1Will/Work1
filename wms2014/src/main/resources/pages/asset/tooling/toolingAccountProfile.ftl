   ...
   function deleteAllDetailTypes() {     //删除生产线的选择框的值
		...
		function scrollNav(pos) {
			var c = 1;	
			 for(var id = pos;id<=13;id++, c++) {
			  ...
			  <#--
			  if(c<=4){
			  getObjByNameRe(c).innerHTML = navItem[id];
			  <#if tooling.id?exists>
			  	if (c == 1) {
		     		document.all.frame.src = getObjByNameRe(navItemId[id]).href;
			 		getObjByNameRe(navItemId[id]).className = "selectedtab";
			 	}
		   	  </#if>
			  }
			  -->
	...
	<script language="javascript" type="text/JavaScript">
	   ...
	   <#if tooling.id?exists>
		 ...
		 getObjByNameRe("toolingExtInfo").className = "selectedtab";
		 try{getObjByNameRe("navl").style.visibility="hidden"}catch(e){};
	   <#else>
		   a = new Date();
	   ...
	   if ('' == getObjByNameRe("toolingType.id").value) {
