...
<script language="javascript">
	...
	*/
	function spareTypeValueChange(spareId, spareDetailTypeId) {
	  var selectedSpareId = getObjByNameRe(spareId).value;
	  if ('-1' != selectedSpareId) {
	    ...
	    getObjByNameRe(spareDetailTypeId).value = -1;
	    getObjByNameRe(spareDetailTypeId).length=1;
	  }
	}
	/*
	 * 给备件明细类的选择框赋值
	*/
	function createDetailTypeSelect(ary, spareDetailTypeId) {     
	  var obj = getObjByNameRe(spareDetailTypeId);
	...
	*/  	
	function deleteAllDetailType(spareId, spareDetailTypeId) {     
      getObjByNameRe(spareDetailTypeId).value='';
	  var obj = getObjByNameRe(spareDetailTypeId);
