...
public class NewsController extends BaseController {
	...
	public String getNewsList() {
		Page<CmsNews> page = getPage();
		return jsonPage(newsService.selectPage(page, null));
	}
	...
	public String editNews( CmsNews news ) {
		boolean rlt = false;
		//判断新闻实体是否为空，为空则不进行操作
		if ( news != null ) {
			//判断新闻是否存在
			if ( news.getId() != null ) {
				//存在，更新信息
				news = getCommNewsInfo(OptionType.OP_EDIT.getOp(),news); //更新状态下，新闻实体的公共信息部分填充
				rlt = newsService.updateById(news); //更新操作
			} else {
				//不存在，插入信息
				news = getCommNewsInfo(OptionType.OP_INSERT.getOp(),news); //插入状态下，新闻实体的公共信息部分填充
				rlt = newsService.insertSelective(news); //插入操作
			}
		}
	 ...
	 * @date 2016-09-01
	 */
	private CmsNews getCommNewsInfo(int flag,CmsNews news){
		Date time = new Date(System.currentTimeMillis()); //获取当前时间
		//操作信息的添加
		//插入标记
		if(flag == 1){
			news.setCreateUser(getSSOToken().getData()); //添加创建用户
			news.setCreateTime(time); //添加当前时间作为创建时间
			
			news.setVersion(1);	//设置版本号
		}
		//更新标记
		if(flag == 2){
			news.setModifyUser(getSSOToken().getData()); //添加更新用户
			news.setModifyTime(time); //添加当前时间作为更新时间
		}
