...
<@ww.form name="'listForm'" action="'saveMyRunTask'" method="'post'">
	...
	<@inputTable>
		...
		<tr>
	        ...
	        <td colspan="4">
	        	<textarea name="remark" rows="4" cols="150" ></textarea>
	        </td>
		</tr>
	</@inputTable>
	<@buttonBar>
...
<ul id="beautytab">
	<li>
		<a id="runPoint" onclick="activeTab(this);"  href='${req.contextPath}/activitiFlow/listRunPoint.html?historyTask=historyTask&bussnessId=#{runTask.bussnessId}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('审批人')}</a>
	</li>
</ul>
<iframe name="frame" frameborder="0.5" src="${req.contextPath}/activitiFlow/listRunPoint.html?historyTask=historyTask&bussnessId=#{runTask.bussnessId}&readOnly=${req.getParameter('readOnly')?if_exists}" marginHeight="0" marginWidth="0" scrolling="auto" vspace=0 hspace=0 width="100%" height="60%"/>
