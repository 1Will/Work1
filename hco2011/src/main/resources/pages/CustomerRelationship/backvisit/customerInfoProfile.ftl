...
<@htmlPage title="${action.getText('客户详细信息页面')}">
<@ww.form name="'listForm'" action="'saveCustomerInfo'" method="'post'">
	...
	<@inputTable>
		<#--<tr>
            ...
            <td>
	             <input type="text" name="customerInfo.name" value="${customerInfo.name?if_exists}" class="underline" onkeyup="showCustomer()" autocomplete="off">
	             <div id="show" style="background-color:#FFFFFF;border:1px solid; overflow:auto; display: none;z-index:2;position:absolute;left:43.7%;top:18%;"></div>
            </td>
		...
		<tr>
			...
			<@ww.textfield label="'${action.getText('customerInfo.registeredCapital')}'" name="'customerInfo.registeredCapital'" value="'${(customerInfo.registeredCapital?string('#,###,##0.00'))?if_exists}'" cssClass="'underline'" />
		...
		<tr>
			...
			<@ww.textfield label="'${action.getText('customerInfo.postCode')}'" name="'customerInfo.postCode'" value="'${customerInfo.postCode?if_exists}'" cssClass="'underline'" />
		</tr>
		<tr>
		...
		<tr>	
			...
			<@ww.textfield label="'${action.getText('customerInfo.telphone')}'" name="'customerInfo.telphone'" value="'${customerInfo.telphone?if_exists}'" cssClass="'underline'" required="true"/>
	...
	<@buttonBar>
		...
		<@vbutton name="'close'" value="关闭" onclick="window.close()" class="button"/>
    </@buttonBar>
</@ww.form>
<script language="JavaScript" type="text/JavaScript"> 
	...
	window.onload = function () {
		...
		//附加信息Tab-->
		<#--<#if customerInfo.id?exists>
			document.all.frame.src='${req.contextPath}/customerRelationship/editAdditionalInfo.html?customerInfo.id=#{customerInfo.id}';
		</#if>-->
 	<#--}
	...
	function storeValidation(){
	    ...
	    if(getObjByName('customerInfo.postCode').value!=""){
	     	if(!isNumber('customerInfo.postCode')){
            	alert('${action.getText('customerInfo.postCode.type')}');
            	getObjByName('customerInfo.postCode').value="";
            	getObjByName('customerInfo.postCode').focus();
	        	return false;
	        }
	     }
		...
		if(getObjByName('customerInfo.telphone').value==''){
	        alert('${action.getText('customerInfo.telphone.not.null')}');
		...
		if(getObjByName('customerInfo.chuanzhen').value!==""){
	        if(!isValidLength(document.forms[0], "customerInfo.chuanzhen", null, 20)){
            	alert('${action.getText('customerInfo.chuanzhen.length')}');
            	getObjByName('customerInfo.chuanzhen').value="";
            	getObjByName('customerInfo.chuanzhen').focus();
	        	return false;
            }
	     }
...
<ul id="beautytab">
	<li>
		<a id="additionalInfo" onclick="activeTab(this);" class="selectedtab" href='${req.contextPath}/customerRelationship/editAdditionalInfo.html?customerInfo.id=#{customerInfo.id}' target="frame" >${action.getText('customerInfo.additionalInfo')}</a>
	</li>
	<li>
		<a id="contactArchives" onclick="activeTab(this);" href='${req.contextPath}/customerRelationship/listCA.html?customerInfo.id=#{customerInfo.id}&type.id=${customerInfo.customerType.id?if_exists}' target="frame" >${action.getText('customerInfo.contactArchives')}</a>
	</li>
</ul>
