...
<@ww.form name="'listForm'" action="'editOnTheRoadBill_'" method="'post'">
	...
	<@inputTable>
		<tr>
			...
			<@pp.datePicker 
				...
				required="true"
				dateFormat="'%Y-%m-%d'"/>
			<@ww.textfield label="'申请人'" name="'applyPerson'" value="'梁戈'" cssClass="'underline'"/>
		</tr>
		<tr>
			<@ww.select 
	    		label="'部门'"
		...
		<tr>
			<@ww.textarea label="'事由'" name="'incident'" value="'去市公安局'" rows="'3'" cols="'100'"/>
		</tr>
		<tr>
	...
	<@buttonBar>
		...
		<@redirectButton value="${'返回'}" url="${req.contextPath}/onTheRoadBill_/listOnTheRoadBill_.html"/>
    </@buttonBar>
</@ww.form>
</@htmlPage>