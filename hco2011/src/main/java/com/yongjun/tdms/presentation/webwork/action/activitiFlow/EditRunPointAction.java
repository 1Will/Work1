	...
	public String save() {
		...
		try {
			if (hasId("runPoint.inspectPser.id")) {
				this.runPoint.setInspectPser(personnelFilesManager.loadPersonnel(Long.valueOf(this.request.getParameter("runPoint.inspectPser.id"))));
			}
			...
			if (isNew) {
				//设置runpPoint序号
				List<RunPoint> runPoints = runPointManager.loadByKey("bussnessId", this.runPoint.getBussnessId());
				if(runPoints != null){
