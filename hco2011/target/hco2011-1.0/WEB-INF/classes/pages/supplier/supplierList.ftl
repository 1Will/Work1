	...
	<@ww.form name="'listForm'" namespace="'/supplierManager'" action="'seacherSupplier'" method="'post'">
        ...
        <@list title="${action.getText('supplierList')}" 
            ...
            <@vcolumn title="${action.getText('supplier.supplierNo')}" property="supplierNo" sortable="desc">
                <a href="editSupplier.html?supplier.id=#{object.id}&readOnly=${req.getParameter('readOnly')?if_exists}">${object.supplierNo}</a>
                <@alignLeft/>
            </@vcolumn>
            </#if>
        ...
        <#if !(action.isReadOnly())>
        <@buttonBar>
          <@crm_disabledOrEnabled_button message="${action.getText('supplier')}" boxName="supplierIds" jsFunctionName="checkInvalidParms()"/>
        </@buttonBar>
        </#if>
        </#if>
    </@ww.form>
