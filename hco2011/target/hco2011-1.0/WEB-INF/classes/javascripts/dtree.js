...
// Node object
function Node(id, pid, name, varId, mark, url, title, target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;		//新增-------------start	/*主要完成含有checkbox的树形菜单	  varId:就是传入当前节点元素在数据库中的id，	  mark：针对层次的表示，例如：单位是一张表，部门是一张表，	        这样就要区分开哪些节点属于单位，哪些节点属于部门	*/	this.varId = varId;		this.mark = mark;	//----------------end
	this.url = url;
	this.title = title;
	this.target = target;
	this.icon = icon;
...
dTree.prototype.add = function(id, pid, name, varId, url, title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, varId, url, title, target, icon, iconOpen, open);
};
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		if(this.obj=='d' || this.obj=='dt'){				}else{			str += '<input type="checkbox" name="id" title="'+node.mark+'" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.varId +'" class="cx"/>';		}		//----------end	}
...
if (!Array.prototype.pop) {
	Array.prototype.pop = function array_pop() {
		...
		function caBox(formId, regx ) {
			if( document.getElementById(formId).elements[regx].checked ){				var form = document.getElementById(formId);
				...
				for(j=0;j<regxArray.length;j++){
					...
					if(document.getElementById(pDiv))
						document.getElementById(formId).elements[pDiv].checked=true;
				}
			}
			else{
				// unChecked				var form = document.getElementById(formId);
