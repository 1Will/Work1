...
function uploadSuccess(file, serverData) {
	try {
		...
		if(serverData.indexOf("exist_") != -1){
			...
			document.getElementById(fileId).cells[0].innerHTML = document.getElementById(fileId).cells[0].innerHTML + "&nbsp;<font color='red'>(此文件已被上传过了!)</font>";
			document.getElementById(fileId).cells[2].innerHTML = "&nbsp;<font color='red'>上传失败</font>";
		}else if(serverData.indexOf("<html>") != -1){
			document.getElementById(fileId+"_filepath").value = "error_";
			document.getElementById(fileId).cells[2].innerHTML = "&nbsp;<font color='red'>上传失败</font>";
		}else{
			var keystr = "[" + file.name + "-" + file.size + "]";
			if(fileList.indexOf(keystr) != -1){
				document.getElementById(fileId+"_filepath").value = "exist_";
				document.getElementById(fileId).cells[0].innerHTML = document.getElementById(fileId).cells[0].innerHTML + "&nbsp;<font color='red'>(此文件已被上传过了!)</font>";
				document.getElementById(fileId).cells[2].innerHTML = "&nbsp;<font color='red'>上传失败</font>";
			}else{
				fileList += keystr;
...
function addReadyFileInfo(file){
	...
	//document.getElementById("td_btnsc").style.display = "";//直接上传不需要点击上传按钮
