...
function uploadSuccess(file, serverData) {
	try {
		...
		var filesId = file.id;
		document.getElementById(filesId+"_filesize").value = file.size;
		document.getElementById(filesId+"_filename").value = file.name;
		document.getElementById(filesId+"_filepath").value = serverData;
		if(serverData.indexOf("exist_") != -1){
			document.getElementById(filesId).cells(0).innerHTML = document.getElementById(filesId).cells(0).innerHTML + "&nbsp;<font color='red'>(此文档已经上传过!)</font>";
		}
	} catch (ex) {
		this.debug(ex);
	}
...
function addReadyFileInfo(file){
	...
	col2.innerHTML = strsize + "<input type='hidden' id='"+filesid+"_filesize' name='filesize' value=''/><input type='hidden' id='"+filesid+"_filename' name='filename' value=''/><input type='hidden' id='"+filesid+"_filepath' name='filepath' value=''/>";
	col3.innerHTML = "<div class=jd1><span class=jd2><div id=jd_"+filesid+" class=jd3></div></span><span id=jdz_"+filesid+" class=jd4>0%</spanv></div>";
	col4.innerHTML = "<a href='javascript:deleteFile(\""+filesid+"\")'>删除</a>";
...
function deleteFile(filesId){
	//用表格显示
	var infoTable = document.getElementById("infoTable");
	var row = document.getElementById(filesId);
	infoTable.deleteRow(row.rowIndex);
	swfu.cancelUpload(filesId,false);
	...
	try {
		if (this.getStats().files_queued > 0) {
			...
			for(var i=1;i<num;i++){
				...
				"<dl><dt><span style='font-weight:normal; color:#FF0000; display:inline;'>*</span>序号：</dt><dd><input id=index_"+i+" name='orderindex' type='text' CK_NAME='排序号' CK_TYPE='NotEmpty,Number' value='"+norder+"'  style='width:100px; height:25px; line-height:25px;' maxlength='25'/></dd></dl>"+
				"<dl><dt><span style='font-weight:normal; color:#FF0000; display:inline;'>*</span>照片简介：</dt><dd><textarea name='digest' CK_NAME='照片简介' CK_TYPE='NotEmpty,MaxLen_100' style='width:315px; height:65px;'>"+filename.substring(0,filename.lastIndexOf("."))+"</textarea></dd></dl>"+
				"<dl><dt>详细介绍：</dt><dd><textarea name='descript' style='width:480px; height:180px;'>"+filename.substring(0,filename.lastIndexOf("."))+"</textarea></dd></dl>";
				if(i == 1 && num > 2){
