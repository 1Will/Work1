...
function saveRecord(){
	...
	if(flag == "2010"){
  	...
  	obj.action="/docFileErrorAction.do?method=<bean:write name="act"/>&type=1";
  	obj.submit();
}
function noTreatment(){
  obj=document.all("docFileErrorActionForm");
  obj.action='/docFileErrorAction.do?method=noTreatment&type=1';
  obj.submit();
}

function returnback(){
	obj = document.all("docFileErrorActionForm");
	obj.action='/docFileErrorAction.do?method=list&type=1';
    obj.submit();
}
function showDialog(){
	...
	diag.CancelEvent = function(){
		if(diag.innerFrame.contentWindow.document.getElementById('uploadimageurl')){
			...
			if(uploadtip != ''){
		  		...
		  		document.docFileErrorActionForm.topreview.src=uploadimageurl;
