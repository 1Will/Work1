...
function doOnLoad(){
 	window.parent.frames["left"].location = "/sysUserGroupMemberAction.do?method=outmember&groupid=<bean:write name="groupid"/>" ; 
}
function delUser(){
  if(num>0){
	  var str="此操作将给用户组删除选中用户，真的要继续么？";
	  var checkids="";
	  if (num>1){
	  for(i=0;i<num;i++){
	...
	if (checkids=="") {
	 ...
	 if(confirm(str)){
	   document.pageForm.action="/sysUserGroupMemberAction.do?method=deletemember";
...
function delUserAll(){
  if(num>0){
	  var str="此操作将给右侧所有用户从当前用户组中删除，真的要继续么？";
	  if(confirm(str)){
	    document.pageForm.action="/sysUserGroupMemberAction.do?method=deletememberall";
	    document.pageForm.submit();
	  }
  }
}
...
function searchRecord(){
  ...
  document.pageForm.action="/sysUserGroupMemberAction.do?method=inmember";
  document.pageForm.submit();
}
</script>
...
<TABLE class="page_maintable">
  ...
  <TR>
    <TD class="page_title" style="font-size:14px;">用户组用户</TD>
  </TR>
       ...
       <TABLE width="100%" align=center border=0><!--serach end-->
              ...
              <TR>
                <TD vAlign=top borderColor=#ffffff align="right">
                   <java2page:pager url="sysUserGroupMemberAction.do?method=inmember" name="pagelist" style="2"/>
