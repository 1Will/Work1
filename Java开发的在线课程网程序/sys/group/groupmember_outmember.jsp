...
function doOnLoad(){
 	window.parent.frames["right"].location = "/sysUserGroupMemberAction.do?method=inmember&groupid=<bean:write name="groupid"/>";
}
function addUser(){
  if(num>0){
	  var str="此操作将把选中用户添加到当前用户组中，真的要继续么？";
	  var checkids="";
	  if (num>1){
	  for(i=0;i<num;i++){
	...
	if (checkids=="") {
	     alert("您还没有选择添加到用户组的用户呢!")
	}
	else{
	 if(confirm(str)){
	   document.pageForm.action="/sysUserGroupMemberAction.do?method=addmember";
...
function addUserAll(){
  if(num>0){
	  var str="此操作将把左侧所有用户添加到当前用户组中，真的要继续么？";
	  if(confirm(str)){
	    document.pageForm.action="/sysUserGroupMemberAction.do?method=addmemberall";
	    document.pageForm.submit();
	  }
  }
}
function searchRecord(){
  ...
  document.pageForm.action="/sysUserGroupMemberAction.do?method=outmember";
  document.pageForm.submit();
}
              ...
              <td class="bg_basecolor">
                <table cellpadding="0" cellspacing="0">
                  <tr>
                    ...
                    <td>
                    <select name="groupid1" onchange="selectGroup(this.value)">
                     <option value="">选择机构</option>
	                 <%
	                 List lst=(List)request.getAttribute("list");
	                 String groupid1=(String)request.getAttribute("groupid1");
			         ...
			         for(int i=0;i<lst.size();i++) {
	                   ...
	                   if(len < 0) len = 0;
	                 %>
	                    <option value="<%=group.getGroupid()%>" <%if(groupid1.equals(group.getGroupid().toString())){ %>selected="selected"<%} %>><%for(int j=0; j<len; j++){%>&nbsp;├<%}%><%=group.getGroupname()%></option>
	                  <%}%>
	                </select>
                    </td>
       ...
       <TABLE width="100%" align=center border=0><!--serach end-->
              ...
              <TR>
                <TD vAlign=top borderColor=#ffffff align="center">
                   <java2page:pager url="sysUserGroupMemberAction.do?method=outmember" style="2" name="pagelist"/>
