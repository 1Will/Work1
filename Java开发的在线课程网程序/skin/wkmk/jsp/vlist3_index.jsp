...
<div class="clearfix">
  ...
  <div class="list clearfix">
      ...
      <div class="slide fl">         
        ...
        <div class="Scroller-Container">
          <div class="slideContent tree-list">
            <h3>
              <li id="ta1"><a href="javascript:gotoUrl('${xueduan }', '${cxueduan }', '${subject }', '${grade }', '${version }', '${orderbytype }')" class="link">目录</a></li>
            </h3>
            <div class="tw_mf_tree float1" id="co1">
              <ul style="-moz-user-select: none;" class="ztree">
                ...
                List list1 = null;
                List list2 = null;
                EduColumnInfo eci = null;
                int temp = 0;
                for(int i=0, size=parentList.size(); i<size; i++){
                	eci = (EduColumnInfo)parentList.get(i);
                	temp++;
                %>
                <li>
                  <a href="javascript:gotoLink('<%=temp %>', '<%=eci.getColumnid() %>')"><span class="leave" id="k_li_<%=temp %>" <%if(eci.getColumnid().toString().equals(column)){ %>style="#40a8ff"<%} %>><%=eci.getColumnname() %></span></a>
                  <%
                  list1 = (List)map1.get(eci.getColumnno());
                  if(list1 != null && list1.size() > 0){
                  %>
                  <ul class="ztree_c">
                    <%
                    for(int m=0, n=list1.size(); m<n; m++){
                    	eci = (EduColumnInfo)list1.get(m);
                    	temp++;
                    %>
                    <li>
                      <a href="javascript:gotoLink('<%=temp %>', '<%=eci.getColumnid() %>')" class="level1" id="k_li_<%=temp %>" <%if(eci.getColumnid().toString().equals(column)){ %>style="#40a8ff"<%} %>><span class="font_3"><%=eci.getColumnname() %></span></a>
                      <%
                      list2 = (List)map2.get(eci.getColumnno());
                      if(list2 != null && list2.size() > 0){
                      %>
                      <ul class="ztree_t">
                        <%
	                    for(int x=0, y=list2.size(); x<y; x++){
	                    	eci = (EduColumnInfo)list2.get(x);
	                    	temp++;
	                    %>
                        <li><a href="javascript:gotoLink('<%=temp %>', '<%=eci.getColumnid() %>')" class="level1" id="k_li_<%=temp %>" <%if(eci.getColumnid().toString().equals(column)){ %>style="#40a8ff"<%} %>><span><%=eci.getColumnname() %></span></a></li>
                        <%} %>
                      </ul>
                      <%} %>
                    </li>
    ...
    <iframe id="listFrame" name="listFrame" src="/v.bo?method=vlist3f&version=${version }&column=${column }&orderbytype=${orderbytype }" width="900" height="70" onload="SetCwinHeight()" frameborder="0" scrolling=no></iframe>
  </div>
</div>
</div>
</form>
<script type="text/javascript">
function gotoLink(id, columnid){
	var count = <%=temp %>;
	...
	document.getElementById('listFrame').src = '/v.bo?method=vlist3f&version=${version }&orderbytype=${orderbytype }&column='+columnid;
}
</script>
