...
nph.Gallery = function (options) { 
	var t = this,
		sn = options.sn || '',
		ids = ['gallery', 'galleryTpl', 'galleryRelat', 'modePhoto', 'modeStream', 'modeSearch', 'setInfo', 'photoType', 'streamType', 'viewOrig', 'viewStream', 'viewPhoto', 'scrl', 'bar', 'thumb', 'scrlPrev', 'scrlNext', 'photo', 'photoIndex', 'photoDescSpan','photoDesc', 'photoView', 'photoPrev', 'photoNext', 'photoLoading', 'stream', 'search' ,'searchInput', 'searchPhoto', 'searchTheme', 'prevSet', 'nextSet', 'photoList', 'photoLayout', 'photoPrevLoading'],
		i = ids.length;
	...
	t.$photoPrevLoading.addEvent('load', function(){
		...
		if(oHash.p){
			t.photoInfo[t.photoIndex[oHash.p]].img;
		...
		if(info.img != t.$photo.src){
			...
			setTimeout(function(){
				if(!isPhotoImageLoaded){
					t.$photo.src = "/public/images/photo/bg06.png";
					t.$photoLoading.removeCss('hidden');
				}
			}, 200);
			t.$photoIndex.attr('innerHTML', index + 1);
			t.$photoPrevLoading.src = info.aimg ? info.aimg : info.img;
			t.$photoDesc.attr('innerHTML', String.format('<h2>%1</h2>', info.title) + (info.desc && String.format('<p>%1</p>', info.desc)));photoDescSpan
			t.$photoDescSpan.attr('innerHTML',info.desc);
