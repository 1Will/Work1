...
function setLocation(url,cur,total){
	...
	if(url != ''){
		if(url.indexOf("beiKeAction") >= 0 || url.indexOf("jyResInfoAction.do?method=beforeAdd") >= 0){
			...
			document.getElementById("right_center").style.width = "788px";
			document.getElementById("right_center").style.padding = "10px";
...
<div id="left">
<h3><a href="/pcenter.do?method=ucenter" target="_top"><img src="/public/images/pcenter/c01.jpg"/></a></h3>
...
<p style="height:10px;line-height:10px;">&nbsp;</p>
	...
	%>
      <div <logic:notEqual value="0" name="isAdmin" scope="session"><%if(i > 0){ %>class="collapsed"<%} %></logic:notEqual>>
