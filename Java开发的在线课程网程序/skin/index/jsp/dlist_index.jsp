...
<title>${keywords }_文档_微课慕课网-分享资源，传递价值</title>
<link rel="stylesheet" href="/skin/index/css/file.css" type="text/css"  />
...
<div class="nav">
	<div class="nav_size">
		<a href="/search.action?id=vod&xueduan=${xueduan }&cxueduan=${cxueduan }&subject=${subject }&grade=${grade }&knopoint=${knopoint }&startcount=0&searchButton=0&searchtools=1&orderby=1&keywords=${keywords }"><span>微课</span></a>
    	<span class="nav_size_font">文档</span>
...
<div class="content">
    ...
    <div class="subnav" id="neirong1">
    	<a href="javascript:searchOrderby('1')"><span class="subnav_span1<logic:equal value="1" name="orderby"> hover</logic:equal>">最新</span></a>
   		...
   		<a href="javascript:searchOrderby('5')"><span class="subnav_span2<logic:equal value="5" name="orderby"> hover</logic:equal>">下载量</span></a>
   		<a href="javascript:searchOrderby('6')"><span class="subnav_span1<logic:equal value="6" name="orderby"> hover</logic:equal>">页数</span></a>
    ...
    <div class="content_left">
    	...
    	for(int i=0, size=datalist.size(); i<size; i++){
    		...
    		author = object[3].toString();
    		unitname = object[14].toString();
    	...
    	<div class="content_left_module"  onMouseOut="out();" onMouseMove="move();">
        	<div class="content_left_module_title">
            	<a href="/v-doc-<%=object[13] %>-<%=object[0] %>.htm" target="_blank"><p><%=title %></p></a>
            </div>
            <div class="content_left_module_main">
            	<a href="/v-doc-<%=object[13] %>-<%=object[0] %>.htm" target="_blank"><img src="<%=object[4] %>" class="content_left_module_main_img" width="70" height="90"/></a>
                <div class="content_left_module_main_font">
                	<a href="/v-user-<%=object[13] %>-<%=object[2] %>.htm" target="_blank"><span class="content_left_module_main_font_span"><%=object[3] %></span></a>
                    <a href="/html/<%=object[13] %>.htm" target="_blank"><span><%=unitname %></span></a>
                    <br />
                    <span>浏览次数:<%=object[8] %></span>&nbsp;
                    <span>收藏次数：<%=object[7] %></span> &nbsp;
                    <span>下载次数：<%=object[9] %></span>  &nbsp;
                    <span>页数：<%=object[10] %></span>  &nbsp;
                    <span>评分:<%=object[5] %>（<%=object[6] %>人评）</span>&nbsp;
                    <br />
                    <img src="/skin/index/images/fileext/<%=object[12].toString().toLowerCase() %>.png" class="content_left_module_main_font_img" />
                    <%
                    if(object[16] != null && object[16].toString().length() > 0){
                    	objKeywords = object[16].toString().split(" ");
    	...
    	<div class="content_right_module">
            ...
            <div class="content_right_module_main">
            	...
            	for(int i=0, size=aboutVodList.size(); i<size; i++){
            		vfi = (VodFilmInfo)aboutVodList.get(i);
            	%>
            	<div class="content_right_module_main_<%=i+1 %>">
                	<a href="/v-play-<%=vfi.getUnitid() %>-<%=vfi.getFilmid() %>.htm" target="_blank"><%if(vfi.getSketch().startsWith("http://")){ %><img src="<%=vfi.getSketch() %>" width="118" height="70" onerror="this.src='/upload_dir/xueda.jpg'"/><%}else{ %><img src="/upload_dir/<%=vfi.getSketch() %>" width="118" height="70" onerror="this.src='/upload_dir/xueda.jpg'"/><%} %></a>
                    <a href="/v-play-<%=vfi.getUnitid() %>-<%=vfi.getFilmid() %>.htm" target="_blank"><img src="/skin/index/images/right_icon1.png" class="content_right_module_main_1_img<%=i+1 %>" /></a>
                	<a href="/v-play-<%=vfi.getUnitid() %>-<%=vfi.getFilmid() %>.htm" target="_blank" title="<%=vfi.getTitle() %>"><p><%=SubStringDirectiveModel.subString(vfi.getTitle(), 16, "...") %></p></a>
                </div>
                <%} %>
            </div>
        </div>
