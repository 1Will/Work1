...
public class StringUtil {
	 ...
	 *            参数Map
	 */
	public static <E> List<Map<String, Object>> listToListMap(String paramName,
			List<E> list) {
		if (list == null) {
			return null;
		}
	 ...
	 * 使用方法：
	 * 	String regex = "\\$\\{(.+?)\\}"; 
        String str = "<a onclick=\"showUserName('${CRECODE}','${session.user.password}');\" >linkme</a>" ;
	 ...
	 * @return ret 結果
	 */
	public static String getInConditionFromList(List<String> lst) {
		String ret = "";
		for (String str : lst) {
			if (isNotEmpty(ret)) {
				ret += "," + str;
