	...
	protected int sendMessage(int size, String pdu) throws Exception
	{
		...
		while (true)
		{
			...
			while (!serialDriver.dataAvailable())
			{
				...
				log.info("CATHandler_SonyEricsson().SendMessage(): Still waiting for response (I) (" + responseRetries + ")...");
				Thread.sleep(5000);
			}
			responseRetries = 0;
			...
			while (response.length() == 0)
			{
				responseRetries++;
				if (responseRetries == 4) throw new NoResponseException();
				log.info("CATHandler_SonyEricsson().SendMessage(): Still waiting for response (II) (" + responseRetries + ")...");
				response = serialDriver.getResponse();
			}
			...
			else if (response.indexOf("CMS ERROR:") >= 0)
			{
				...
				if (errorRetries == 4)
				{
					log.error("GSM CMS Errors: Quit retrying, message lost...");
					refNo = -1;
					break;
				}
				else log.error("GSM CMS Errors: Possible collision, retrying...");
			}
			else refNo = -1;
		}
		return refNo;
