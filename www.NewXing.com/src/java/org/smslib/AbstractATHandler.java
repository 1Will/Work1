...
abstract public class AbstractATHandler
{
	...
	protected String memoryLocations = "";
	...
	public AbstractATHandler(CSerialDriver serialDriver, Logger log, CService srv)
	{
		...
		this.log = log;
		this.srv = srv;
		memoryLocations = "MTSMMESR";
	}

	abstract protected String getMemoryLocations();
	...
	abstract protected int sendMessage(int size, String pdu) throws Exception;

	abstract protected String listMessages(CIncomingMessage.MessageClass messageClass) throws Exception;
		...
		for (int i = 0; i < 3; ++i)
		{
			try
			{
				if (handlerClassNames[i] != null)
				{
					...
					java.lang.reflect.Constructor handlerConstructor = handlerClass.getConstructor(new Class[] { CSerialDriver.class, Logger.class, CService.class });
					atHandler = (AbstractATHandler) handlerConstructor.newInstance(new Object[] { serialDriver, log, srv });
