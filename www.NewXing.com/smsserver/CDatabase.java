...
class CDatabase
{
	...
	public void saveMessage(CIncomingMessage message) throws Exception
	{
		...
		{
			case Incoming:
				sqlCmd.executeUpdate("insert into sms_in (type, originator, message_date, text) values ('I', '" + message.getOriginator() + "', " + escapeDate(message.getDate(), true) + ", '" + message.getText().replace("'", "''") + "')");
				break;
			case StatusReport:
				CStatusReportMessage msg = (CStatusReportMessage) message;
				sqlCmd.executeUpdate("insert into sms_in (type, originator, message_date, text, ref_no, original_sent_date, date_received) values ('T', '" + msg.getOriginator() + "', " + escapeDate(new java.util.Date(), true) + ", '" + msg.getText().replace("'", "''") + "', " + msg.getRefNo() + ", " + escapeDate(msg.getDateOriginal(), true) + ", " + escapeDate(msg.getDateReceived(), true) + ")");
				break;
		}
		connection.commit();
		sqlCmd.close();
	...
	public void saveSentMessage(COutgoingMessage message) throws Exception
	{
		...
		{
			sqlCmd = connection.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
			sqlCmd.executeUpdate("insert into sms_out (recipient, text, dispatch_date, flash_sms, status_report, src_port, dst_port, validity_period) values ('" + message.getRecipient() + "', '" + message.getText().replace("'", "''") + "', " + escapeDate(message.getDate(), true) + ", " + (message.getFlashSms() ? 1 : 0) + ", " + (message.getStatusReport() ? 1 : 0) + ", " + message.getSourcePort() + ", " + message.getDestinationPort() + ", " + message.getValidityPeriod() + ")");
			connection.commit();
			sqlCmd.close();
		}
	}
	...
	public void checkForOutgoingMessages() throws Exception
	{
		...
		ResultSet rs;
		List<COutgoingMessage> messageList = new LinkedList<COutgoingMessage>();
