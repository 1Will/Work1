...
public boolean deliveryCountIsOrNot(int number,long id,long iId) {
	  ProductList productList=loadProductList(id);
	  String hql = "select i from Invoices as i where i.id=" + iId;
	  List<Invoices> codeList = getSession().createQuery(hql).list();
	  int count =0;
	  int count_now=0;
	  if(codeList!=null&&codeList.size()>0){
		  Invoices invoices=codeList.get(0);
		  count_now-=invoices.getDeliveryCount();
	  }
      if (null!= productList) {
    	  count+=productList.getCount();
    	  count_now+=productList.getDeliveryedCount();
    	  count_now+=number;
      }
      if(count<count_now){
    	  return false;
