...
public class ListCustomerInfoAction extends ValueListAction {
	...
	public ListCustomerInfoAction(CustomerInfoManager customerInfoManager, CodeValueManager codeValueManager,
			...
			GroupManager groupManager,DepartmentManager departmentManager) {
	...
	protected Map getRequestParameterMap() {
		...
		if("-1".equals(map.get("classification.id"))){
			map.remove("classification.id");
		}
		if("-1".equals(map.get("businessType.id"))){
			map.remove("businessType.id");
		}
		return map;
	}
