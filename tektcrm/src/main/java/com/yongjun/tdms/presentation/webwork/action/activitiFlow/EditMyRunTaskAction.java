...
public class EditMyRunTaskAction extends PrepareAction {
	...
	public String save() {
		...
		boolean isSubmit;//是否是提交
		
		if("1".equals(request.getParameter("isSaved"))){
			isSubmit = true;
		}else{
			isSubmit = false;
		}
		...
		if(!isBatch){
			if(!isSubmit){
				addActionMessage(getText("审批任务保存成功！"));
				//保存返回编辑页面
				return "new";
			}else{
				return "success";
	...
	private ActiviFlow approvalRuntask(RunTask eachRunTask) throws DaoException {
		...
		String[] key = { "bussnessId", "flow.id" ,"myNum"};
		String[] value = { eachRunTask.getBussnessId() + "", eachRunTask.getFlow().getId()+"",eachRunTask.getMyNum()+""};
