...
public class EditOnTheRoadBillAction extends PrepareAction {
	...
	public EditOnTheRoadBillAction(OnTheRoadBillManager onTheRoadBillManager, CodeValueManager codeValueManager,
			...
			EventTypeManager eventTypeManager, GroupManager groupManager,FlowManager flowManager,CopySendPersonManager copySendPersonManager) {
	...
	public String save() throws Exception {
		...
		if (!StringUtils.isEmpty(this.request.getParameter("flow.id"))) {
			this.onTheRoadBill.setFlow(this.flowManager.loadFlow(Long.valueOf(this.request
		...
		try {
			...
			if(isNew){
				Set<User> users = groupManager.getGroupByGroupName("出差通知组").getUsers();
				if(users!=null && users.size()>0){
					for(User user:users){
						CopySendPerson c=new CopySendPerson();
						c.setBussnessId(this.onTheRoadBill.getId());
						c.setFlow(this.onTheRoadBill.getFlow());
	...
	public List<Flow> getAllFlows() {
		List flowList = null;
		try {
			CodeValue codeValue =codeValueManager.loadByKey("code","t_onTheRoadBill").get(0);
