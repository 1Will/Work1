...
public class EditReturnPlanAction extends PrepareAction {
	...
	public void prepare() throws Exception {
		...
		if (hasId("contractManagement.id")) {
			...
			if (null != this.contractManagement) {
				...
				List<ReturnPlan> rPlans = returnPlanManager.loadByKey("contractManagement.id", getId("contractManagement.id"));
				double sum = 0D;
				if (rPlans != null && rPlans.size()>0) {
					for (int i = 0; i < rPlans.size(); i++) {
						sum += rPlans.get(i).getSum();
					}
				}
				this.returnPlan.setSum(this.contractManagement.getContractMoney() - sum);
			}
		}
	}
	...
	public String save() throws Exception {
		...
		if (isNew) {
			this.returnPlan.setIsExport("2");
		}else {
			if(!"2".equals(returnPlan.getIsExport())){
				this.returnPlan.setIsExport("1");
			}
		}
		if (hasId("payment.id")) {
		...
		if ((null != contractManagement) && (sum.doubleValue() == contractManagement.getContractMoney())) {
			this.contractManagement.setOverReturnPlan("yes");
		}
