...
public class ListContactArchivesAction extends ValueListAction {
	...
	public ListContactArchivesAction(ContactArchivesManager contactArchivesManager, CodeValueManager codeValueManager,
			...
			ProjectInfoContractManager projectInfoContractManager,GroupManager groupManager,CustomerInfoManager customerInfoManager) {
	...
	public String delete() {
		...
		// 执行删除方法后，增加cookie标记，用于判断是否上层frame
		Cookie deleteFlagCookie = new Cookie("deleteBack", "1");
		response.addCookie(deleteFlagCookie);
		...
		try {
			...
			if ("remove".equals(request.getParameter("remove"))) {
				for (ContactArchives contactArchives : cas) {
					...
					if("1".equals(contactArchives.getIsKeyContact())){
						CustomerInfo customerInfo = customerInfoManager.loadByKey("keyContacter",contactArchives.getName()).get(0);
						customerInfo.setKeyContacter("");
			...
			for (int i = 0; i < this.cas.size(); i++) {
				...
				if("1".equals(this.cas.get(i).getIsKeyContact())){
					List<CustomerInfo> customerInfos = customerInfoManager.loadByKey("keyContacter", this.cas.get(i).getName());
