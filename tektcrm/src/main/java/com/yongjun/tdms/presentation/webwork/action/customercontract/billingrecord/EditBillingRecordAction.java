...
public class EditBillingRecordAction extends PrepareAction {
	...
	public void prepare() throws Exception {
		if (hasId("billingRecord.id")) {
			...
			tempSum = this.billingRecord.getSum();
	...
	public String save() throws Exception {
		...
		if (isNew) {
			this.billingRecord.setIsExport("2");
		}else {
			if(!"2".equals(billingRecord.getIsExport())){
				this.billingRecord.setIsExport("1");
			}
		}
		...
		try {
			...
			//保存开票记录之后，更新合同的已开票金额------
			Double hasBillSum = this.contractManagement.getHasBillSum();
			if(isNew){
				this.contractManagement.setHasBillSum(hasBillSum+this.billingRecord.getSum());
			}else{
				this.contractManagement.setHasBillSum(hasBillSum-tempSum+this.billingRecord.getSum());
			}
