...
public class ListBackVisitAction extends ValueListAction {
	...
	protected Map getRequestParameterMap() {
		...
		backVisitList.add(b);
		try {
			List<PersonnelFiles> tempList = this.personnelFilesManager.loadByKey("code",
				...
				if (isProjectInfoGroup()) {
					if (personnelFiles.getBusinessType() != null) {
						if (personnelFiles.getBusinessType().getName().equals("军品")
							...
							List<BackVisit> bvts2 = backVisitManager.loadByKey("projectInfo.customer.businessType.id",
									personnelFiles.getBusinessType().getId());
					...
					List<Long> proIdList = this.projectInfoPersonnelsManager
							.loadProjectInfoIdByPersonnel(this.userManager.getUser().getCode());
					if (proIdList != null && proIdList.size() > 0) {
						List<BackVisit> bvts4 = backVisitManager.loadBackVisitByPj(
								".id in " + proIdList.toString().replace("[", "(").replace("]", ")"));
		...
		if (hasId("month")) {
			...
			if (request.getParameter("personnelFiles.id") != null
				...
				if(personnelFiles!=null&&personnelFiles.getId()!=null){
					map.put("personnelFiles.id", personnelFiles.getId());
					map.remove("user.id");
				}
