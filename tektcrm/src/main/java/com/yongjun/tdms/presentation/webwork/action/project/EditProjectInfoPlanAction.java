...
public class EditProjectInfoPlanAction extends PrepareAction {
	...
	private String productionOperationDetailId;
	...
	public EditProjectInfoPlanAction(CodeValueManager codeValueManager, ProjectInfoManager projectInfoManager,
			...
			PersonnelFilesToUserManager personnelFilesToUserManager,ProductionOperationDetailManager productionOperationDetailManager) {
	...
	public void prepare() throws Exception {
		...
		if (hasId("productionOperationDetail.id")) {
			this.productionOperationDetailId = getId("productionOperationDetail.id") + "";
			this.productionOperationDetail =productionOperationDetailManager.loadProductionOperationDetail(getId("productionOperationDetail.id"));
			this.title = "生产经营计划节点信息维护";
		}
		...
		if (this.projectInfoPlan == null)
			if (hasId("projectInfoPlan.id")) {
				...
				if (!hasId("productionOperationDetail.id")) {
					if(this.projectInfoPlan .getProductionOperationDetail()!=null){
						this.productionOperationDetailId = this.projectInfoPlan .getProductionOperationDetail().getId() + "";
						this.productionOperationDetail =this.projectInfoPlan .getProductionOperationDetail();
						this.title = "生产经营计划节点信息维护";	
					}
				}
	...
	public String save() {
		...
		if (hasId("productionOperationDetail.id")) {
			if (hasId("productionPoint.id")) {
				CodeValue cv = this.codeValueManager.loadCodeValue(getId("productionPoint.id"));
				projectInfoPlan.setName(cv.getName());
				projectInfoPlan.setProductionPoint(cv);
			}
			this.projectInfoPlan.setProductionOperationDetail(this.productionOperationDetail);
			if(isNew){
				map_1.put("productionOperationDetailId", getId("productionOperationDetail.id"));
				orderNumber= projectInfoPlanManager.loadMaxOrderNumber(map_1);
				projectInfoPlan.setOrderNumber(orderNumber);
				this.projectInfoPlan.setIsExport("2");
			}else {
				if(!"2".equals(projectInfoPlan.getIsExport())){
					this.projectInfoPlan.setIsExport("1");
				}
			}
		}
		if (hasId("contractManagement.id")) {
			this.projectInfoPlan.setContractManagement(this.contractManagement);
			if(isNew){
		...
		if ("1".equals(this.request.getParameter("projectInfoPlan.isSaved"))) {
			try {
				...
				if (this.projectInfoPlan.getProjectInfo() != null) {
					...
					if (this.projectInfoPlan.getPercentt() == 0) {
						...
						map.put("name",
								new SimpleDateFormat("yyyy-MM-dd").format(this.projectInfoPlan.getCreatedTime()) + ","
										+ this.projectInfoPlan.getCreator() + "提交了项目计划:"
										+ this.projectInfoPlan.getName());
					} else {
						List<EventType> eventTypes = this.eventTypeManager.loadByKey("code", "10023");
						if (eventTypes != null && eventTypes.size() > 0) {
							eventType = eventTypes.get(0);
						} else {
							logger.info("eventType不存在！");
				...
				}else if (this.projectInfoPlan.getContractManagement() != null) {// 合同工作计划提交
					List<User> users = this.userManager.loadByKey("code", this.projectInfoPlan.getPersonnelFiles()
							.getCode());
					String pids = "";
					if (users != null && users.size() > 0) {
				...
				else if (this.projectInfoPlan.getProductionOperationDetail() != null) {// 生产经营计划提交
					List<User> users = this.userManager.loadByKey("code", this.projectInfoPlan.getPersonnelFiles()
							.getCode());
//					if (users != null && users.size() > 0) {
//						pids = this.personnelFilesToUserManager.loadUserIdToStrByProjectInfoId(this.projectInfoPlan
...
//					}
					...
					if (this.projectInfoPlan.getPercentt() == 0) {
						...
						map.put("name",
								new SimpleDateFormat("yyyy-MM-dd").format(this.projectInfoPlan.getCreatedTime()) + ","
										+ this.projectInfoPlan.getCreator() + "提交了生产经营计划:"
										+ this.projectInfoPlan.getName());
					} else {
						List<EventType> eventTypes = this.eventTypeManager.loadByKey("code", "10023");
						if (eventTypes != null && eventTypes.size() > 0) {
						...
						if (this.projectInfoPlan.getPercentt() == 100) {
							map.put("name",
									new SimpleDateFormat("yyyy-MM-dd").format(this.projectInfoPlan.getCreatedTime())
											+ "," + this.projectInfoPlan.getCreator() + "完成了生产经营计划:"
											+ this.projectInfoPlan.getName());
						} else {
							map.put("name",
									new SimpleDateFormat("yyyy-MM-dd").format(this.projectInfoPlan.getCreatedTime())
											+ "," + this.projectInfoPlan.getCreator() + "更新了生产经营计划:"
											+ this.projectInfoPlan.getName());
						}
					}
	...
	public String getProductionOperationDetailId() {
		return productionOperationDetailId;
	}

	public void setProductionOperationDetailId(String productionOperationDetailId) {
		this.productionOperationDetailId = productionOperationDetailId;
	}
