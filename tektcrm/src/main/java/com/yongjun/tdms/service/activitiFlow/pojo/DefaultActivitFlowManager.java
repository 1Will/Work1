...
//				long 	runPointId =Long.parseLong(map.get("runPointId")+"");
				...
				long historyTaskinstId = (long)11; // 测试获取
				...
				runPoint.setInspectTime(new Date());
				 ...
				 //根据业务id 和审批人的code获取这个人的历史审批节点  
      			 ...
      			 if(historyTaskinsts!=null&&historyTaskinsts.size()>0){
      				...
      				historyTaskinstId=LastHistoryTaskinst.getId();
				...
				if(activiFlow.getResult()!=null&&activiFlow.getResult().getCode().equals("21403")){
		    		...
		    		this.runTaskDao.storeBussnessState(startActiviti);
		  			 ...
		  			 ///
		  			 //插入 拒绝事件
		  			storeFailActivitiEvent(runTask,runPointId,historyTaskinstId);
        			...
        			//拼接 查询删除的预节点以后的所有审批预节点
        			 String hql ="from RunPoint r where r.myNum >"+runPoint.getMyNum();
    					...
    					newRunPoint.setMyNum(runPoint.getMyNum()+1);
            			 ...
            			 newRunPoint.setSubmitTime(runPoint.getInspectTime());
        			    ...
        			    RunTask nextRunTask = new RunTask();
    	       			...
    	       			nextRunTask.setLinkHref(runTask.getLinkHref());
    	       			nextRunTask.setLastAssignPer(runPoint.getInspectPser());//设置当前审批节点的审批人为下一个审批任务的上一个审批人
    	       			nextRunTask.setAssignTime(runPoint.getInspectTime());//设置当前审批节点的审批时间为下一个审批任务的上一次审批时间
    	       			...
    	       			historyTaskinst.setLinkHref(runTask.getLinkHref());
    	       			historyTaskinst.setLastAssignPer(runPoint.getInspectPser());//设置当前审批节点的审批人为下一个审批任务的上一个审批人
    	       			historyTaskinst.setAssignTime(runPoint.getInspectTime());//设置当前审批任务的审批时间为下一个审批任务的上一次审批时间
    	       			this.historyTaskinstDao.storeHistoryTaskinst(historyTaskinst);
    	       			historyTaskinstId=historyTaskinst.getId();	
    	       			 //插入事件  通知下一位审批
    	       			 storeNextActivitiEvent(nextRunTask,runPointId,historyTaskinstId );
					...
					if(runPoints!=null&&runPoints.size()>0){
						RunPoint nextRunPoint =runPoints.get(0);
	        			 ...
	        			 nextRunPoint.setSubmitTime(runPoint.getInspectTime());
	        			 this.runPointDao.storeRunPoint(nextRunPoint);
	       			...
	       			nextRunTask.setLinkHref(runTask.getLinkHref());
	       			nextRunTask.setLastAssignPer(runPoint.getInspectPser());//设置当前审批节点的审批人为下一个审批任务的上一个审批人
	       			nextRunTask.setAssignTime(runPoint.getInspectTime());//设置当前审批任务的审批时间为下一个审批任务的上一次审批时间
	       			 this.runTaskDao.storeRunTask(nextRunTask);//分配下一个人审批任务
        			 ...
        			 storeNextActivitiEvent(nextRunTask,runPointId,historyTaskinstId);	
					    ...
					    StartActiviti startActiviti = new StartActiviti();
			  			 ...
			  			 //查询事件
			  			 storeSuccessActivitiEvent(runTask,runPointId,historyTaskinstId);
		  ...
		  private String remark;//审核意见*/
			        	 ...
			        	 if(sa.getIsSaved()!=null && !sa.getIsSaved().equals("0")){
			        		 ...
			        		 String [] key={"myNum","bussnessId","flow.id"}; 
			    	         String [] value={myNum_str,sa.getBussnessId()+"",sa.getFlow().getId()+""}; 
			        		 ...
			        		 if(runPoints!=null&&runPoints.size()>0){
			        			 ...
			        			 runPoint.setSubmitTime(new Date());
			        			 this.runPointDao.storeRunPoint(runPoint);
...
//			        			 runTask.setContent(sb.toString());
					...
					private PersonnelFiles assignee;//任务办理人
			        			 ...
			        			 //插入事件  通知下一位审批
			        			 storeNextActivitiEvent(runTask,runPoint.getId(),historyTaskinst.getId());
...
//			}
			...
			//通知下一位审批
			public void storeNextActivitiEvent(RunTask runTask,Long runPointId,Long historyTaskinstId){
			...
			//审批拒绝通知提交人
			public void storeFailActivitiEvent(RunTask runTask,Long runPointId,Long historyTaskinstId){
			...
			//最终审批通过通知提交人及抄送人
			public void storeSuccessActivitiEvent(RunTask runTask,Long runPointId,Long historyTaskinstId){
