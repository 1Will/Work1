...
function getObjByName(name){
 ...
 //增加日期输入提示
/* function addDateTip(dateName){
	 var oSpan = document.createElement('span');
     oSpan.style = "color:red;";
	 oSpan.innerText = "(如：yyyy-MM-dd)";
	 var myDate = getObjByName(dateName);
	 myDate.parentNode.appendChild(oSpan);
 }*/
 //验证日期输入格式
/* function checkDateStyle(dateName,temp){
	 var dateText = getObjByName(dateName).value
	 //正则验证
	 var dateRegExp = new RegExp("^(?:(?!0000)[0-9]{4}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-8])|(?:0[13-9]|1[0-2])-(?:29|30)|(?:0[13578]|1[02])-31)|(?:[0-9]{2}(?:0[48]|[2468][048]|[13579][26])|(?:0[48]|[2468][048]|[13579][26])00)-02-29)$");
	 if(!dateRegExp.test(dateText)){
		alert(temp+"日期格式错误，正确格式为yyyy-MM-dd！");
		getObjByName(dateName).value = "";
		getObjByName(dateName).focus();
		return false;
	 }
	 return true;
 }*/
