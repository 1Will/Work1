	...
	<@ww.form action="'saveDaily'" namespace="'/workReport'" method="'post'">
	   ...
	   <#-- 
	    ...
	    --> 
		  ...
		  <td id='customerNum' style="wid"></td>
		  ...
		  <tr>
		  <@textarea id="daily.workContext" name="daily.workContext" anothername="workContext" label="${action.getText('daily.workContext')}" value="${daily.workContext?if_exists}" rows="4" />
	      <script language="JavaScript" type="text/JavaScript">
	      
			 var width=document.body.clientWidth/9;
						getObjByName("daily.workContext").cols =width;
		 ...
		 <tr>
		    <@textarea id="daily.questions" name="daily.questions" anothername="questions" label="${action.getText('收获/问题/建议')}" value="${daily.questions?if_exists}" rows="4" />
	     		<script>
	     		 var width=document.body.clientWidth/9;
						getObjByName("daily.questions").cols =width;
