...
<@htmlPage title="${action.getText('customerInfo.title')}">
	<@ww.form name="'listForm'" action="'searchCustomerInfo'" method="'post'">
        ...
        <@list title="${action.getText('customerInfo.list.title')}" 
            includeParameters="customerInfo.code|customerInfo.name|type.id|country.id|province.id|city.id|industry.id|companyNature.id|isPartner|customerInfo.keyContacter|customerInfo.salesman|step.id|customerInfo.state|readOnly|onlyInvalid|onlyValid|department|departmentName|businessType.id|classification.id|" 
        	fieldMap="like:customerInfo.code|customerInfo.name|customerInfo.keyContacter|customerInfo.salesman|departmentName|" >
        	<#if !(action.isReadOnly())>
            ...
            <@vcolumn title="${action.getText('customerInfo.businessType')}" property="businessType.name" sortable="desc">
     			<@alignLeft/>
            </@vcolumn>
            <@vcolumn title="${action.getText('customerInfo.classification')}" property="classification.name" sortable="desc">
     			<@alignLeft/>
            </@vcolumn>
            <@vcolumn title="${action.getText('customerInfo.parlorDept')}" property="department.name" sortable="desc">
     			<@alignLeft/>
            </@vcolumn>
            ...
            <@vcolumn title="${action.getText('customerInfo.customerInfoIntegrity')}" property="customerInfoIntegrity" sortable="desc">
             ...
             <@vcolumn title="${action.getText('合同数')}" property="contracSum" sortable="desc">
              <#if object.contracSum?exists>
              <a href="javascript: contractManagement_Dialog(#{object.id?if_exists});"">
                ${object.contracSum? if_exists}
                </a>
              <#else>  
                ${object.contracSum? if_exists}
              </#if>  
            ...
            <#if object.salesman?exists>
              <a href="javascript: salesman_Dialog(#{object.salesman.id?if_exists});"">
...
<script>
  ...
  var url =  "${req.contextPath}/personnelFile/editPersonnelFile.html?readOnly=${req.getParameter('readOnly')?if_exists}&personnelFile.id="+id+"&backvist=backvist";
	  popupModalDialog(url, 1100, 600);
	   ...
	   function contractManagement_Dialog(id){
  var url =  "${req.contextPath}/contractManagement/listContractManagementAction.html?customerInfo.id="+id+"&readOnly=${req.getParameter('readOnly')?if_exists}";
	  popupModalDialog(url, 1100, 600);
	  }
</script>
