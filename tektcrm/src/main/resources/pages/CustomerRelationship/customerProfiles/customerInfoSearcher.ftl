...
<@inputTable>
	...
	<!--部门列表-->
	<@ww.textfield label="'${action.getText('customerInfo.parlorDept')}'" name="'departmentName'" value="'${req.getParameter('departmentName')?if_exists}'" cssClass="'underline'" />
	</tr>
	<tr>
	<!--业务属性--><!--片区分类-->
...
var stateSelector=document.all("customerInfo.state");
    var stateGroups=stateSelector.options.length;
     for(i=0;i<stateGroups;i++){
      <#if req.getParameter('customerInfo.state')?exists>
        if(stateSelector.options[i].value=="${req.getParameter('customerInfo.state')?if_exists}"){
