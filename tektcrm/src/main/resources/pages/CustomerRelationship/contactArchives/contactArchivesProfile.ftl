...
<@ww.form namespace="'/customerRelationship'" name="'contactArchivesInfo'" action="'saveContactArchives'" method="'post'">
   ...
   <tr>
		...
		<td align="right" valign="top">
	        ...
	        <td colspan="5">
	        	<textarea name="contactArchives.enterpriseSynopsis" rows="4"  >${contactArchives.enterpriseSynopsis?if_exists}</textarea>
	        </td>
	        <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("contactArchives.enterpriseSynopsis").cols =width;
		   </script>
		<!---->
	</tr>
    ...
    <@buttonBar>
		...
		<#if popWindowFlag?exists&&popWindowFlag=='popWindowFlag'>
		<!-- 关闭按钮 -->
			 <#if backvist?exists && backvist!=''>
		    <@vsubmit name="'close'" value="'关闭'" onclick="'closeThis()'"/>
		    <#else>
			<@vbutton name="close" value="${action.getText('close')}" class="button" onclick="closeAndRefresh();"/>
			</#if>
  		<#else>
