...
<@htmlPage title="${action.getText('部门目标查询页面')}">
	<@ww.form name="'listForm'" action="'searchDepartmentTargetAction'" method="'post'">
        ...
        <@buttonBar>
			...
			<@redirectButton value="${action.getText('new')}" url="${req.contextPath}/departmentTarget/editDepartmentTargetAction.html?readOnly=${req.getParameter('readOnly')?if_exists}"/>
			</#if>
        </@buttonBar>
