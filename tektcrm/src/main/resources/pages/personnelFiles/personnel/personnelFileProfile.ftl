     ...
     <@ww.form action="'savePersonnelFile'" namespace="'/personnelFile'" method="'post'" >
       ...
       <@ww.hidden name="'backvist'" value="'${backvist?if_exists}'"/>
         ...
         <@inputTable>
        	...
        	<!---->
        	<@ww.select label="'${action.getText('businessType.name')}'" 
...
//	window.onload=function(){
        <#if uFiles?exists>
           ...
           <#if uFiles.businessType?exists>
	        sBusinessType=${uFiles.businessType.id?if_exists};
	        </#if>
...
<ul id="beautytab">
	...
	<li>
		<a id="qualification" onclick="activeTab(this);" href='${req.contextPath}/personnelFile/listQualificationTab.html?personnelFiles.id=${personnelFile.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('资格证书')}</a>
	</li>
	<li>
