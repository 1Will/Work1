...
<@ww.form namespace="'/projectInfo'" name="'projectInfo'" action="'saveProPlan'" method="'post'">
    ...
    <@inputTable>
    	...
    	<#if productionOperationDetailId?exists>
    		<@ww.hidden name="'productionOperationDetail.id'" value="'${productionOperationDetailId?if_exists}'"/>
    		<#assign returnUrl=returnUrl + '&productionOperationDetail.id=${productionOperationDetailId?if_exists}'/>
    	</#if>
	   ...
	   <#if projectInfoPlan.projectInfoPlan_2 ?exists>
    		<@ww.hidden name="'projectInfoPlan_2.id'" value="#{projectInfoPlan.projectInfoPlan_2.id?if_exists}"/>
       <#else>
    	    <@ww.hidden name="'projectInfoPlan_2.id'" value="''"/>
	   </#if>
  <tr>
         <td align="right" valign="top">
	       		<label class="label">${action.getText('前置任务')}:</label>
	     	</td>
	     	<td>
		   		<input type="text" name="projectInfoPlan_2.name" class="underline"  value="<#if projectInfoPlan.projectInfoPlan_2?exists>${projectInfoPlan.projectInfoPlan_2.name?if_exists}</#if>"  maxlength="70" size="60" disabled="true"/>
  ...
  <tr>
			...
			<#if productionOperationDetailId?exists>
		<@ww.select label="'${action.getText('projectInfoPlan.name')}'" 
			...
			<#if projectInfoPlan.productionPoint?exists>
	      	getObjByName('productionPoint.id').value='${projectInfoPlan.productionPoint.id?if_exists}';
	        </#if>
	        </script>
			<#else>
			<!--任务名称-->
		<td align="right" valign="top">
			...
			<td>
			<input type="text" name="projectInfoPlan.name" class="underline"  value="${projectInfoPlan.name?if_exists}" maxlength="70" size="60" />
			</td>
			</#if>
   ...
   <tr>
     ...
     <tr>
	        ...
	        <td colspan="10">
	        	<textarea name="projectInfoPlan.outline" rows="4"  >${projectInfoPlan.outline?if_exists}</textarea>
	        </td>
	         <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("projectInfoPlan.outline").cols =width;
		   </script>
		</tr>
    </@inputTable>
    <@buttonBar>
    	<#if !(action.isReadOnly())>
			...
			<@redirectButton value="${action.getText('newNext')}" 
				url="${returnUrl}"/>
				<#--
			<input class="button" type="button" value="${action.getText('保存计划模板')}"  onclick="savaTemplet()">
			-->
			<#else>
			<@redirectButton name="newNext" value="${action.getText('newNext')}" 
				url="${req.contextPath}/projectInfo/editProPlan.html"/>
				<#--
				<input class="button" type="button" value="${action.getText('保存计划模板')}" name="templet" >
				-->
			<script language="JavaScript" type="text/JavaScript"> 
			getObjByName("newNext").disabled="true";
			<#--
			getObjByName("templet").disabled="true";
				-->
			</script>
			</#if>
		</#if>
		<#if productionOperationDetailId?exists>
		<@vbutton class="button" value="${action.getText('提出异常')}" onclick="newProductionOperationException()"/>
		</#if>
...
//	}
	...
	function salesman_OpenDialog(){
	   ...
	   var url="";
	   <#if productionOperationDetailId?exists>
	    url =  "${req.contextPath}/personnelFile/listPersonByUser.html?readOnly=${req.getParameter('readOnly')?if_exists}";
	   <#else>
	    
	   <#if projectInfoId?exists>
		 	url =  "${req.contextPath}/personnelFile/listPersonByUser.html?readOnly=${req.getParameter('readOnly')?if_exists}&onlyProject.id=${projectInfoId?if_exists}";
    	</#if>
    	<#if contractManagement?exists>
    	<#if contractManagement.project?exists>
		 	 url =  "${req.contextPath}/personnelFile/listPersonByUser.html?readOnly=${req.getParameter('readOnly')?if_exists}&onlyProject.id=${contractManagement.project.id?if_exists}";
    	</#if>
    	</#if>
	   </#if>
	 ...
	 function projectInfoPlanFirst_OpenDialog(){
	   var projectInfoPlan_2Id=getObjByName("projectInfoPlan_2.id").value;
	   var url="";
	   <#if productionOperationDetailId?exists>
	  url= "${req.contextPath}/projectInfo/listProPlan.html?readOnly=${req.getParameter('readOnly')?if_exists}&productionOperationDetail.id=${productionOperationDetailId?if_exists}&backCheckBox=backCheckBox&projectInfoPlan.id=${projectInfoPlan.id?if_exists}&projectInfoPlan_2Id="+projectInfoPlan_2Id;
	   <#else>
	   	<#if projectInfoId?exists>
		 	 url =  "${req.contextPath}/projectInfo/listProPlan.html?readOnly=${req.getParameter('readOnly')?if_exists}&projectInfo.id=${projectInfoId?if_exists}&backCheckBox=backCheckBox&projectInfoPlan.id=${projectInfoPlan.id?if_exists}&projectInfoPlan_2Id="+projectInfoPlan_2Id;
    	</#if>
    	<#if contractManagement?exists>
    	<#if contractManagement.id?exists>
		 	 url =  "${req.contextPath}/projectInfo/listProPlan.html?readOnly=${req.getParameter('readOnly')?if_exists}&contractManagement.id=${contractManagement.id?if_exists}&backCheckBox=backCheckBox&projectInfoPlan.id=${projectInfoPlan.id?if_exists}&projectInfoPlan_2Id="+projectInfoPlan_2Id;
    	</#if>
    	</#if>
	   </#if>
	   ...
	   popupModalDialog(url, 1100, 600, creatorSelectorHandler_1);
	   //window.open(url);
	 }
	 	function creatorSelectorHandler_1(result) {
		if (null != result) {
			getObjByName("projectInfoPlan_2.id").value = result[0];
	   		getObjByName("projectInfoPlan_2.name").value = result[1];
		}
	}
	...
	function checkPercentt(){
		...
		if(percentt==100){
			...
			getObjByName('planState.id').value="544";//滞后
			}
		}
		return true;
	}
	...
	function storeValidation(){
		/* 验证责任人*/
		<#if productionOperationDetailId?exists>
		if('' == getObjByName('productionPoint.id').value){
		...
		var  percentt =getObjByName("projectInfoPlan.percentt").value;
		//当完成百分比为100的时候。就是自动更新状态为已完成
		if(percentt==100 && '' == endFact){
		   alert("计划已完成，请选择实际结束时间！");
		   return false;
		}
		if(percentt!='100' && ''!= endFact){
		   alert("计划已完成，请修改项目完成比为100！");
		   return false;
		}
		if('' != startFact){
	...
	function savaTemplet(){
	<#if projectInfoPlan.id?exists>
	   var url =  "${req.contextPath}/projectInfo/editProPlanTemplet.html?readOnly=${req.getParameter('readOnly')?if_exists}&projectInfoPlan.id=#{projectInfoPlan.id?if_exists}&backCheckBox=backCheckBox";
	 </#if>
	   popupModalDialog(url, 1100, 600);
	}
	//通过模板写计划
	 function projectInfoPlan_OpenDialog(){
		var url =  "${req.contextPath}/projectInfo/listProPlanTemplet.html?readOnly=${req.getParameter('readOnly')?if_exists}&backCheckBox=backCheckBox";
		popupModalDialog(url, 800, 600, projectInfoPlan_OpenDialog_);
	 }
	 //获得模态窗体返回值
	function projectInfoPlan_OpenDialog_(result) {
		if (null != result) {
			document.forms[0].elements["projectInfoPlan.name"].value = result[0];
	   		document.forms[0].elements["projectInfoPlan.priority"].value = result[1];
	   		document.forms[0].elements["projectInfoPlan.outline"].value = result[2];		 	
		}
	}
  ...
  //  window.open(url,'','width=850,height=600,top=30%, left=30%'); 
     //    window.opener=null; 
  		openNewWindow(url);
  ...
  function updatestartDate(){
    <#if productionOperationDetailId?exists>
    var ts=getObjByName('productionPoint.id');
    var text = ts.options[ts.selectedIndex].text; 
    if("交付节点"==text){
        var s='${productionOperationDetail.deliverDate?if_exists}';
        updatestartDate_now(s);
    }else if("厂检节点"==text){
        var s='${productionOperationDetail.testDate?if_exists}';
        if(s!=null &&　""!=s){
         updatestartDate_now(s);
        }
    }else if("制造节点"==text){
  	    var s='${productionOperationDetail.makeDate?if_exists}';
        updatestartDate_now(s);
    }else if("采购节点"==text){
        var s='${productionOperationDetail.purchaseDate?if_exists}';
        updatestartDate_now(s);
    }else if("技术资料节点"==text){
        var s='${productionOperationDetail.technologyDate?if_exists}';
        if(s!=null &&　""!=s){
         updatestartDate_now(s);
        }
    }else if("军检节点"==text){
        var s='${productionOperationDetail.jtestDate?if_exists}';
        if(s!=null &&　""!=s){
         updatestartDate_now(s);
        }
    }
  ...
  function updatestartDate_now(s){
        s = s.replace(/-/g,"/");
        var date =new Date(s);
        getObjByName("projectInfoPlan.startDate").value = date.format("yyyy-MM-dd");
         getObjByName("projectInfoPlan.endDate").value = date.format("yyyy-MM-dd");
