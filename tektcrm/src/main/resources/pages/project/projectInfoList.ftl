...
<@htmlPage title="${action.getText('projectInfo')}">
	<@ww.form name="'listForm'" namespace="'/projectInfo'" action="'searchProjectInfo'" method="'post'">
        ...
        includeParameters="code|name|customer.name|customer.id|contact.name|backVisitCheckBox|controller.name|state.id|readOnly|onlyInvalid|onlyValid|departmentName|month|personnelFiles.id" 
        fieldMap="like:code|name|customer.name|contact.name|controller.name|departmentName|month" >
	      ...
	      <#else>
            ...
            <@alignLeft/>
             ...
             <a href="javascript:customer_OpenDialog(#{object.customer.id?if_exists})" title="完整度：${object.customer.customerInfoIntegrity?if_exists}%; 客户状态：<#if object.customer.customerType?exists>${object.customer.customerType.name?if_exists}</#if>">${object.customer.name?if_exists}</a>
            <@alignLeft/>
            </@vcolumn>
            ...
            <a href="javascript:controller_OpenDialog(<#if (object.controller?exists)>#{object.controller.id?if_exists}</#if>)"><#if (object.controller?exists)>${object.controller.name}</#if></a>
            <@alignLeft/>
            </@vcolumn>
             <@vcolumn title="${action.getText('projectInfo.controllerDept')}" property="department.name" sortable="desc">
            <@alignLeft/>
            </@vcolumn>
            ...
            <@vcolumn title="${action.getText('合同数')}" property="backVisitSum" sortable="desc">
            	<a href="javascript:contractManagement_OpenDialog(#{object.id?if_exists})" >${object.contractManagementSum?if_exists}</a>
            <@alignRight attributes="width:50;"/>
            </@vcolumn>
            <@vcolumn title="${action.getText('合同总金额')}" property="contractManagementMoney" sortable="desc">
            <@alignRight attributes="width:50;"/>
            </@vcolumn>
