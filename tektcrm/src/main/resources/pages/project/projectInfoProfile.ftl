...
<@ww.form namespace="'/projectInfo'" name="'projectInfo'" action="'saveProjectInfo'" method="'post'">
    ...
    <@inputTable>
    	...
    	<#if projectInfo.controller?exists>
	     	<input type="hidden" name="controller.id"  value="${projectInfo.controller.id?if_exists}" maxlength="140" size="20" disabled="true"/>
	     	<#else>
	     	<input type="hidden" name="controller.id"  value="<#if personnelFiles?exists>${personnelFiles.id?if_exists}</#if>"/>
	     	</#if>
    ...
    <tr>
	      ...
	      <td align="right" valign="top">
		   ...
		   <!--联系人-->
			...
			<#else>
			    getObjByName('businessType.id').value = '${businessTypeId?if_exists}';
			</#if>
			</script>
     ...
     <tr>
	        ...
	        <td colspan="10">
	        	<textarea name="projectInfo.conOutline" rows="4"  >${projectInfo.conOutline?if_exists}</textarea>
	        </td>
	        <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("projectInfo.conOutline").cols =width;
		   </script>
			<!---->
		</tr>
    ...
    <td align="right" valign="top">
	     	...
	     	<#else>
	     	<input type="text" name="controller.name" class="underline"  value="<#if personnelFiles?exists>${personnelFiles.name?if_exists}</#if>" maxlength="140" size="20" disabled="true"/>
	     	</#if>
			...
			<!--业务员部门-->
			<#if projectInfo.department?exists>
			<@ww.hidden id="'department.id'" name="'department.id'" value="'${projectInfo.department.id?if_exists}'"/>
			<#else>
			<@ww.hidden id="'department.id'" name="'department.id'" value="''"/>
			</#if>
    ...
    <tr>
	        ...
	        <td colspan="10">
	        	<textarea name="projectInfo.outline" rows="4"  >${projectInfo.outline?if_exists}</textarea>
	        </td>
	        <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("projectInfo.outline").cols =width;
		   </script>
			<!---->
		</tr>
    </@inputTable>
    	...
    	<#if !(action.isReadOnly())>
			...
			<#else>
				...
				<#if projectInfo.id?exists>
					...
					<@vbutton value="${action.getText('复制项目组成员和项目总计划')}" class="button" onclick ="projectPerAndPlan()"/>
...
<#if projectInfo.department?exists>
getObjByName("departmentShow").value = '${projectInfo.department.id?if_exists}';
</#if>
	window.onload=function(){
		...
		<#if !projectInfo.id?exists>
			...
			<#if personnelFiles.dept?exists>
			    getObjByName("department.id").value=${personnelFiles.dept.id?if_exists};
		        getObjByName("departmentShow").value=${personnelFiles.dept.id?if_exists};    
			</#if>
			</#if>
		</#if>
	...
	//弹出项目列表
	function projectPerAndPlan(){
	   var url =  "${req.contextPath}/projectInfo/listProjectInfo.html?readOnly=${req.getParameter('readOnly')?if_exists}&backVisitCheckBox=backVisitCheckBox&projectInfoByCopyId=${projectInfo.id?if_exists}";
	   popupModalDialog(url, 800, 600, projectPerAndPlan_);
	   //window.open(url);
	 }
	 //获得模态窗体返回值
	function projectPerAndPlan_(result) {
		if (null != result) {
		    var projectInfo_id1=result[0];
		    var projectInfo_id2=getObjByName('projectInfo.id').value;
		    DWREngine.setAsync(false); 
	 		CopyProjectPerAndPlan.saveProjectPerAndPlan(projectInfo_id1,projectInfo_id2);
...
<ul id="beautytab">
	<li>
		<a id="customers" onclick="activeTab(this);" class="selectedtab"  href='${req.contextPath}/projectInfo/listProCus.html?projectInfo.id=#{projectInfo.id}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('projectInfo.customers')}</a>
	</li>
	<li>
