<@inputTable>
	...
	<!--部门-->
	<@ww.textfield label="'${action.getText('projectInfo.controllerDept')}'" name="'departmentName'" value="'${req.getParameter('departmentName')?if_exists}'" cssClass="'underline'"/>
	<tr>
	<!--项目状态-->
...
<script language="javascript">
	...
	function contactArchives_OpenDialog(id){
	   var url = "${req.contextPath}/customerRelationship/editContactArchives.html?readOnly=${req.getParameter('readOnly')?if_exists}&contactArchives.id="+id+"&popWindowFlag=popWindowFlag&notNewFlag=notNewFlag&backvist=backvist";
	   //popupModalDialog(url, 800, 600, SelectorHandlerContactArchives);
	   openNewWindow(url);
	 }
	    //弹出项目负责人窗口
	  function controller_OpenDialog(id){
	...
	function contractManagement_OpenDialog(id){
	   var url = "${req.contextPath}/contractManagement/listContractManagementAction.html?readOnly=${req.getParameter('readOnly')?if_exists}&projectInfo.id="+id;
	   //popupModalDialog(url, 900,700, SelectorHandlerVisitBack);
	   openNewWindow(url);
	 }
