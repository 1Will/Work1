...
<@ww.form name="'listForm'" action="'saveMyHistoryTask'" method="'post'">
	...
	<@inputTable>
		...
		<td ><label class="label">
			<#if historyTaskinst.flow?exists>
				<a href="javascript: flow_Dialog(#{historyTaskinst.flow.id?if_exists});"">
...
getObjByName('result.id').value='${runPoint.result.id}'
		...
		<tr>
	        ...
	        <td colspan="4">
	        	<textarea name="remark" rows="4" cols="150" disabled="disabled" ><#if runPoint?exists>${runPoint.remark?if_exists}</#if></textarea>
	        </td>
		</tr>
	</@inputTable>
	<@buttonBar>
