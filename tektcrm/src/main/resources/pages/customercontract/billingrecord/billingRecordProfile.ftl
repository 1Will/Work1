...
<@ww.form namespace="'/contractManagement'" name="'contractManagement'" action="'saveBillingRecord'" method="'post'">
	...
	<tr>
		...
		<tr>	
			...
			<td colspan=10">
				<textarea name="billingRecord.content" rows="4"  >${billingRecord.content?if_exists}</textarea>
			</td>
			<script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("billingRecord.content").cols =width;
		   </script>
		</tr>
    </@inputTable>
    <@buttonBar>
    	<#if !(action.isReadOnly())>
			...
			<#-- 继续新建按钮   -->
			<#if billingRecord.id?exists>
			<@redirectButton value="${action.getText('newNext')}" url="${req.contextPath}/contractManagement/editBillingRecord.html?readOnly=${req.getParameter('readOnly')?if_exists}&contractManagement.id=#{billingRecord.contractManagement.id?if_exists}"/>
			<#else>
			<@redirectButton name="newNext" value="${action.getText('newNext')}" url="${req.contextPath}/contractManagement/editBillingRecord.html?readOnly=${req.getParameter('readOnly')?if_exists}&contractManagement.id=#{billingRecord.contractManagement.id?if_exists}"/>
