...
<@framePage>
	<@ww.form name="'listForm'" action="'saveContractAdditionalInfo'" method="'post'">
		...
		<tr>
			<@textarea label="${action.getText('备注')}" name="contractAdditionalInfo.comment" value="${contractAdditionalInfo.comment?if_exists}" rows="4" />
			 <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("contractAdditionalInfo.comment").cols =width;
		   </script>
		</tr>
		</@inputTable>
