...
<@htmlPage title="合同导入">
  <@ww.form  name="'importContractManagementForm'" action="'importCM'" enctype="'multipart/form-data'" method="'post'" validate="true">
  <@ww.token name="importContractManagementToken"/>
    <@inputTable>
      <tr>
        <@ww.file label="'${action.getText('ContractManagement.upload')}'" size="60" name="'file'"  required="true" />
      </tr>
    </@inputTable>
    <@buttonBar>
