...
<@ww.form name="'listForm'" action="'saveContractManagementAction'" method="'post'">
	...
	<@inputTable>
		<tr>
			<@textfield id="code" label="${action.getText('contractManagement.code')}" maxlength="30"  name="contractManagement.code"  value="${contractManagement.code?if_exists}"  required="false" anothername="checkcode" disabled="false" required="true"/>
		...
		<tr>
			...
			<@textfield id="telephone" label="${action.getText('contractManagement.telephone')}" maxlength="15"  name="contractManagement.telephone"  value="${contractManagement.telephone?if_exists}"  required="false" anothername="checkTelephone" readonly="false"/>
		</tr>
		
		
		<tr>
			<@textfield  label="${action.getText('contractManagement.address')}" name="contractManagement.address" value="${contractManagement.address?if_exists}" required="false"/>
			<td align="right" valign="top">
	       		<span class="required">*</span>
		...
		<tr>
			...
			<script language="javascript">
				<#if contractManagement.startTime?exists>
				<#else>
					var currentDate = new Date();
					if(getObjByName("contractManagement.startTime").value==''){
						getObjByName("contractManagement.startTime").value = currentDate.format("yyyy-MM-dd");
					}
				</#if>
				<#if contractManagement.ciemdinghTime?exists>
				<#else>
					var currentDate = new Date();
					if(getObjByName("contractManagement.ciemdinghTime").value==''){
						getObjByName("contractManagement.ciemdinghTime").value = currentDate.format("yyyy-MM-dd");
					}
				</#if>
			</script>
		</tr>
		...
		<tr>
			...
			<td align="right" valign="top">
				 <span class="required"></span>
	        		<label class="label">
	        			${action.getText('contractManagement.twoCFourF')}:
	        		</label>
	      </td>
	 
	      <td>
	       <select name="contractManagement.twoCFourF">
	   ...
	   <tr>
	       <td align="right" valign="top">
				 <span class="required"></span>
	        		<label class="label">
	        			${action.getText('contractManagement.back')}:
	        		</label>
	      </td>
	      <td> 		
	       <select name="contractManagement.back">
			  <option value ="否">否</option>
			  <option value ="是">是</option>
		   </select>
		   <script language="javascript">
		   <#if contractManagement.back?exists >
					getObjByName("contractManagement.back").value='${contractManagement.back?if_exists}';
					</#if>
			</script>
	      </td>
	      <td align="right" valign="top">
				 <span class="required"></span>
	        		<label class="label">
	        			${action.getText('contractManagement.receipt')}:
	        		</label>
	      </td>
	      <td> 		
	       <select name="contractManagement.receipt">
			  <option value ="0">否</option>
			  <option value ="1">是</option>
		   </select>
	   ...
	   <tr>
	      ...
	      <@pp.datePicker 
				label="'${action.getText('contractManagement.back.time')}'" 
				name="'contractManagement.backTime'" 
	   			value="'${(contractManagement.backTime?string('yyyy-MM-dd'))?if_exists}'"
				cssClass="'underline'" 
				dateFormat="'%Y-%m-%d'"
				maxlength="10"/>
	   ...
	   <@ww.select label="'${action.getText('contractManagement.state')}'" 
		...
		<tr>
		    <@textarea id="contractContent" name="contractManagement.contractContent" rows="4"  label="${action.getText('contractManagement.contractContent')}" anothername="contractContent"  required="true" value="${contractManagement.contractContent?if_exists}"/>
		    <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("contractManagement.contractContent").cols =width;
		   </script>	
	    </tr>
		<tr>
		    <@textarea name="contractManagement.remark" rows="4" label="${action.getText('contractManagement.remark')}" anothername="remark" maxLength="500" required="false" value="${contractManagement.remark?if_exists}"/>	
			  <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("contractManagement.remark").cols =width;
		   </script>	
		</tr>
		</@inputTable>
	<@buttonBar>
...
<script type='text/javascript' src='${req.contextPath}/dwr/interface/projectInfo.js'></script>
<script language="JavaScript" type="text/JavaScript"> 
<#if contractManagement.id?exists>
...
function instiChange(){
	...
	function creatorSelector1Handler(result) {
		//判断选择用户有多少项目，只有一个时显示项目信息---------------------------------------------------------
		DWREngine.setAsync(false); 
 		projectInfo.loadProjectInfoByCustomerId(result[0],function(data){
 			if(data.length != 0){
 				getObjByName("project.id").value=data[0];
				getObjByName("projectName").value=data[1];
	 			getObjByName("linkman.id").value=data[2];
				getObjByName("linkmanName").value=data[3];
				getObjByName("contractManagement.telephone").value=(data[4]==null?"":data[4]);
 			}else{
 				getObjByName("project.id").value="";
				getObjByName("projectName").value="";
	 			getObjByName("linkman.id").value="";
				getObjByName("linkmanName").value="";
				getObjByName("contractManagement.telephone").value="";
 			}
 		});
 		DWREngine.setAsync(true); 
	...
	function storeValidation(){
		if(getObjByName("contractManagement.code").value==""){
			alert("请填写合同编码！");
			getObjByName("contractManagement.code").focus();
			return false;
		}
		...
		if(getObjByName("contractManagement.endTime").value != ""){
			if(!dateCheckPicker(true,'contractManagement.endTime','${action.getText('validation.endTime')}','%Y-%m-%d')){
				return false;
			}
		}
		if(getObjByName("contractManagement.backTime").value != ""){
			if(!dateCheckPicker(true,'contractManagement.backTime','返回日期：','%Y-%m-%d')){
				return false;
			}
		}
		//验证开始日期是否大于终止时间
		...
		if(getObjByName("contractManagement.back").value=="L"){
		getObjByName("contractManagement.backTime").value='';
		}
		return true;
	}
	//提交
...
//	window.onload=function (){
        <#if contractManagement.back?exists>
            var back="${contractManagement.back?if_exists}";
            if(back=="H"){
            getObjByName("contractManagement.back").value=("${contractManagement.back?if_exists}");
            }
		</#if>
        <#if contractManagement.twoCFourF?exists>
			getObjByName("contractManagement.twoCFourF").value=("${contractManagement.twoCFourF?if_exists}");
		</#if>
		<#if contractManagement.receipt?exists>
			getObjByName("contractManagement.receipt").value=("${contractManagement.receipt?if_exists}");
		</#if>
		...
		<#if contractManagement.moneyType?exists>
			getObjByName("moneyType.id").value=("${contractManagement.moneyType.id?if_exists}");
		</#if>
		...
		/*getObjByName("moneyType.id").change(function(){
