...
<@ww.hidden name="'readOnly'" value="'${req.getParameter('readOnly')?if_exists}'"/>
    ...
    <@inputTable>
        ...
        <tr>
            <@ww.textfield label="'${action.getText('出差时长（天）')}'" name="'onTheRoadBill.manDay'" value="'${onTheRoadBill.manDay?if_exists}'" cssClass="'underline'" required="true" readonly="false"/>
            <#if onTheRoadBill.status?exists && onTheRoadBill.status.code != '02000'>
...
<script type="text/javascript">
	window.onload=function(){
		...
		<#if onTheRoadBill.flow?exists>
			getObjByName('flow.id').value='${onTheRoadBill.flow.id?if_exists}';
		</#if>
	}
	//计算请假天数
	function saveDays(){
		var star = getObjByName("onTheRoadBill.startTime").value;
		var end = getObjByName("onTheRoadBill.endTime").value;
		if(star!="" && end!=""){
			...
			if(end<star){
			   ...
			   getObjByName("leaveBill.endTime").value="";
				...
				getObjByName("onTheRoadBill.manDay").value =days;
			}
		}
	}
	function getDate(dt){
	...
	function storeValidation(){
		...
		if(getObjByName('onTheRoadBill.manDay').value==''){
			alert("${action.getText('出差天数不能为空')}");
			getObjByName('onTheRoadBill.manDay').focus();
			return false;
		}
		if(getObjByName('flow.id').value==''){
			alert("${action.getText('流程类型不能为空！')}");
...
<ul id="beautytab">
	<li>
		<a id="runPoint" class="selectedtab" onclick="activeTab(this);"  href='${req.contextPath}/activitiFlow/listRunPoint.html?flow.id=${onTheRoadBill.flow.id?if_exists}&bussnessId=#{onTheRoadBill.id}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('审批人')}</a>
	</li>
	<li>
		<a id="CopySendPerson" onclick="activeTab(this);"  href='${req.contextPath}/activitiFlow/listCopySendPerson.html?flow.id=${onTheRoadBill.flow.id?if_exists}&bussnessId=#{onTheRoadBill.id}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('抄送人')}</a>
	</li>
	<li>
...
<iframe name="frame" frameborder="0.5" src="${req.contextPath}/activitiFlow/listRunPoint.html?flow.id=${onTheRoadBill.flow.id?if_exists}&bussnessId=#{onTheRoadBill.id}&readOnly=${req.getParameter('readOnly')?if_exists}" marginHeight="0" marginWidth="0" scrolling="auto" vspace=0 hspace=0 width="100%" height="60%"/>
</#if>
