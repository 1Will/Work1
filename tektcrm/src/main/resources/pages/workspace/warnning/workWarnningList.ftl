...
<@htmlPage title="${action.getText('listWorkWarnning.title')}">
	 <@ww.form name="'listWorkWarnning'" action="'searchWorkWarnning'" method="'post'">
        ...
        <@list title="${action.getText('workWarnning.list')}" 
             includeParameters="type|loginUser.id|warnningDate|warnningDate_start|warnningDate_end|onlyUnRead|onlyRead|name" 
