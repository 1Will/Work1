...
<@htmlPage title="${action.getText('products.info.profile')}">
	<@ww.form namespace="'/productsManager'" name="'listForm'" action="'saveProductsAction'" method="'post'">
		...
		<@inputTable>
			...
			<tr>
	           	<@ww.textfield label="'${action.getText('products.code')}'" name="'products.code'" value="'${products.code?if_exists}'" cssClass="'underline'" required="true" disabled="true"/>
			...
			<tr>	
			    ...
			    <td align="left">
		        	<input type="radio" id="products.isNoMain1" name="products.isNoMain" value=true>是
		        	<input type="radio" id="products.isNoMain2" name="products.isNoMain" value=false>否
		        	<script language="javascript">
			     		if(${(products.isNoMain?string)?if_exists}){
			...
			<tr>
		        ...
		        <td colspan="10">
		        	<textarea name="products.remark" rows="4"  >${products.remark?if_exists}</textarea>
		        </td>
		        <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("products.remark").cols =width;
		   </script>
	   		</tr>
		</@inputTable>
		<@buttonBar>
...
//		window.onload=function(){
	<#if personnelFiles?exists>
	         var sBusinessType=539; 
	         <#if personnelFiles.businessType?exists>
	         sBusinessType=${personnelFiles.businessType.id?if_exists};
	        </#if>
	        if(sBusinessType!=539){
	         getObjByName("businessType.id").value=sBusinessType;
	         getObjByName("businessType.id").disabled=true;
	         //设置同步
...
//		}
	...
	function storeValidation(){
	    ...
	    <#-- 推出时间 12-->
		  if('' != document.forms["listForm"].elements["products.launch"].value){
		 	 if(!isDate('products.launch')){
				 ...
				 alert("推出时间：${action.getText('dateTimeFormat.error')}");
	   ...
	   getObjByName("businessType.id").disabled=false;
		return true;
	}
