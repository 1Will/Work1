    ...
    <@ww.hidden name="'readOnly'" value="'${req.getParameter('readOnly')?if_exists}'"/>
        ...
        <tr>
			<td align="right" valign="top">
	       		<label class="label">${action.getText('institution.address')}:</label>
	     	</td>
			<td colspan="10">
				<input type="text" name="institution.address" class="underline"  value="${institution.address?if_exists}" maxlength="70" size="60" />
			</td>
			 <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("institution.address").cols =width;
		   </script>
        </tr>
        <tr>
	        <td align="right" valign="top">
	        ...
	        <td colspan="10">
	        	<textarea name="institution.comment" rows="4" >${institution.comment?if_exists}</textarea>
	        </td>
	         <script language="javascript">
			 var width=document.body.clientWidth/9;
					getObjByName("institution.comment").cols =width;
		   </script>
        </tr>
    </@inputTable>
    <@buttonBar>
...
<script type="text/javascript">
	...
	function confirmDelete(msg){
 	...
 	function storeValidation(){
		...
		if("" != getObjByName("institution.setupTime").value){
			if(!isDate("institution.setupTime")){
				alert("创立日期：${action.getText('dateTimeFormat.error')}");
				getObjByName('institution.setupTime').value="";
				getObjByName('institution.setupTime').focus();
				return false;
			}
		}
		parent.frames["dTreeFrame"].location.reload();
...
<ul id="beautytab">
	<li>
		<a id="qualification" onclick="activeTab(this);" href='${req.contextPath}/personnelFile/listQualification.html?institution.id=${institution.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('资格证书')}</a>
	</li>
</ul>
<iframe name="frame" frameborder="0.5" src="${req.contextPath}/personnelFile/listQualification.html?institution.id=${institution.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}" marginHeight="0" marginWidth="0" scrolling="auto" vspace=0 hspace=0 width="100%" height="50%"/>
</#if>
-->