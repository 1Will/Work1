...
<@ww.form namespace="'/paymentorder'"  name="'paymentorder'" action="'savePaymentorderAction'" method="'post'">
	...
	<@inputTable>
		<tr>
			<@textfield id="code" label="${action.getText('paymentorder.code')}" maxlength="10"  name="paymentorder.code"  value="${paymentorder.code?if_exists}"  required="false" anothername="checkcode" disabled="true" readonly="true"/>
		...
		<tr>
		    <@textarea name="paymentorder.remark" label="${action.getText('paymentorder.remark')}" anothername="content" rows="4"  maxLength="500" required="false" value="${paymentorder.remark?if_exists}"/>
		    <script language="javascript">
			 var width=document.body.clientWidth/9;
						getObjByName("paymentorder.remark").cols =width;
		   </script>	
	    </tr>
		</@inputTable>
	<@buttonBar>
