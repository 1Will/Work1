...
<@framePage>
	<@ww.form name="'listForm'" action="'searchPoint'" method="'post'">
        ...
        <@list title="${action.getText('节点列表')}" excel=false setupTable=false
			...
			<a href="###" onclick="openPage('editPoint.html?point.id=#{object.id}&backvist=${backvist?if_exists}&flow.id=${req.getParameter('flow.id')?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}')">
			#{object.myNum}</a>
				<@alignCenter />
			</@vcolumn>
