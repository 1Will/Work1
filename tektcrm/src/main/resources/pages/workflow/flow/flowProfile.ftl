...
<@ww.hidden name="'readOnly'" value="'${req.getParameter('readOnly')?if_exists}'"/>
	...
	<#if req.getParameter('activitiFLow')?exists>
...
<ul id="beautytab">
	<li>
		<a id="id" onclick="activeTab(this);" class="selectedtab" 
		           href='${req.contextPath}/point/listPoint.html?flow.id=${flow.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}&backvist=${backvist?if_exists}' target="frame" >
		    ${action.getText('流程节点')}
		</a>
	</li>
</ul>
<iframe name="frame" frameborder="0.5" src="${req.contextPath}/point/listPoint.html?flow.id=${flow.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}&backvist=${backvist?if_exists}" 
