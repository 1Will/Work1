...
-->
<#macro eam2008_remoteGetLocation code=""  loopNum="" inputName=""  disabled="false" >
 	<#if "${disabled}"='false'>
      ...
      function location_OpenDialog(){
        ...
        <#if spareInBill?exists>
        	<#if spareInBill.warehouse.id?exists>
       		url = "${req.contextPath}/newLocation/locationSearcherCommon.html?warehouse=#{spareInBill.warehouse.id}&fromSpareInBillDetail=true";
       		</#if>
	 	<#else>
	   		url = "${req.contextPath}/newLocation/locationSearcherCommon.html";
	 	</#if>
...
<#--
   ...
   inputName : input类型控件name属性的前部分值 [name属性格式: inputName + loopNum]
-->
<#macro eam2008_getBuyer code=""  name="" loopNum="" inputName="" inputId="" inputN="" disabled="false" >
 	<#if "${disabled}"='false'>
		...
		<input type="hidden" name="${inputId}${loopNum}" id="${inputId}${loopNum}"/>
    <label id=""></label>
