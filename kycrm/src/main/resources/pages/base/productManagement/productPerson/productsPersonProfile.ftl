...
<@ww.form namespace="'/productsManager'" name="'productsPerson'" action="'saveProductsPerson'" method="'post'">
    ...
    <tr>
        ...
        <#if productsPerson.explain?exists>
        	<textarea name="productsPerson.explain" rows="4"  >${productsPerson.explain?if_exists}</textarea>
        <#else>
        	<textarea name="productsPerson.explain" rows="4"  ></textarea>
       </#if>
       <script language="javascript">
			var width=document.body.clientWidth/9;
						getObjByName("productsPerson.explain").cols =width;
	   </script>
        </td>
	</tr>
    </@inputTable>
