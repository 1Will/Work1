...
<@htmlPage title="${action.getText('uploadManual.title')}">
  ...
  <@ww.token name="saveManualToken"/>
    ...
    <#if manual.id?exists>
      ...
      <tr>
        <@ww.file label="'${action.getText('manual.upload')}'" size="60" name="'file'"  required="true" onchange="'getName();'"/>
      </tr>
      <tr>
      ...
      <tr>
	    <@ww.textarea label="'${action.getText('helpManual.description')}'" 
					  ...
					  value="'${manual.description?if_exists}'" rows="'4'" 
					  required="false"/>
		<script language="javascript">
			var width=document.body.clientWidth/9;
						getObjByName("helpManual.description").cols =width;
		   </script>
      </tr>
    </@inputTable>
    <@buttonBar>
