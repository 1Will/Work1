...
<@htmlPage title="${action.getText('weekly.Manager')}">
	<@ww.form id="listFrom" name="'listFrom'" namespace="'/workReport'" action="'searchWeekly'" method="'post'">
   	...
   	<@list title="${action.getText('weekly.List')}" includeParameters="code|name|rapporteur|inst.id|dept.id|duty.id|startDate|endDate|onlyInvalid|onlyValid|personnelFiles.id|month" fieldMap="like:code|name|rapporteur|month" >
