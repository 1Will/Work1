...
<@htmlPage title="${action.getText('co.title')}">
	<@ww.form name="'listFrom'" namespace="'/com'" action="'searchCoWindow'" method="'post'">
		<@ww.token name="searchCoWindowToken"/>
        ...
        <@list title="${action.getText('co.list.title')}" 
            includeParameters="co.code|co.customerInfo|co.deliveryTime|co.products|co.linkman|deliveryWay.id|type.id|co.phone||onlyInvalid|onlyValid" 
        	fieldMap="like:co.code|co.customerInfo|co.products|co.linkman|co.phone,date:co.deliveryTime" >
        	<#if !object.disabled>
	        	 <@vcolumn title="${action.getText('co.code')}" property="code" sortable="asc">
