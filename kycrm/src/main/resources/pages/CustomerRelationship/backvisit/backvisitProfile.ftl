...
<@ww.form name="'listForm'" namespace="'/backvisit'" action="'saveBackVisit'" method="'post'">
	...
	<@inputTable>
		...
		<tr>
	        ...
	        <td colspan="10">
	        	<textarea name="backVisit.backVisitContent" rows="4" >${backVisit.backVisitContent?if_exists}</textarea>
	        	<script>
	        
				var width=document.body.clientWidth/9;
				getObjByName("backVisit.backVisitContent").cols =width;
		...
		<tr>
	        ...
	        <td colspan="10">
	        	<textarea name="backVisit.feedback" rows="4" >${backVisit.feedback?if_exists}</textarea>
	        	<script>
	        		var width=document.body.clientWidth/9;
				getObjByName("backVisit.feedback").cols =width;
		...
		<tr>
	        ...
	        <td colspan="10">
	        	<textarea name="backVisit.attention" rows="4" >${backVisit.attention?if_exists}</textarea>
	        	<script>
	        	var width=document.body.clientWidth/9;
				getObjByName("backVisit.attention").cols =width;
		...
		<tr>
	        ...
	        <td colspan="10">
	        	<textarea name="backVisit.remarks" rows="4"  >${backVisit.remarks?if_exists}</textarea>
	        	<script>
	        	var width=document.body.clientWidth/9;
				getObjByName("backVisit.remarks").cols =width;
		...
		<tr id="changeReasonTr" name="changeReasonTr"  style="display:none" >
	        ...
	        <td colspan="10">
	        	<textarea name="changeReason" rows="4"  >${backVisit.changReason?if_exists}</textarea>
	        </td>
	        <script language="javascript">
				var width=document.body.clientWidth/9;
				getObjByName("changeReason").cols =width;
			</script>
			<!---->
		</tr>
		...
		<tr id="changStateReasonTr" name="changStateReasonTr"  style="display:none" >
	        ...
	        <td colspan="10">
	        	<textarea name="changStateReason" rows="4"  >${backVisit.changStateReason?if_exists}</textarea>
	        </td>
	        <script language="javascript">
				var width=document.body.clientWidth/9;
				getObjByName("changStateReason").cols =width;
			</script>
			<!---->
		</tr>
	...
	function check(){
		...
		if('' == getObjByName('backVisit.backVisitDate').value){
			...
			if(getObjByName('backVisit.backVisitDate').value!=''){
			  ...
			  if(dateNow<getObjByName('backVisit.backVisitDate').value){
			     alert("回访时间必须小于等于今天！");
			     return false;
			  }
			}
		...
		if(getObjByName('continueBackVisit0').checked == true && '' != getObjByName('backVisit.nextBackVisitDate').value && getObjByName('backVisit.nextBackVisitDate').value < getObjByName('backVisit.backVisitDate').value ){
				alert("下次回访时间必须大于回访时间！");
				getObjByName('backVisit.nextBackVisitDate').focus();
				return false;
		}
		/* 验证回访内容 */
