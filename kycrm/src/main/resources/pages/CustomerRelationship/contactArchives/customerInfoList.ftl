	...
	<@ww.form name="'listForm'" action="'searchCustInfo'" method="'post'">
        ...
        <@list title="${action.getText('customerInfo.list.title')}" 
            ...
            <@vcolumn title="${action.getText('customerInfo.code')}" property="code" sortable="desc">
                <a href="javascript: returnDialog(new Array(#{object.id}, '${object.name}','<#if object.customerType?exists>${object.customerType.id}</#if>','<#if object.customerType?exists>${object.customerType.name}</#if>','<#if object.industry?exists>${object.industry.name}</#if>','<#if object.salesman?exists>${object.salesman.id}</#if>','<#if object.salesman?exists>${object.salesman.name}</#if>','${object.address?if_exists}','${object.keyContacter?if_exists}','<#if object.step?exists>${object.step.id?if_exists}</#if>','${object.effectDescribe?if_exists}'));">
	                    ${object.code}
                </a>
				<@alignLeft/>
            </@vcolumn>
