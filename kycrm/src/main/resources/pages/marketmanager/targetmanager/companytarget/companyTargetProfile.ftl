...
<@htmlPage title="${action.getText('targetManagement.profile.title')}">
<@ww.form name="'listForm'" action="'saveCompanyTargetAction'" method="'post'">
	...
	<@inputTable>
		<tr>
			...
			<@textfield id="targetName" label="${action.getText('目标名称')}" maxlength="10"  name="targetManagement.targetName"  value="${targetManagement.targetName?if_exists}"  required="true" anothername="checktargetName"/>
		...
		<tr>
	        ...
	        <td colspan="5">
	        	<textarea name="targetManagement.remark" rows="6" >${targetManagement.remark?if_exists}</textarea>
	        </td>
	        <script language="javascript">
				var width=document.body.clientWidth/9;
				getObjByName("targetManagement.remark").cols =width;
			</script>
			</tr>
	</@inputTable>
	<@buttonBar>
