...
<@htmlPage title="${action.getText('listWorkWarnning.title')}">
	 <@ww.form name="'listWorkWarnning'" action="'searchWorkWarnning'" method="'post'">
        ...
        <@list title="${action.getText('workWarnning.list')}" includeParameters="type|loginUser.id|warnningDate|warnningDate_start|warnningDate_end|onlyUnRead|onlyRead|name" 
        	fieldMap="like:type,name,date:warnningDate_start|warnningDate_end">
             ...
             <@vcolumn title="${action.getText('warnning.warnningDate')}" property="warnningDate" format="yyyy-MM-dd hh:mm" sortable="desc">
               <@alignLeft/>
             </@vcolumn>
             <#assign status=''/>
         	 <#if object.readFlag>
