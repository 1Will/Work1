...
<@htmlPage title="${action.getText('personnel.profile.title')}">
     <@ww.form action="'savePersonnelFile'" namespace="'/personnelFile'" method="'post'" >
       ...
       <@ww.hidden name="'notOpen'" value="'${notOpen?if_exists}'"/>
        <@ww.hidden name="'backvist'" value="'${backvist?if_exists}'"/>
       <@ww.hidden name="'isSaved'" value="''"/>
    	...
    	<tr>
    	<@ww.select label="'${action.getText('职称')}'" 
				name="'professor.id'" 
				value="'${req.getParameter('professor.id')?if_exists}'"
				listKey="id"
				listValue="name"
				list="allProfessor"
				required="false"
    	...
    	<@ww.select label="'${action.getText('职级')}'" 
				name="'rank.id'" 
				value="'${req.getParameter('rank.id')?if_exists}'"
				listKey="id"
				listValue="name"
				list="allRank"
				required="false"
    	...
    	<@ww.select label="'${action.getText('等级')}'" 
				name="'grade.id'" 
				value="'${req.getParameter('grade.id')?if_exists}'"
				listKey="id"
				listValue="name"
				list="allGrade"
				required="false"
    	...
    	<tr>
         ...
         <@buttonBar>
           	 <#if !(action.isReadOnly())>
	         	...
	         	<#if personnelFile.isSaved?exists &&personnelFile.isSaved=='0' >
		    	...
		    	<#if personnelFile.id?exists>
	         		<@vbutton value="${action.getText('职级等级变更')}" class="button" onclick ="changeState()"/>
		         </#if>
	         </#if>
	         <#if backvist?exists && backvist=="backvist">
...
<script>
  ...
  //姓名
  	...
  	function changeState(){
    	var url = "${req.contextPath}/personnelFile/editRankAndGrade.html?personnelFiles.id=${personnelFile.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}";
    	openNewWindow(url);
    }
...
//	window.onload=function(){
		...
		<#else>
		 ...
		 //职称
		<#if personnelFile.professor?exists>
			getObjByName('professor.id').value='${personnelFile.professor.id?if_exists}';
		<#else>
			<#if req.getParameter('professor.id')?exists>
				getObjByName('professor.id').value='${req.getParameter('professor.id')?if_exists}';
			</#if>    
		</#if>
		 //职级
		<#if personnelFile.rank?exists>
			getObjByName('rank.id').value='${personnelFile.rank.id?if_exists}';
		<#else>
			<#if req.getParameter('rank.id')?exists>
				getObjByName('rank.id').value='${req.getParameter('rank.id')?if_exists}';
			</#if>    
		</#if>
		 //职级
		<#if personnelFile.grade?exists>
			getObjByName('grade.id').value='${personnelFile.grade.id?if_exists}';
		<#else>
			<#if req.getParameter('grade.id')?exists>
				getObjByName('grade.id').value='${req.getParameter('grade.id')?if_exists}';
			</#if>    
		</#if>
        //保存成功或者更新时保留下拉列表的值
...
<ul id="beautytab">
	...
	<li>
		<a id="salarySet" onclick="activeTab(this);" href='${req.contextPath}/personnelFile/listQualification.html?personnelFiles.id=${personnelFile.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('资格证书')}</a>
	</li>
	<li>
		<a id="salarySet" onclick="activeTab(this);" href='${req.contextPath}/salarySet/editSalarySet.html?personnelFiles.id=${personnelFile.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('工资信息')}</a>
	</li>
	<li>
	...
	<li>
		<a id="wordExperience" onclick="activeTab(this);" href="${req.contextPath}/personnelFile/listWorkExperience.html?pf.id=${personnelFile.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}" target="frame" >${action.getText('personnel.wordExperience')}</a>
	</li>
	<li>
	...
	<li>
		<a id="rankAndGrade" onclick="activeTab(this);" href='${req.contextPath}/personnelFile/listRankAndGrade.html?personnelFiles.id=${personnelFile.id?if_exists}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('职级等级变更历史')}</a>
	</li>
