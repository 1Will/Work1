...
<@vcolumn title="${action.getText('序号')}" property="id"  >
                   <a href="javascript:editProduct('#{object.products.id}')">#{itemNo}</a>
	            <@alignLeft attributes="width:100;"/>
	            </@vcolumn >
	            <#assign itemNo=itemNo + 1/>
             <@vcolumn title="${action.getText('products.code')}" property="products.code" sortable="desc">
             <#--
...
function product_OpenDialog(){
		...
		popupModalDialog(url, 1100, 600, creatorSelectorHandlerProduct);
	}
