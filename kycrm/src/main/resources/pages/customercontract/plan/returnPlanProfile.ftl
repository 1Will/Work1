...
<@ww.hidden name="'readOnly'" value="'${req.getParameter('readOnly')?if_exists}'"/>
    ...
    <@inputTable>
		...
		<tr>
		<#if returnPlan.house?exists>
         <@ww.textfield label="'${action.getText('房间名称')}'" name="'returnPlan.house'" value="'${returnPlan.house?if_exists}'" readonly="true" cssClass="'underline'"/>
		<#--
         <td align="right" valign="top">
				<label class="label">房间名称:</label>
			</td>
			<td>
				<input type="text" style=" border:none #FFFFFF;  border-bottom:#000000 solid 1px;  background:transparent;  " value="${returnPlan.house.name?if_exists}" readonly="readonly" id="house。name" name="house。name" >
			</td>
			-->
		</#if>
		<@ww.select 
		...
		<tr>	
			...
			<td colspan=10">
				<textarea name="returnPlan.remark" rows="4" >${returnPlan.remark?if_exists}</textarea>
			</td>
			<script language="javascript">
				var width=document.body.clientWidth/9;
				getObjByName("returnPlan.remark").cols =width;
			</script>
		</tr>
    </@inputTable>
    <@buttonBar>
    	<#if !(action.isReadOnly())>
			...
			<#if returnPlan.id?exists>
				<#if returnPlan.contractManagement?exists>
					<@redirectButton value="${action.getText('newNext')}" url="${req.contextPath}/contractManagement/editReturnPlan.html?readOnly=${req.getParameter('readOnly')?if_exists}&contractManagement.id=${returnPlan.contractManagement.id?if_exists}&popWindowFlag=${popWindowFlag?if_exists}"/>
				<#else>
					<@redirectButton value="${action.getText('newNext')}" url="${req.contextPath}/contractManagement/editReturnPlan.html?readOnly=${req.getParameter('readOnly')?if_exists}&popWindowFlag=${popWindowFlag?if_exists}"/>
				</#if>
			<#else>
...
<script type="text/javascript">
	...
	<#if returnPlan.batch?exists>
		...
		<#if returnPlan.mold?exists>
		getObjByName("mold.id").value="#{returnPlan.mold.id?if_exists}";
	    </#if>	
	...
	function setCustomer(result) {
		if (null != result) {
		 	...
		 	//getObjByName('returnPlan.contactArchives').value=result[8];
		 	//getObjByName('contactArchives.id').value=result[9];
		}
	}
	...
	function contactArchives_OpenDialog(){
		...
		if('' == customerid){
			...
			var url = "${req.contextPath}/com/listContactArchivesWindow.html?readOnly=${req.getParameter('readOnly')?if_exists}&customer.id="+customerid;
	...
	function chargMan_OpenDialog(){
	   var url = "${req.contextPath}/personnelFile/listPersonByUser.html?readOnly=${req.getParameter('readOnly')?if_exists}";
	   popupModalDialog(url, 800, 600, setChargMan);
	   //window.open(url);
	 }
	 //获得模态窗体返回值
	function setChargMan(result) {
		if (null != result) {
	...
	-->
		if(getObjByName('customer.id').value==''){
			alert("${action.getText('returnPlan.customerInfo.requiredstring')}");
			return false;
		}
		...
		if(getObjByName('mold.id').value==''){
			alert("${action.getText('请选择收款类型')}");
			getObjByName('mold.id').focus();
			return false;
		}
