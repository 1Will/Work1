...
<@ww.form namespace="'/contractManagement'" name="'contractManagement'" action="'saveBillingRecord'" method="'post'">
    ...
    <@inputTable>
    	...
    	<#else>
	 	...
	 	<#if billingRecord.returnPlan?exists>
    		<@ww.hidden name="'returnPlan.id'" value="'#{billingRecord.returnPlan.id?if_exists}'"/>
	 	<#else>
...
<script type="text/javascript">
	...
	function getRest(){
		...
		var rest = toDecimal(plan-has-sum);
		if(rest>=0){
	...
	-->
		if(getObjByName('billingRecord.contactArchives').value==''){
			alert("${action.getText('请选择联系人！')}");
			return false;
		}
