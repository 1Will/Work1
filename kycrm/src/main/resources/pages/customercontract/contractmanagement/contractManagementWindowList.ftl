        	...
        	<#if !object.disabled>
        		<#if object.customerInfo.additional?exists>
	        	 <@vcolumn title="${action.getText('contractManagement.code')}" property="code" sortable="asc">
		            <a href="javascript: returnDialog(new Array(#{object.id},'${object.contractName}','${object.customerInfo.name}','${object.linkman.name}','#{object.customerInfo.id}','#{object.linkman.id}','${object.telephone}','<#if object.payType?exists>${object.payType.code}<#else></#if>','#{object.contractMoney}','${object.customerInfo.additional.bankAccount}','<#if object.project?exists>${object.project.name}<#else></#if>'));">
		            	${object.code}
		            </a>
		            <@alignLeft/>
		        </@vcolumn>
		        <#else>
			        <@vcolumn title="${action.getText('contractManagement.code')}" property="code" sortable="asc">
			            <a href="javascript: returnDialog(new Array(#{object.id},'${object.contractName}','${object.customerInfo.name}',<#if object.linkman?exists>'${object.linkman.name}'<#else>''</#if>,'#{object.customerInfo.id}',<#if object.linkman?exists>'#{object.linkman.id}'<#else>''</#if>,'${object.telephone}','<#if object.payType?exists>${object.payType.code}<#else></#if>','#{object.contractMoney}','','<#if object.project?exists>${object.project.name?if_exists}<#else></#if>'));">
			            	${object.code}
			            </a>
			            <@alignLeft/>
			        </@vcolumn>
