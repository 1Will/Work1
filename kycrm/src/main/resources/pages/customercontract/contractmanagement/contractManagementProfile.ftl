...
<@ww.form name="'listForm'" action="'saveContractManagementAction'" method="'post'">
	...
	<@inputTable>
		...
		-->
			<@textfield id="square" label="${action.getText('contractManagement.square')}" maxlength="10"  name="contractManagement.square"  value="#{contractManagement.square?if_exists}"  required="false" anothername="square" readonly="true"/>
		...
		<tr>
		    <@textarea id="contractContent" name="contractManagement.contractContent" rows="4"  label="${action.getText('contractManagement.contractContent')}" anothername="contractContent" maxLength="500" required="true" value="${contractManagement.contractContent?if_exists}"/>	
		...
		<#if !(action.isReadOnly())>
			...
			<#-- 添加收款单   -->
				<#if !(action.isReadOnly())>
					...
					<#if hasReturnPlan>
						<@vbutton name="returnPlan" value="${action.getText('生成收款计划')}" class="button" onclick ="createReturnPlan(#{contractManagement.id?if_exists})"/>
						<script language="JavaScript" type="text/JavaScript"> 
							getObjByName("returnPlan").disabled="true";
						</script>
		   			<#else>
						<@vbutton value="${action.getText('生成收款计划')}" class="button" onclick ="createReturnPlan(#{contractManagement.id?if_exists})"/>
		   			</#if>
		   		</#if>
	   		</#if>
		</#if>
...
<script type='text/javascript' src='${req.contextPath}/dwr/interface/AutoReturnPlan.js'></script>
<script language="JavaScript" type="text/JavaScript"> 
	function createReturnPlan(id){
		DWREngine.setAsync(false); 
		AutoReturnPlan.autoReturnPlan(id,refreshPage);
...
<ul id="beautytab">
	...
	<li>
		<a id="houseList" onclick="activeTab(this);" class="selectedtab" href='${req.contextPath}/productList/listHouseList.html?contractManagement.id=#{contractManagement.id}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('房源明细')}</a>
	</li>
	
	<li>
		<a id="productInfo" onclick="activeTab(this);" href='${req.contextPath}/productList/listProductList.html?contractManagement.id=#{contractManagement.id}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('服务产品')}</a>
	</li>
	<li>
		<a id="houseList_back" onclick="activeTab(this);" href='${req.contextPath}/productList/listHouseListBack.html?contractManagement.id=#{contractManagement.id}&readOnly=${req.getParameter('readOnly')?if_exists}' target="frame" >${action.getText('退房明细')}</a>
	</li>
	<li>
...
<iframe name="frame" frameborder="0.5" src="${req.contextPath}/productList/listHouseList.html?contractManagement.id=#{contractManagement.id}&readOnly=${req.getParameter('readOnly')?if_exists}" marginHeight="0" marginWidth="0" scrolling="auto" vspace=0 hspace=0 width="100%" height="50%"/>
</#if>