...
var child_id;
var m;
function industryDWR(cmId,batchId,msg){
	cur_id = cmId;
	child_id = batchId;
	m = msg;
	var pid = getObjByName(cur_id).value;
	
	if(null != pid && "" != pid ){
		Industry.loadByPid(pid,industryBack);
	}
}

function industryBack(data){
	if(null != data){
		getObjByName(child_id).length = 0;
		for(var i =0 ;i<data.length;i++){
			if(i == 0){
				getObjByName(child_id).options.add(new Option(m,-1));
				//document.all(bh_id).options[i].value = -1;
				getObjByName(child_id).options.add(new Option(data[0].name,data[0].id));
			}else{
				getObjByName(child_id).options.add(new Option(data[i].name,data[i].id));
			}
		}	  
	}
}