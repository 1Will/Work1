...
public class DefaultCustomerInfoManager extends BaseManager implements CustomerInfoManager {
	...
	public void storeCustomerInfo(CustomerInfo ci) {
		if (ci.isNew()) {
			ci.setCode((String) this.yongJunSequenceManager.generateByCodeType(YongJunSequenceConstant.CODE_CUSTOMER));
		}
		...
		if(customerInfos!=null&&customerInfos.size()>0){
			for(CustomerInfo cu:customerInfos){
				...
				if(cu.getCode()==null||cu.getCode().equals("")){
					cu.setCode((String)this.yongJunSequenceManager.generateByCodeType(YongJunSequenceConstant.CODE_CUSTOMER));
				}
