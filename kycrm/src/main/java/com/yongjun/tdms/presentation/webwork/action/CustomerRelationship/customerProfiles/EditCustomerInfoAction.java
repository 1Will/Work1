...
public class EditCustomerInfoAction extends PrepareAction {
	...
	public String save() {
		...
		try {
			...
			if (hasId("nationalEconomyP.id")) {
				String id = this.request.getParameter("nationalEconomyP.id");
				Industry nationalEconomyP = industryManager.loadIndustry(Long.parseLong(id));
				customerInfo.setNationalEconomyP(nationalEconomyP);
			}
			if (hasId("nationalEconomyC.id")) {
				String id = this.request.getParameter("nationalEconomyC.id");
				Industry nationalEconomyC = industryManager.loadIndustry(Long.parseLong(id));
				customerInfo.setNationalEconomyC(nationalEconomyC);
			}
			if (hasId("technologyEconomyP.id")) {
				String id = this.request.getParameter("technologyEconomyP.id");
				Industry technologyEconomyP = industryManager.loadIndustry(Long.parseLong(id));
				customerInfo.setTechnologyEconomyP(technologyEconomyP);
			}
			if (hasId("technologyEconomyC.id")) {
				String id = this.request.getParameter("technologyEconomyC.id");
				Industry technologyEconomyC = industryManager.loadIndustry(Long.parseLong(id));
				customerInfo.setTechnologyEconomyC(technologyEconomyC);
			}
			if (isNew) {
	...
	public List<Industry> getAllNationalEconomy(){
		...
		try {
			//构建参数数组
			String[] keys = new String[]{"pid","type"};
			Long[] values = new Long[]{0l,0l};
			
			List<Industry> inList = industryManager.loadByKeyArray(keys, values);
	...
	public List<Industry> getAllNationalEconomyC(){
		List<Industry> list = new ArrayList<Industry>();
		try {
			//构建参数数组
			String[] keys = new String[]{"pid","type"};
			...
			List<Industry> inList = industryManager.loadByKeyArray(keys, values);
			if(inList.size() > 0 && inList != null){
				Long id = inList.get(0).getId();
				List<Industry> children = industryManager.loadByPid(id);
	...
	public List<Industry> getAllTechnologyEconomy(){
		List<Industry> list = new ArrayList<Industry>();
		try {
			//构建参数数组
			String[] keys = new String[]{"pid","type"};
	...
	public List<Industry> getAllTechnologyEconomyC(){
		List<Industry> list = new ArrayList<Industry>();
		try {
			//构建参数数组
			String[] keys = new String[]{"pid","type"};
