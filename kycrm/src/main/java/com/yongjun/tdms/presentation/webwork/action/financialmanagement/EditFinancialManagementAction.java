	...
	public void prepare() throws Exception {
		...
		if (hasId("financialManagement.id")) {
			...
			if (hasId("returnPlan.id")) {
				ReturnPlan returnPlan = returnPlanManager.loadReturnPlan(getId("returnPlan.id"));
				this.financialManagement.setReturnPlan(returnPlan);
				this.financialManagement.setContractManagement(returnPlan.getContractManagement());
				if (returnPlan.getContractManagement() != null) {
					this.financialManagement.setCustomerInfo(returnPlan.getContractManagement().getCustomerInfo());
				} else {
					this.financialManagement.setCustomerInfo(returnPlan.getCustomerInfo());
				}
				this.batchs.add(returnPlan.getBatch());
				this.financialManagement.setSumReceivable(Double.valueOf(format.format(returnPlan.getSum())));
				this.financialManagement.setTotalSum(Double.valueOf(format.format(returnPlan.getFactSum())));
			}
		}
	...
	public String save() throws Exception {
		...
		try {
			...
			if (hasId("collectionType.id")) {
				this.financialManagement.setCollectionType(this.codeValueManager
						.loadCodeValue(getId("collectionType.id")));
			}
			...
			if ("1".equals(this.request.getParameter("financialManagement.isSaved"))) {
				...
				map.put("name", new SimpleDateFormat("yyyy-MM-dd").format(this.financialManagement.getCreatedTime())
						+ "," + this.financialManagement.getPayee().getName() + "提交了收款单:"
						+ this.financialManagement.getContractManagement().getContractName());
				map.put("url",
						"financialManagement/editFinancialManagement.html?popWindowFlag=popWindowFlag&financialManagement.id="
								+ this.financialManagement.getId());
				...
				// );
				// mapData.put("submitNum",
				// this.financialManagement.getSubmitNum());
		...
		if (this.financialManagement.getContractManagement() != null) {
			// 修改合同已付金额
			...
			// 水电收款后逻辑
			if(this.financialManagement.getReturnPlan()!=null){
				ReturnPlan r = this.financialManagement.getReturnPlan();
				r.setPayee(this.financialManagement.getPayee());
				r.setFactSum((this.financialManagement.getTotalSum() + this.financialManagement.getTrueSum()));
				if (this.financialManagement.getWithoutGotSum().doubleValue() == 0.0D) {
					r.setIsOrNot("0");
