...
public class ListProjectInfoCustomerAction extends ValueListAction {
	...
	private String delete() {
		String isKey = "";
		try {
			for (ProjectInfoCustomer pic : projectInfoCustomer) {
				if("1".equals(pic.getIsKeyCustomer())){
					isKey = "1";
				}
			}
			if(!"1".equals(isKey) ){
				this.projectInfoCustomerManager.deleteAllProjectInfoCustomer(this.projectInfoCustomer);
				addActionMessage(getText("projectInfoCustomer.delete.success"));
			}else{
				Cookie cookie = new Cookie("isKey", "1");
				response.addCookie(cookie);
			}
			
			return "success";
		} catch (Exception e) {
			addActionMessage(getText("projectInfoCustomer.delete.failer"));
		}
		return "error";
	}
