	...
	public void prepare() throws Exception {
		if (hasId("billingRecord.id")) {
			...
			if (hasId("returnPlan.id")) {
				...
				this.billingRecord.setContractManagement(returnPlan.getContractManagement());
				this.billingRecord.setReturnPlan(returnPlan);
				if (returnPlan.getContractManagement() != null) {
					this.billingRecord.setCustomerInfo(returnPlan.getContractManagement().getCustomerInfo());
				} else {
					this.billingRecord.setCustomerInfo(returnPlan.getCustomerInfo());
				}
				this.batchs.add(returnPlan.getBatch());
				this.billingRecord.setPlanSum(Double.valueOf(format.format(returnPlan.getSum())));
				if (returnPlan.getBillMoney() != null) {
					this.billingRecord.setHasBillSum(Double.valueOf(format.format(returnPlan.getBillMoney())));
				} else {
					this.billingRecord.setHasBillSum(0.0);
				}
			}
		}
...
//				this.billingRecord.setLastSubmitMoney(billingRecord.getSum());
		...
		if(this.billingRecord.getContractManagement()!=null){
			...
			if (null != this.billingRecord.getReturnPlan()) {
			List<BillingRecord> billList = billingRecordManager.loadByKey("returnPlan", this.billingRecord.getReturnPlan());
			// 判断收款计划是否全部到款，并修改状态
			Double money = 0D;
			for (int i = 0; i < billList.size(); i++) {
				money += billList.get(i).getSum();
			...
			// 修改收款计划中开票相关项
				ReturnPlan r = this.billingRecord.getReturnPlan();
				r.setBillDate(this.billingRecord.getBillingTime());
				r.setBillMoney(money);
				r.setBillingOrNot("1");
				if (money >= r.getSum()) {
