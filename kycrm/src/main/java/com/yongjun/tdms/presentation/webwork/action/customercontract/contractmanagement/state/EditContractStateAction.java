...
public class EditContractStateAction extends FileTransportAction{
	...
	public EditContractStateAction(ContractManagementManager contractManagementManager,ContractStateManager contractStateManager,
			...
			ProjectInfoPlanManager projectInfoPlanManager,HouseListManager houseListManager) {
	...
	public void prepare() throws Exception {
		...
		if(hasId("contractState.id")){
			...
			this.contractManagement = this.contractState.getContractManagement();
		}else{
	...
	public String save() {
		...
		try {
			...
			if (codeValue.getCode().equals("06605")) {
				...
				//房源变为不在使用
				List<HouseList> houseLists =houseListManager.loadByKey("contractManagement.id", contractManagement.getId());
				if(houseLists!=null && houseLists.size()>0){
					for (int i = 0; i < houseLists.size(); i++) {
						HouseList h = houseLists.get(i);
						h.setIsuse(false);
						houseListManager.storeHouseList(h);
					}
				}
			}
			// 合同状态变为已结束
			if (codeValue.getCode().equals("06604")) {
				//房源变为不在使用
				List<HouseList> houseLists =houseListManager.loadByKey("contractManagement.id", contractManagement.getId());
				if(houseLists!=null && houseLists.size()>0){
					for (int i = 0; i < houseLists.size(); i++) {
