...
public class EditContractManagementAction extends PrepareAction {
	...
	public EditContractManagementAction(ContractManagementManager contractManagementManager,
			...
			PersonnelFilesToUserManager personnelFilesToUserManager, ContactToHistoryManager contactToHistoryManager,
			DataManager dataManager, HouseListManager houseListManager,ReturnPlanManager returnPlanManager) {
...
//				this.contractManagement.setContractMoney(new Double(format.format(this.houseListManager
//						.getSum(getId("contractManagement.id")))));
				if(returnPlanManager.loadByKey("contractManagement.id", getId("contractManagement.id"))!=null){
					this.hasReturnPlan = true;
				}else {
					this.hasReturnPlan = false;
				}
			} else {
				...
				this.payType = codeValueManager.loadByKey("code", "06501").get(0);
	...
	public String save() {
		...
		try {
			...
			if ("1".equals(this.request.getParameter("contractManagement.isSaved"))) {
				...
				map.put("url",
						"contractManagement/editContractManagementAction.html?popWindowFlag=popWindowFlag&contractManagement.id="
								+ this.contractManagement.getId());
				map.put("name",
						new SimpleDateFormat("yyyy-MM-dd").format(this.contractManagement.getCreatedTime()) + ","
								+ this.contractManagement.getSaleman().getName() + "提交了合同:"
								+ this.contractManagement.getContractName());
				...
				// mapData.put("thisMoney",
				// contractManagement.getContractMoney());
				// mapData.put("lastMoney",contractManagement.getLastSubmitMoney()
				// );
				// mapData.put("submitNum",
				// this.contractManagement.getSubmitNum());
	...
	public boolean isHasReturnPlan() {
		return hasReturnPlan;
	}

	public void setHasReturnPlan(boolean hasReturnPlan) {
		this.hasReturnPlan = hasReturnPlan;
	}
